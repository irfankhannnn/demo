...
# =============================================================================
# property-pages-ms — Config-Only Deploy
# =============================================================================
# Usage: ./infra/config-deploy.sh <dev|prod>
#
# Same safety model as reality-flow-authentication/infra/config-deploy.sh
...
# what .env.$ENV would produce today (infra/lib/params.sh — the same
# function infra/deploy.sh uses); a changed parameter on the allowlist
...
# diffed (always carried forward from live — it's a freshly timestamped key
# only a real deploy's upload produces).
# =============================================================================
...
echo "============================================="
echo " property-pages-ms — Config-Only Deploy"
echo "============================================="
...
# (describe-stacks masks them as '****'). All four are also on the
# allowlist, so this only affects whether they're diffed, not whether
# they're allowed to change.
NOECHO_PARAMS=(PublicPagesInternalApiKey VisitSessionSecret HcaptchaSecretKey GoogleMapsEmbedApiKey)
...
"$AWS_BIN" cloudformation deploy \
  --template-file "$SCRIPT_DIR/cfn-property-pages.yaml" \
  --stack-name "$STACK_NAME" \
