...
/**
 * PR-C — Cookie consent banner tests (DPDP-compliant).
 *
 * Two groups:
 *  1. LP banner  — the vanilla-JS partial is served on a real https origin via
 *     route fulfillment (so localStorage works) and exercised directly. These
 *     run anywhere, no dev server required.
 *  2. CRM banner — drives the React app at baseURL (http://localhost:3000).
 *     Auto-skips when the dev server is not reachable.
 */
...
test.describe('LP cookie banner (vanilla JS)', () => {
  ...
  test('Customize: 4 toggles; analytics off + marketing on persists correctly', async ({ page }) => {
    ...
    // 4 categories: Essential (locked, no checkbox) + 3 toggles
    ...
    // Accept-all defaults are off in modal until toggled; set analytics off, marketing on
...
test.describe('CRM cookie banner (React)', () => {
  test.beforeEach(async ({ page, baseURL }) => {
    let reachable = false;
    try {
      const res = await page.request.get(baseURL || 'http://localhost:3000', {
        timeout: 4000,
      });
