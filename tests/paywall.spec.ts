/**
 * paywall.spec.ts — Playwright tests for Trial Countdown Banner + PaywallModal
 *
 * Tests mock the subscription API response to simulate different trial states.
 */
...
test.describe('Paywall Modal', () => {
  ...
  test('/billing route still accessible when trial expired', async ({ page }) => {
    ...
    // Paywall modal should NOT show on whitelisted routes
    const modal = page.locator('text=Your trial has ended — pick a plan to continue');
    await expect(modal).not.toBeVisible();
  });

  test('Razorpay checkout opens on tier selection', async ({ page }) => {
    // Mock Razorpay script load
    ...
    // Verify Razorpay was invoked (via our mock)
