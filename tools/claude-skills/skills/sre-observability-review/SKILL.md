...
Telemetry here is CloudWatch only (about 16 alarms, one dashboard, metric filters defined in the CFN templates). X-Ray `TracingConfig` is on in `services/ai-calling-service` and `services/followup-agent-service`. Health checks exist only for `services/whatsapp-platform` on ECS Fargate; Lambda has no liveness or readiness probe. There is no Prometheus, Grafana or Datadog, and no Kubernetes. Deploys are manual, so post-deploy monitoring is a human checklist.
