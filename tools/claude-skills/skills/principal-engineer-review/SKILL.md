...
Codebase: Node/Express on Lambda in `agency-app/api/` (CommonJS, `routes/`, `middleware/`, `*DynamodbService.js`, REST paths `/api/crm/<resource>`), React + TypeScript + Vite frontends in `apps/*` with strict mode on, TypeScript services in `platform/auth` and `platform/mcp`. Error responses are `{ error: string, details?: string }`.
