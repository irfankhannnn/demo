...
- `apps/crm/server/`: Node/Express on Lambda + API Gateway. CommonJS, `*DynamodbService.js` data modules, `routes/` per resource, `middleware/` for auth, tenancy and credit metering. REST paths are `/api/crm/<resource>`.
- `apps/crm/real-estate-crm-app/`, `apps/onboarding/`, `apps/landing-pages/`, `apps/property-pages-ms/`: React + TypeScript + Vite, TailwindCSS, TypeScript **strict** mode.
