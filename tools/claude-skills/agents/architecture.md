...
- `*/infra/*.yaml` / `*.yml` — the CloudFormation templates of an app or service
  (for example `apps/crm/server/infra/cfn-backend.yaml`, `apps/crm/server/infra/apigw-explicit-routes*.yaml`, `apps/crm/server/infra/launch-tables-cfn.yaml`, `services/reality-flow-authentication/infra/auth-explicit-routes.yaml`)
...
- `*/Dockerfile`, `*/docker-compose.yml` — `services/whatsapp-platform` on ECS Fargate
