...
See tools/engineering-change-intelligence/README.md for platform-specific instructions.
EOF
      exit 0
      ;;
    *) GATHER_ARGS+=("$1"); shift ;;
...
Instructions:
1. Read tools/engineering-change-intelligence/MASTER_SYSTEM_PROMPT.md
...
7. Format Slack message per tools/engineering-change-intelligence/templates/slack-message.md
$(if [[ "$POST_SLACK" == "true" ]]; then echo "8. Run: bash tools/engineering-change-intelligence/scripts/post-to-slack.sh $OUTPUT_DIR/release-readiness.md"; fi)
