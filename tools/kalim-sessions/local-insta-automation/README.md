...
**2026-09-17: consolidated here.** Everything for day-to-day manual runs — code,
config, the sample export, the workbook, parsed/analysis JSON, logs — now lives
in this one folder (`kalim-sessions/local-insta-automation/`), which is
untracked/local like the rest of `kalim-sessions/`. The versioned source of
truth for the code stays at
`tools/kalim-sessions/kalim-automations/hp-insta-lead-automation/` (the copy
git tracks and what `.claude/skills/insta-lead-excel/SKILL.md` still points
to) — this folder was copied from there so Kalim has a single place to run
things without hopping between `tools/...` and the old, deeper
`kalim-sessions/kalim-automations/hp-insta-lead-automation/` path. If you
change a script here in a way worth keeping, port it back to the tracked copy
too, since this folder itself is gitignored.
...
```bash
cd tools/kalim-sessions/kalim-automations/hp-insta-lead-automation
...
under `tools/kalim-sessions/` which the repository already ignores. Do not commit
