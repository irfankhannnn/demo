...
Static hosting for RealEstateFlow Homes (`public-app/web`)
...
`agency-app/web/infra/` closely.

This is a raw single-shot deploy. For build/release tracking (numbered
builds, rollback) use the CI/CD wrapper: `infra/cicd/public-app/web/deploy.sh <dev|prod>`.
...
`rollback-content` / `rollback-full`, use `infra/cicd/public-app/web/deploy.sh`.
