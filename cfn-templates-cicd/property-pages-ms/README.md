# cfn-templates-cicd/property-pages-ms

CI/CD wrapper for the public property-pages microservice (`property-pages-ms/`
...
`property-pages-ms/infra/deploy.sh` does the real work: test, install,
...
```bash
cd cfn-templates-cicd/property-pages-ms
...
Reads `property-pages-ms/.env.<env>`. There is no plain `.env` fallback —
...
- **"env file not found"** — copy `property-pages-ms/.env.sample` to
  `.env.dev` or `.env.prod` in `property-pages-ms/` and fill in the
