...
`cfn-templates-cicd/reality-flow-authentication`'s, because the underlying
...
- ~100 of this template's ~120 CFN parameters are consumed by the app at
  ...
  (`server/config/ssmBootstrap.js`), not via CFN `Environment.Variables` — a
  ...
  it only ever needed `server/infra/sync-ssm-params.sh` (already existed,
...
| API | env vars (`server/.env.<env>`) | dev base path | prod base path |
|-----|-------------------------------|---------------|----------------|
...
- The old single-URL vars `AUTH_SERVICE_URL`, `MCP_BASE_URL`, `AI_CALLING_SERVICE_URL`
  ...
  composes URLs in `server/config/serviceUrls.js`, which also rejects any
...
- `deploy`, `config-deploy`, `server/infra/deploy.sh` and `server/infra/config-deploy.sh`
  all run `server/infra/lib/validate-service-endpoints.sh` first: each `*_DOMAIN_NAME`
