...
| `SUBSCRIPTIONS_TABLE` | `dev-realestateflow-subscriptions` | `prod-realestateflow-subscriptions` | Must exactly match what `server/infra/launch-tables-cfn.yaml` names the table for the *same* `EnvironmentName` — this stack only reads it, it doesn't create it. Both stacks now use the same env-first `<env>-realestateflow-*` convention. |
