# cfn-templates-cicd/followup-agent-service

CI/CD wrapper for the follow-up agent microservice (`followup-agent-service/`).
...
`followup-agent-service/infra/deploy.sh` does the real work: test, install,
...
- Node 20+, `zip`
- `followup-agent-service/.env.<env>` filled in (see its `.env.example`)
...
Reads `followup-agent-service/.env.<env>` (gitignored). Stack name is
