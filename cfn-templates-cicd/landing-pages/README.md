...
as `cfn-templates-cicd/real-estate-crm-app` (see that folder's README for
...
`landing-pages/infra/README.md` for what that changes in the CFN template
...
```
cd cfn-templates-cicd/landing-pages
...
`cfn-templates-cicd/common-infra/vpc-networking.yaml`). If
...
1. Resolves `ENV_FILE = landing-pages/.env.<env>` — the argument selects
