...
#
# Examples:
#   ./deploy.sh                       # Full deploy (package + CFN)
#   ./deploy.sh --skip-package        # CFN only (no build/zip/upload)
#   ./deploy.sh --skip-cfn            # Package only (no CFN deploy)
#   ./deploy.sh prod --skip-package   # CFN only, env=prod
#
# Prerequisites: .env file, AWS CLI, Node.js 20.x, zip
#
# NOTE: This script lives in cfn-templates-cicd/reality-flow-mcp/ but deploys
# the reality-flow-mcp/ Lambda source. PROJECT_DIR is resolved two levels up
# + into reality-flow-mcp/ (not just "..") to reach the actual service root.
# =============================================================================

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../../reality-flow-mcp" && pwd)"
