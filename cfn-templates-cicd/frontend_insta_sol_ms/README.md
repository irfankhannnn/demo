# cfn-templates-cicd/frontend_insta_sol_ms

CI/CD wrapper for the Instagram Solution frontend app
(`frontend_insta_sol_ms/`). Same design as the `server` and
`reality-flow-authentication` wrappers in this folder.

## What this adds over the raw deploy script

`frontend_insta_sol_ms/infra/deploy.sh` does the real work: install, package,
