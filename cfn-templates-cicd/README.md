...
- A brand-new service, `real-estate-crm-app/` (S3 + CloudFront static frontend hosting), added in PR #37 — was completely absent from this folder until now.
...
- AI-calling env var wiring from PR #39 (`AiCallingInternalApiKey`, `AiCallingServiceUrl`) — same 3 `server/` files.
...
```
cfn-templates-cicd/
├── server/                        # CRM backend + public/CRM API Gateway routes + Call Intelligence pipeline
├── ai-calling-service/            # AI voice calling Lambda (Exotel + ElevenLabs)
...
| `ai-calling-service/cfn-template.yaml` | `cloudberry-dev-ai-calling-service` | ✅ identical |
| `reality-flow-mcp/cfn-backend.yaml` | `dev-realestate-flow-mcp-stack` | ✅ identical |
| `reality-flow-authentication/cfn-backend.yaml` | `dev-reality-flow-auth-stack` | ✅ identical |
| `reality-flow-authentication/auth-explicit-routes.yaml` | nested `ApiGatewayRoutesStack` (under `dev-reality-flow-auth-stack`) | ✅ identical |
| `whatsapp-platform/cfn-platform.yaml` | `dev-realestate-flow-whatsapp-platform` | ✅ identical |
...
| `real-estate-crm-app/cfn-frontend.yaml` | *(none)* | ❌ no live stack — never deployed (new in PR #37, see Discrepancy 7) |
...
### 4. `reality-flow-authentication/cfn-params.sample.json` is stale
...
### 5. `whatsapp-platform/cfn-params.example.json` is missing `SessionTableName`
Present on both the template and the live stack, absent from the example params file.

### 6. `reality-flow-mcp/cfn-params.sample.json` is missing `FrontendUrl`
...
|---|---|---|
| server | `cd cfn-templates-cicd/server && ./deploy.sh` | Reads `.env` from `server/`; toggle `DEPLOY_LAMBDA`/`DEPLOY_INSTALL`/`DEPLOY_ZIP`/`DEPLOY_CFN` at top of script. Now also provisions the Call Intelligence SQS/Lambda resources — see Discrepancy 7 before running against a live stack |
| ai-calling-service | `cd cfn-templates-cicd/ai-calling-service && ./deploy-lambda.ps1` | Packages source from `ai-calling-service/` via the new `-SourceDir` param (auto-resolved); pass CLI params for secrets rather than relying on the hardcoded defaults in the script |
