...
Deploys `server/infra/launch-tables-cfn.yaml` — six standalone DynamoDB
...
| `Subscriptions` | tenant plan, seats, trial/billing status — read/written by `server/subscriptionService.js` + `server/routes/subscriptions.js`, and read-only by `reality-flow-authentication`'s invite flow (seat-limit check) |
| `NPSResponses` | Net Promoter Score survey responses |
| `BetaInvites` | beta program invite list |

`AIEmployeeProvisioning` is deliberately **not** here — `server/infra/cfn-backend.yaml`
...
`cfn-templates-cicd/README.md` Discrepancy 1 for the historical incident on
...
```
cd cfn-templates-cicd/launch-tables
...
it. `server/.env.launch-tables.dev` / `.env.launch-tables.prod` must exist
first (copy from the `.sample` file in `server/`) — these are separate from
`server/.env.dev` / `.env.prod` (which are for `cfn-backend.yaml`) because
