...
# server/infra/deploy-launch-tables.sh. This wrapper's only job is release
# bookkeeping — see README.md in this folder for the full design.
#
# Unlike every other cfn-templates-cicd/<service> wrapper, this stack has no
# application code (no Lambda, no container) — it's pure
# AWS::DynamoDB::Table resources (server/infra/launch-tables-cfn.yaml). So
...
#     nested-template key — see server/infra/deploy-launch-tables.sh)
...
cmd_deploy() {
  ...
  if [ "$status" = "deployed" ]; then
    ...
    if [ -f "$artifacts_file" ]; then
      ...
      echo "WARNING: deploy reported success but $artifacts_file was not written — server/infra/deploy-launch-tables.sh may be an older version missing this. Artifact keys will be recorded as 'unknown'."
    fi
  fi
