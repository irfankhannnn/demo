param(
  ...
  # Root of the ai-calling-service Lambda source to package. Defaults to the
  # real service directory (../../ai-calling-service from this script's new
  # location under cfn-templates-cicd/ai-calling-service/). Override if the
...
if ([string]::IsNullOrWhiteSpace($SourceDir)) {
  $SourceDir = (Resolve-Path (Join-Path $scriptDir "..\..\ai-calling-service")).Path
}
if (-not (Test-Path $SourceDir)) {
  throw "ai-calling-service source directory not found at $SourceDir. Pass -SourceDir explicitly."
}
...
try {
  ...
  Write-Host "==> Copying backend source from $SourceDir into build directory" -ForegroundColor Cyan
  ...
  if (-not (Test-Path (Join-Path $buildDir "src"))) {
    if (Test-Path (Join-Path $SourceDir "src")) {
      Copy-Item (Join-Path $SourceDir "src") -Destination (Join-Path $buildDir "src") -Recurse -Force
    }
  }
