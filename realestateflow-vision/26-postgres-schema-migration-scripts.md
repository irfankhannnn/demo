...
|---|---|---|
| **Schema** | `server/db/schema.ts` | Drizzle ORM definitions |
| **Connection** | `server/db/client.ts` | RDS Proxy pool + Drizzle instance |
| **Middleware** | `server/middleware/tenantContext.ts` | Set `app.tenant_id` per request |
| **Services** | `server/services/*.ts` | Lead, Contact, Property, etc. CRUD |
| **Migrations** | `server/db/migrations/*.sql` | DDL + RLS policies (auto-generated) |
| **Validation** | `scripts/validateParity.ts` | Dual-write verification |
