...
description: >
  Deploy a RealtyFlow/Cloudberry microservice through its cfn-templates-cicd
  CI/CD wrapper, but only after checking it against this repo's CFN/CI-CD
  ...
  cfn-templates-cicd/<service> path (or a bare service name) and an
...
- `WRAPPER_DIR` = the given `cfn-templates-cicd/<service>` path (resolve a bare name against `cfn-templates-cicd/*` — if more than one matches, or none do, list what exists and ask which one).
- `SERVICE_NAME` = `basename(WRAPPER_DIR)`.
- `SERVICE_DIR` = `<repo-root>/<SERVICE_NAME>` — the established convention in this repo is that the wrapper folder name and the microservice folder name at repo root are identical. Verify `SERVICE_DIR` exists. If it doesn't, or if `WRAPPER_DIR/deploy.sh` doesn't reference `SERVICE_DIR/infra/deploy.sh` anywhere, **stop and ask** which real microservice folder this wrapper is supposed to delegate to — do not guess a mapping.
