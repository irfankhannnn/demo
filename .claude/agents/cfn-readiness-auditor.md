...
- `SERVICE_DIR` — the microservice's own folder (e.g. `agency-app/api`, `platform/auth`)
- `WRAPPER_DIR` — its CI/CD wrapper (e.g. `infra/cicd/agency-app/api`)
- `ENV` — `dev` or `prod`
...
`infra/cicd/platform/auth` is the reference
