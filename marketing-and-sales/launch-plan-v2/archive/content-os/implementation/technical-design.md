...
The authoritative engineering design for the in-product GTM layer. Grounded in the **real** RealEstateFlow stack: Express on **AWS Lambda** (`agency-app/api/lambda-handler.js`, `agency-app/api/server.js`), **DynamoDB single-table** (`PK`/`SK`/`GSIn`, `EntityType`, `TENANT#` prefix) accessed via `crmDynamodbService.js` and `notificationDynamodbService.js`, multi-tenancy via `tenantMiddleware.js` (`extractTenantId`), JWT auth (`middleware/validateToken.js`), and the separate `agency-app/ai-calling/` Lambda (Exotel + ElevenLabs). Marketing MCPs (`higgsfield`, `meta-ads`, `blotato`) live in `.mcp.json`.
...
| `agency-app/ai-calling/` | reuse | Auto-qualify call on qualified inbound lead (invoked async). |
...
| Auto-qualify call | `agency-app/ai-calling/` Lambda (Exotel + ElevenLabs) |
