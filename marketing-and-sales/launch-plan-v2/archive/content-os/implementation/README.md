...
Express + AWS Lambda (`agency-app/api/lambda-handler.js`) · DynamoDB single-table (`PK/SK/GSI`, `EntityType`, `TENANT#`) · multi-tenant (`tenantMiddleware.js`) · notification + scheduled-notification engine (`notificationDynamodbService.js`) · `agency-app/ai-calling/` (Exotel+ElevenLabs) · React/Vite frontend · RBAC (`rbac.ts`) · MCPs (higgsfield/meta-ads/blotato). **Confirmed gap:** leads have no source/UTM/attribution/score today.
