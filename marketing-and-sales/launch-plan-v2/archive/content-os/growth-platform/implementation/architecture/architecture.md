...
The end-state architecture of the Growth Platform on the **real** RealEstateFlow AWS stack: Express on AWS Lambda behind API Gateway (`agency-app/api/lambda-handler.js`), DynamoDB single-table (`PK/SK/GSIn`, `EntityType`, `TENANT#`), `crmDynamodbService.js` / `notificationDynamodbService.js`, separate `agency-app/ai-calling/` Lambda, React/Vite frontend (`agency-app/web/`), CloudFormation deploy. MCPs: `higgsfield`/`meta-ads`/`blotato`. **Additive** — consolidates `marketing-and-sales/launch-plan-v2/60-automation/README.md` (links here). Components/services detail in `../technical-designs/technical-designs.md`.
