...
|---|---|---|---|
| A1 Add attribution attrs to lead create/update | `apps/crm/server/crmDynamodbService.js` `createLead@2205`, `updateLead@2330` | new leads persist `leadSource,utm*,campaignId,contentRef,consent,firstTouchAt(immutable),lastTouchAt,dedupeHash` | 100% new leads attributed |
| A2 Accept attribution on API + UTM capture | `apps/crm/server/routes/leads.js` (POST/PUT); `real-estate-crm-app` signup/lead forms | source/UTM accepted + validated (enum) | % attributed |
...
| Task | File(s) | Acceptance | Metric |
|---|---|---|---|
| B1 `marketingEventsService.js` (normalize→dedupe→persist→enqueue) | new `apps/crm/server/marketingEventsService.js` | provider payload → canonical `MKT_EVENT`; conditional put on `dedupeKey` | <1s ingest→routed (p95) |
| B2 REST ingest route | new `apps/crm/server/routes/marketingEvents.js` (`POST /api/marketing/events`, JWT + `extractTenantId`) | authed event accepted, written, enqueued | events ingested |
| B3 Webhook routes (IG/WhatsApp/Meta Lead Ads/Blotato) | new `apps/crm/server/routes/webhooks/*.js` | HMAC verify → tenant resolve → hand to B1; no business logic | signed webhooks only |
...
| B5 `attributionResolver.js` (TOUCHPOINT + ATTRIBUTION_PATH + credit) | new `apps/crm/server/attributionResolver.js` | on each touch: write TOUCHPOINT (GSI9), update PATH credit (first/last/U/linear/decay); revenue on `paid` | per-lead path renders |
| B6 Mount routes | `apps/crm/server/lambda-handler.js` / `apps/crm/server/server.js` | new routes registered behind API GW | 200s in prod |
...
| Task | File(s) | Acceptance | Metric |
|---|---|---|---|
| C1 Dashboard read API | new `apps/crm/server/routes/marketingDashboard.js` (`GET /dashboard/attribution`, `/funnel`, `/path`) | groupBy=source/content/campaign + model=first/last/multi from GSIs | matches sheet formulas |
| C2 React dashboard | new `apps/crm/real-estate-crm-app/src/pages/crm/MarketingDashboard.tsx` | renders D1–D5 (`dashboards.md`); date-range + tenant scope | live funnel + OPP-* ROI |
| C3 RBAC gate | reuse `apps/crm/real-estate-crm-app/src/utils/rbac.ts`, `PermissionGuard.tsx` | Admin full; Member sees own-leads slice only | no cross-tenant read |
| C4 Path tab on lead detail | `apps/crm/real-estate-crm-app/src/pages/crm/LeadDetails.tsx` | TOUCHPOINT timeline per lead | journey visible |
...
| D3 `contentRoiService.js` rollup + classify | new `apps/crm/server/contentRoiService.js` (nightly via EventBridge) | per-`OPP-*` reach→paid; class = customer-maker/activation/engagement/dud | weekly content-ROI review |
