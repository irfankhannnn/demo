...
Complete `TENANT#`-scoped entities for attribution, **additive** to the existing CRM table (`CRM_TABLE_NAME`) accessed via `apps/crm/server/crmDynamodbService.js`. No attribute is removed/repurposed. Naming follows the verified convention: `PK`/`SK`/`GSInPK`/`GSInSK`, `EntityType`, `TENANT#{tenantId}` prefix, ISO-8601 `createdAt`/`updatedAt`.
...
Existing item (`apps/crm/server/routes/leads.js`, `apps/crm/server/crmDynamodbService.js` `createLead@2205`/`updateLead@2330`): `name, phone, email, leadType, status, priority, assignedTo, createdBy, createdAt, updatedAt`. **Add** (full table in `database-requirements.md` §1 — do not fork; summary for self-containment):
