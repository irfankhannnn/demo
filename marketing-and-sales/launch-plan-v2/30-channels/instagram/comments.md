...
>
> Each keyword doubles as the **rule trigger** in `dm-workflows.md` §1. Comment-to-private-reply automation is built — the keyword rules live in `apps/instagram/backend_insta_sol_ms` (`services/ruleMatcher.js`) and are driven from the Instagram console. The service is still in Meta **Development Mode**, and `INSTA_DRY_RUN_SENDS` is on in dev, so it cannot reach real followers until App Review passes. Until then a person sends the DM manually within 15 minutes. Replies are drafted first and never auto-sent unreviewed. Graph API only — never browser automation.
>
