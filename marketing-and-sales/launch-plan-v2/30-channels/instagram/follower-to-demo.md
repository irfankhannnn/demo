...
- **What the CRM stores:** `source: 'Instagram'`, `sourceAdapter: 'instagram'`, `externalRef`, `reelRef` (`agency-app/api/leadIngestion.js`). There is no per-surface source value (`instagram_reel` / `_story` / `_dm`), no `contentRef`, no `utm` on a lead, and `/api/marketing/events` does not exist.
...
- **Instrumentation that does exist:** UTM is captured at signup (`utm_source`, `agency-app/api/routes/auth.js`) and PostHog is wired into the CRM. See `../../50-measurement/posthog-event-map.md` and `../../50-measurement/attribution-today.md`.
