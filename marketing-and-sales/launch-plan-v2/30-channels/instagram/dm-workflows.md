...
**Assets must exist before the keyword ships.** The Leakage Audit, the follow-up tracker and the setup guide are not built yet — the on-site calculator is a placeholder (`apps/landing-pages/agency-owners/index.html`). A keyword that promises a file we cannot send is worse than no keyword.
...
**Which WhatsApp number does this?** Today the product's WhatsApp is a self-hosted Baileys service (`services/whatsapp-platform/`) used as the agency's own command channel, not a Business-API sender. Which number talks to prospects is not settled.
...
A lead created from Instagram carries `source: 'Instagram'`, `sourceAdapter: 'instagram'`, `externalRef` and `reelRef` (`apps/crm/server/leadIngestion.js`), and emits an EventBridge `lead.created` that runs AI qualification. There is **no** `leadSource`, `contentRef` or `utm` field on a lead, and `/api/marketing/events` is not built.
