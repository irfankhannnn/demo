...
- **The Instagram service is real and is not live.** Comment keyword rules, private replies, DM reading, enquiry scoring and the CRM hand-off are built (`apps/instagram/backend_insta_sol_ms/services/ruleMatcher.js`). It is deployed to dev, **still in Meta Development Mode with App Review pending**, and `INSTA_DRY_RUN_SENDS` is on in dev. Until App Review passes, a person sends every reply by hand, within 15 minutes.
