...
This folder is the **delta plan** produced by applying the uploaded *AI Employee & CRM — Master Prompt* (`HappyProperties-MasterPrompt.md`) against the existing `launch-plan-v2/` playbook.

> **Product name:** the product is **RealEstateFlow** (`realestateflow.in`). The whole of this pack is written for RealEstateFlow.

> **About "Happy Properties":** the master prompt's file name and copy used **"Happy Properties"** as if it were the product. That was a mistake in the prompt. **Happy Properties is a broker company — the first client / pilot user of this CRM** (it is literally the sample agency name shown in the product's own signup and profile forms: `real-estate-crm-app/src/pages/Profile.tsx`, `onboarding-page/web/index.html`). Throughout this pack, "Happy Properties" is therefore treated as the **first/pilot broker client**, never as the product brand.
...
| First/pilot client | **Happy Properties** (a broker company) — the sample agency used in onboarding/profile and a natural pilot/testimonial reference |
...
The master prompt is a useful **strategic template**, but it carried two inaccuracies and a market mismatch versus the locked plan:

| Dimension | Master prompt (as written) | Reality / locked plan |
|---|---|---|
| Product name | "Happy Properties" | **RealEstateFlow** — and "Happy Properties" is the **first broker client**, not the product |
...
| `01-BRAND-POSITIONING-RECONCILIATION.md` | §1, §6 | Confirms RealEstateFlow as the product (and Happy Properties as first client); the messaging hierarchy + brand voice to adopt; the remaining palette/typography decision |
