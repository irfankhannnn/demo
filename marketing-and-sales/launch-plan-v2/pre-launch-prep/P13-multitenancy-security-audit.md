...
## Acceptance Criteria
- [ ] Static analysis CSV at `marketing-and-sales/launch-implement/pre-launch/13-security/route-tenant-coverage.csv` listing every route in `agency-app/api/routes/*.js` with columns: `file, method, path, hasValidateToken, hasExtractTenantId, dynamoCallsCount, dynamoCallsWithTenantIdInKey, isPublic, securityNotes`
...
Read these inputs:
- `agency-app/api/routes/*.js` (every file — auth, leads, owners, buyers, properties, tenants, b2bLeads, khata, calendar, hierarchy, analytics, rentedProperties, notifications, areas, developers, projects, grievance (P9), billing (P11), aiEmployeeStatus (P11), subscriptions (P12))
- `agency-app/api/tenantMiddleware.js`
- `agency-app/api/middleware/*.js` (validateToken, etc.)
- `agency-app/api/crmDynamodbService.js` (108KB — find each function's DDB key conditions)
- `agency-app/api/grievanceDynamodbService.js` (P9)
- `agency-app/api/aiEmployeeProvisioningService.js` (P11)
- `agency-app/api/subscriptionService.js` (P12)
- `agency-app/api/server.js` (route mounting)
