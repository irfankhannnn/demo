...
- **`app.realestateflow.in`** (CRM SPA) — `agency-app/web/` — React/Vite SPA, deployed separately
...
2. **CRM analytics module** (`agency-app/web/src/lib/analytics.ts`) — PostHog ONLY, with `posthog.identify()` post-login
3. **Server PostHog SDK** (`agency-app/api/lib/posthog.js`) — server-side events for payments, grievances, provisioning
4. **Sentry** — SPA (`src/main.tsx`) + Server (`agency-app/api/lambda-handler.js`) only
...
### CRM SPA Surface (`app.realestateflow.in` — `agency-app/web/`)
- [ ] `posthog-js` added to `agency-app/web/package.json`; initialised in `src/main.tsx`
...
- [ ] PostHog Node SDK in `agency-app/api/lib/posthog.js` for server-side events
- [ ] Sentry DSN configured in `agency-app/api/lambda-handler.js`; `SENTRY_DSN_SERVER` env var
...
Read these inputs:
- `agency-app/web/src/App.tsx` (existing routes + initAuth logic)
- `agency-app/web/src/main.tsx` (entry point)
- `agency-app/web/src/pages/**/*.tsx` (enumerate user actions)
- `agency-app/api/routes/*.js` (server actions)
- `agency-app/api/lambda-handler.js` (Lambda entry)
...
## 2. `agency-app/web/src/lib/analytics.ts` — CRM analytics module (PostHog ONLY)
...
## 3. `agency-app/web/src/components/CookieConsentBanner.tsx` — CRM version (PostHog gate only)
Tailwind banner:
...
In `agency-app/web/src/pages/PhoneLogin.tsx` (the CRM signup entry):
...
## 6. `agency-app/api/lib/posthog.js` — Server-side PostHog
- Wrapper around `posthog-node`
- `serverTrack(distinctId, event, properties)` — called from billing webhook, grievance route
- Initialise in `agency-app/api/lambda-handler.js`
- Add to `agency-app/api/routes/billing.js` (P11) for the 7 server payment/subscription events
- Add to `agency-app/api/routes/grievance.js` (P9) for `grievance_received`
...
2. **LP env vars** → `creative/landing-pages/.env` (build-time, NOT deployed to git):
   ...
   - `LINKEDIN_PARTNER_ID`
   - `HOTJAR_ID`, `HOTJAR_SV`
3. **CRM env vars** → `agency-app/web/.env` (Vite, NOT deployed to git):
