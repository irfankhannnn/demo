...
The authoritative engineering design for the in-product GTM layer. Grounded in the **real** RealEstateFlow stack: Express on **AWS Lambda** (`server/lambda-handler.js`, `server/server.js`), **DynamoDB single-table** (`PK`/`SK`/`GSIn`, `EntityType`, `TENANT#` prefix) accessed via `crmDynamodbService.js` and `notificationDynamodbService.js`, multi-tenancy via `tenantMiddleware.js` (`extractTenantId`), JWT auth (`middleware/validateToken.js`), and the separate `ai-calling-service/` Lambda (Exotel + ElevenLabs). Marketing MCPs (`higgsfield`, `meta-ads`, `blotato`) live in `.mcp.json`.
...
| `ai-calling-service/` | reuse | Auto-qualify call on qualified inbound lead (invoked async). |
...
| Auto-qualify call | `ai-calling-service/` Lambda (Exotel + ElevenLabs) |
