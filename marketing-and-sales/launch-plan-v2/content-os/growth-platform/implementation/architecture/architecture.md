...
The end-state architecture of the Growth Platform on the **real** RealEstateFlow AWS stack: Express on AWS Lambda behind API Gateway (`server/lambda-handler.js`), DynamoDB single-table (`PK/SK/GSIn`, `EntityType`, `TENANT#`), `crmDynamodbService.js` / `notificationDynamodbService.js`, separate `ai-calling-service/` Lambda, React/Vite frontend (`real-estate-crm-app/`), CloudFormation deploy. MCPs: `higgsfield`/`meta-ads`/`blotato`. **Additive** — consolidates `../../automation-os/architecture.md` (links here). Components/services detail in `../technical-designs/technical-designs.md`.
...
Cross-refs: `../technical-designs/technical-designs.md`, `../../automation-os/architecture.md`, `../../../implementation/{database,api,infrastructure}-requirements.md`, `../../analytics/dashboard-strategy.md`.
