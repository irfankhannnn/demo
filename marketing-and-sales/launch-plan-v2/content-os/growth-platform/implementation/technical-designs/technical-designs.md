...
The authoritative engineering design for the **whole** Growth Platform. **Extends + consolidates** `../../../implementation/technical-design.md` (which holds the base flows §1–7) and `../../automation-os/architecture.md` — this doc is now the single source of truth; those link here. Grounded in the real stack: Express on AWS Lambda (`server/lambda-handler.js`, `server/server.js`), DynamoDB single-table (`PK/SK/GSIn`, `EntityType`, `TENANT#`) via `crmDynamodbService.js` + `notificationDynamodbService.js`, `tenantMiddleware.js`, `validateToken.js`, separate `ai-calling-service/` (Exotel+ElevenLabs). **Additive only.**
...
| `ai-calling-service/` | reuse | auto-qualify call (async) |
