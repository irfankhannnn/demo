...
| `first_ai_call` | **`ai-calling-service/` callback** (`aiCallingInternal.js`) | first AI call placed/qualified | **AHA**, M3 | M3 |
...
```
Fired from the `ai-calling-service/` outcome callback (Exotel+ElevenLabs). `live_with_cs` lets the dashboard separate self-serve aha vs assisted aha.
...
1. **Where to instrument:** UI/route handlers emit raw events via `POST /api/marketing/events` (REST, JWT) or the `ai-calling-service/` callback; `marketingEventsService` normalizes + dedupes (`technical-design.md` §1). UI never computes scores — it only emits facts.
