...
### The fastest path
1. Open `production-sop/11-prompt-library.md`.
...
| Build the founder's presence | `distribution-os/founder-brand.md`, `founder-engine.md` | Personal-brand demand |
| Run the sale | `sales-os/` (customer-journey → qualification → demo-script → objections → closing-script) | DM → demo → paid |
| Onboard & retain | `sales-os/onboarding-script.md`, `followup-script.md` | Paid → activated → referral |
...
| Sequence execution | `roadmap-30-60-90.md` | 30/60/90 plan |

**End-to-end conversion path (the spine):**
`reel (OPP-*)` → attention (DM) → distribution (channel) → `sales-os/qualification.md` (band) → demo → trial → paid → `referral`.
...
- Metrics → `growth-platform/analytics/metric-dictionary.md` (never define a metric anywhere else)
- DB entities → `growth-platform/implementation/technical-designs/technical-designs.md`
- Workflows → `growth-platform/automations/workflow-catalog.md`
...
The system is **spec-complete** but the product (`server/`, React app) does not yet measure anything. **Confirmed gap from the code: leads have no source / UTM / attribution / score today.** That is the P0 gate — until it ships, scoring, dashboards, and automations have no data.
...
Full sequencing: `growth-platform/implementation/roadmaps/roadmap-30-60-90-180.md` and `roadmap-30-60-90.md`.
...
All definitions live in `growth-platform/analytics/metric-dictionary.md`.
...
|---|---|---|
| **Founder / CEO** | `master-index.md` §0 | `roadmap-30-60-90.md` + `growth-platform/implementation/roadmaps/roadmap-30-60-90-180.md` |
| **Content / Social** | `production-sop/11-prompt-library.md` | `frameworks/`, `characters/`, `workspaces/realestateflow/` |
| **Distribution / Growth** | `GTM-OS-README.md` | `distribution-os/`, `attention-os/`, `growth-platform/campaigns/` |
| **Sales / CS** | `sales-os/customer-journey.md` | `sales-os/qualification.md`, `growth-platform/{lead-scoring,activation,customer-scoring}/` |
| **Engineering** | `implementation/epics.md` | `growth-platform/implementation/epics/epics.md` → `technical-designs/` → start EP-1/EP-2 |
| **Analytics / BI** | `growth-platform/analytics/metric-dictionary.md` | the 6 dashboard files |
...
1. **Brand truth is upstream of everything** — `/.brand/brand-kit.md` + `/.brand/positioning.md`. `#2563EB`, Inter, Hinglish 70/30 (+Marathi), mobile-first, ₹/lakh/crore, RERA-safe.
