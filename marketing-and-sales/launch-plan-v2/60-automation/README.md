...
|---|---|---|
| Lead ingestion, one door | every automated lead enters through `ingestLead()`, which sets `source`, `sourceAdapter`, `externalRef`, `dedupeKey`, dedupes on normalised phone, and emits an EventBridge `lead.created` | `agency-app/api/leadIngestion.js`, `docs/lead-adapter-architecture.md` |
| Lead qualification | EventBridge rule on `crm.leads` / `lead.created` → `lead-qualifier-handler`, which scores HOT/WARM/COLD against the shared rubric and emits `lead.qualified` | `agency-app/api/infra/cfn-backend.yaml` (lead-qualifier rule), `scripts/lead-qualifier-handler.js`, `utils/leadRubric.js` |
...
| Inbound WhatsApp | `whatsapp.incoming` / `message.received` → WhatsApp processor | `cfn-backend.yaml`, `scripts/whatsapp-message-processor.js`, `platform/whatsapp-platform` |
| Instagram comments and DMs | Meta webhook → keyword rule matcher → lead analyst (rules + Gemini) → CRM hand-off | `agency-app/instagram-api/routes/webhooks.js`, `services/ruleMatcher.js`, `services/leadAnalyst.js`, `services/crmBridge.js` |
| AI follow-up calls | scheduled, retried twice 45 minutes apart inside the tenant's business hours, escalated to a human when needed | `agency-app/followup-agent` |
...
| Billing lifecycle | Razorpay webhooks drive provisioning, invoicing, failures, refunds, cancellations, seats | `agency-app/api/routes/billing.js` |
