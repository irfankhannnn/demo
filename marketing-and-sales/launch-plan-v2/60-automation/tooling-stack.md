...
|---|---|---|---|
| Razorpay | subscriptions, payments, refunds, GST invoicing | in code | `apps/crm/server/routes/billing.js`, `razorpayOrders.js`; commercial terms in `pricing.json` |
| PostHog | product and funnel analytics, client and server | in code | `src/lib/analytics.ts`, `apps/crm/server/lib/posthog.js` |
| Instagram Graph API | comments, DMs, media, insights, keyword rules | in code, **dev only** | `apps/instagram/backend_insta_sol_ms`; prod blocked on Meta App Review |
| WhatsApp (self-hosted Baileys) | the product's WhatsApp channel and the AI Employee's transport | in code | `services/whatsapp-platform`; inbound via EventBridge `whatsapp.incoming` and `apps/crm/server/routes/webhooks.js` |
| Exotel + ElevenLabs | AI calling | in code | `services/ai-calling-service`; metered in credits (`creditConfig.js`) |
| Follow-up agent | scheduling, retry and escalation of AI calls | in code | `services/followup-agent-service` |
| ManyChat | property-page conversations | in code | `apps/crm/server/routes/webhooks.js`, `docs/property-pages/02-MANYCHAT-SETUP.md` |
| Amazon SES | transactional email from the CRM | in code | `apps/crm/server/emailService.js` |
| Brevo | transactional email in the launch stack, including trial emails | in code / planned | `apps/crm/server/scripts/trial-reminder-cron.js`; stack choice in `00-PLAN-OVERVIEW.md` §7 |
| MCP server | 50+ CRM tools exposed to agents | in code | `services/reality-flow-mcp` |
| AWS EventBridge | the domain event bus (`lead.created`, `lead.qualified`, `whatsapp.incoming`) and nine scheduled rules | in code | `apps/crm/server/infra/cfn-backend.yaml` |
...
| GA4, Meta Pixel, LinkedIn Insight, Hotjar | landing-page measurement | in code, consent-gated | `apps/landing-pages/_partials/head-analytics.hbs` |
...
|---|---|---|
| Tenant / AI Employee messaging | `services/whatsapp-platform`, self-hosted Baileys, per tenant | in code |
