...
>
> They are not interchangeable. A trial user **cannot** reach the AI Employee milestone — it is a paid add-on with no trial (`../pricing.json`). An AI call, by contrast, runs on the trial's included credit allowance (`apps/crm/server/creditConfig.js`), so a trial user can reach it on day two.
>
