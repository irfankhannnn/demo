...
**What exists today.** A lead carries `source`, `sourceAdapter`, `externalRef` and `reelRef` (`apps/crm/server/leadIngestion.js`) and emits EventBridge `lead.created`, which runs AI qualification. UTM is captured at signup (`utm_source`, `apps/crm/server/routes/auth.js`). PostHog is wired into the CRM. Razorpay holds the billing truth. There is an NPS surface in the product.
