...
> **The lead already carries an AI score.** `lead.created` fires an EventBridge event that runs the AI lead qualifier (`agency-app/api/scripts/lead-qualifier-handler.js`), so every lead arrives with a temperature. Read that first, then apply your own band below. The two are independent readings, not a hierarchy.
