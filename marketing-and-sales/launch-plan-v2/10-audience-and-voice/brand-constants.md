...
| Demo page | `realestateflow.in/demo/` | `agency-app/landing-pages/demo/index.html` |
| Signup | `app.realestateflow.in/signup` | `agency-app/landing-pages/` nav on every page |
...
The CRM app uses blue `#2563EB` and Inter (`agency-app/web/tailwind.config.js`). That is the **product UI**, and it legitimately differs from the marketing brand. A screen recording of the real CRM will therefore not match the marketing palette, and that is correct. Never edit product-UI colour references to the brand-kit values.
...
| CRM product UI | English, no i18n. The AI agent speaks Hinglish on calls | `agency-app/web/src/pages/crm/AICalling.tsx` |
