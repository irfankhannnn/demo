...
**Purpose:** the `[AI]` generation layer of the content engine. This maps the 25 frameworks (`FW-*`), the cast (`CH-*`) and the visual presets (`VP-*`) onto Higgsfield's image and video generation, and gives copy-paste recipes.

> Upstream context:
> - Frameworks → [`framework-library.md`](framework-library.md) (25 × `FW-*`)
> - Cast → [`cast-and-presenter.md`](cast-and-presenter.md) (each with a consistency prompt)
> - Visual system → [`visual-system.md`](visual-system.md) (`VP-CAM-*`, `VP-LIGHT-*`, `VP-MOTION-*`, scene bundles)
> - Pipeline → [`production-pipeline.md`](production-pipeline.md) (where this fits, and the `[AI]`/`[REC]`/`[ED]` routing)
> - MCP wiring → repo `.mcp.json` (`higgsfield` = `https://mcp.higgsfield.ai/mcp`, HTTP transport, OAuth)
> - Brand truth → `marketing-and-sales/creative/realestateflow-launch/brand-kit.md` — v3 "Bazaar Signal": ink `#1C1512` and paper `#FBF2E4` grounds, marigold `#FF7A1A` accent, gulal `#FF3D7F` once per screen, tulsi `#1FAA59` for checkmarks only; Unbounded 800/900 display + Manrope body; gradient marigold→gulal
> - Honesty rules → [`../10-audience-and-voice/claims-and-proof-policy.md`](../10-audience-and-voice/claims-and-proof-policy.md). **Nothing generated here may depict a customer, a testimonial or a real person's endorsement.**

> **The June research below (§0–§1) is a snapshot.** Read live tool names on connect. Month-1 production drives the hosted server through `get_workflow_instructions()` and the `soul_2` workflow (see `marketing-and-sales/realestateflow/content-strategy-first-month/11-GENERATION-PROMPTS.md`); the table in §1.8 is a third-party reference implementation, not the hosted contract.
...
**Call `get_workflow_instructions()` first and use the workflow names it returns.** That is how month-1 production actually runs, and the character work goes through the `soul_2` workflow. The table below is the *shape* to expect (image / video / character / status / list), taken from a public reference implementation — do not hardcode these names.

| Tool (reference impl.) | Purpose | Key params |
|---|---|---|
...
- Whether per-clip audio/voice is generated by Higgsfield for our use — ⚠️ we keep voice on **ElevenLabs** regardless (voice-overs are ElevenLabs-only). Higgsfield has no usable Indian male voice preset, so the presenter's voice is a cloned Hinglish ElevenLabs voice.
...
### HF-PROOF-HOME — handover scene — **restricted**
- **Produces:** warm golden-hour two-shot at a new home, keys in hand.
- **Restriction:** this may only be used inside a **clearly fictional, labelled skit**. It may never be used as proof, as a testimonial, or as a "happy customer" of RealEstateFlow. The "happy customer" couple it was built around has been removed from the cast. `FW-CUSTOMER` and `FW-CASE` are disabled pre-launch, so in practice this workflow is idle until we have a real, consented customer — at which point the customer appears as themselves and this workflow is still not what you want.
- **Model/feature:** HF-IMAGE-2SHOT → image-to-video, soft motion.
- **Camera:** `VP-CAM-2SHOT` + `VP-CAM-INSERT` (keys).
- **Motion:** slow push / `VP-MOTION-DRAMA`-lite.
- **Use:** FW-BAB "after" beat in labelled fiction only.

### HF-INTRO-STING — Gradient logo sting
- **Produces:** 0.5s brand intro/outro (gradient `#FF7A1A→#FF3D7F` on ink, marigold→gulal, + logo).
...
> The "happy customer" pair from the June draft has been removed. No Soul is trained for a customer role.
...
| **FW-CUSTOMER** — *disabled pre-launch* | — | — | — | — | — |
...
**Output spec (default):** 1080×1920, **9:16**, 24–30 fps, H.264 MP4, audio −14 LUFS, subtitles burned in (Manrope 700, key word in marigold), logo bug bottom-left (except raw UGC), ≤ 90s reel (30–45s is the working target). File named `REF-<TYPE><NN>-<slug>-v<N>.mp4`.

### Step 1 — Inputs
- A script built from ONE `FW-*` (structure + hook + CTA + language) from [`framework-library.md`](framework-library.md).
...
- Generate UI/number/text cards with **Nano Banana Pro** (`nano-banana-pro` skill) and/or build pixel-accurate typography, intro/outro sting and kinetic type in **Remotion** (`marketing-and-sales/video-projects/my-video/`, rendered with `tools/claude-skills/scripts/render-remotion.ps1`) or **HyperFrames**. Real product UI means a real `[REC]` screen recording — never AI-faked UI.

### Step 6 — Voice & subtitles
- Generate Hinglish VO via **ElevenLabs** (`tools/claude-skills/scripts/elevenlabs-tts.ps1`) — a cloned Hinglish voice, since Higgsfield has no usable Indian male preset. Subtitles in romanized Latin for spoken Hinglish; Devanagari only as a deliberate title accent, never inside a romanized word.

### Step 7 — Stitch & finish (FFmpeg / Remotion)
- Assemble clips + cards + VO + music + captions. Programmatic assembly in **Remotion** (`marketing-and-sales/video-projects/my-video/`) or **HyperFrames** for branded/templated pieces; **FFmpeg** for concatenation, trims, audio mix, burn-in and final encode. Multi-shot narrative reels are an `[ED]` job — hand them over as a job card instead.
- Apply pacing (`VP-MOTION-*`): first cut within 1s, cut every 1.5–3s, one brand accent per frame.
- Add `HF-INTRO-STING` (marigold→gulal logo sting) on hero pieces only.

### Step 8 — QC against the 10 Non-Negotiables ([`visual-system.md`](visual-system.md) §9)
- 9:16 · Unbounded/Manrope · one accent per frame (marigold *or* gulal) · hook visual <1s · subtitles on · product on phone/laptop and the UI is real · real Indian faces, natural light · logo bug · cast per `cast-and-presenter.md`. Then the claims check: no customer, count or testimonial; no unsourced figure; no AI face posing as a real person; AI Employee mentions carry the price and no-trial line. Reject and regenerate if any fail.

### Step 9 — Export & route
- Export 1080×1920 MP4 named `REF-<TYPE><NN>-<slug>-v<N>.mp4` → `marketing-and-sales/creative/` (there is no `marketing-and-sales/assets/`), and log the row in `marketing-and-sales/realestateflow/content-strategy-first-month/13-ASSET-TRACKER.csv`.
- Publishing route is not settled: the `blotato` MCP is configured and can schedule IG/FB/LinkedIn posts; Meta Business Suite upload is the manual fallback; stories, broadcast channels and group actions stay manual either way. No paid ads in M1, so the `meta-ads` route is off until the PMF gate.
...
photorealistic, golden-hour handheld candid feel with slight grain, warm marigold #FF7A1A accent on a warm near-black or cream ground,
...
mid shot slight low angle, shallow depth of field, vertical 9:16, warm marigold #FF7A1A accent"
...
`…, graded VP-GRADE-CLEAN (bright true-to-life)` / `VP-GRADE-MOODY (low-key amber accents)` / `VP-GRADE-GOLDEN (teal-orange golden hour)` / `VP-GRADE-BAZAAR (crisp cool, vivid blue)`.
...
5. **Supercomputer Skill packaging** — once an `HF-*` recipe is stable, publish it as a slash workflow (`/realestateflow-drama`, `/realestateflow-ugc`, `/realestateflow-demo`) so one brief → finished, on-brand, production-grade reel. Version + share across the team. **Reference template + schema: [`higgsfield-skills.md`](higgsfield-skills.md).** 🟡 verify plan access.
6. **Higgsfield Marketing Studio** — use for branded variants when we want platform-native polish fast; hold the brand tokens and Unbounded/Manrope type via Remotion or HyperFrames overlays rather than trusting generated text.
...
|---|---|
| Higgsfield video model unavailable / poor result | Generate high-quality Soul / **Nano Banana Pro** stills → animate and composite in **Remotion** (`marketing-and-sales/video-projects/my-video/`, `tools/claude-skills/scripts/render-remotion.ps1`) or HyperFrames; Ken-Burns + text motion via FFmpeg. |
...
| Number/text/Hinglish typography off-brand | Render text and ₹ cards in **Remotion** / **Nano Banana Pro** with Unbounded + Manrope and the exact v3 tokens; never rely on AI text-in-image. |
| Voice / VO | Always **ElevenLabs** (`tools/claude-skills/scripts/elevenlabs-tts.ps1`) — do not depend on Higgsfield audio, which has no usable Indian male preset. |
