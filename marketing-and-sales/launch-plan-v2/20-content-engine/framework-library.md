...
**25 reusable frameworks.** Each has a stable ID (`FW-*`). A script = framework structure + cast + hook + CTA + language. The structures are business-agnostic; the examples are skinned for RealEstateFlow.
...
> Language guidance follows [`../10-audience-and-voice/language-and-tone.md`](../10-audience-and-voice/language-and-tone.md); brand copy is 70% English / 30% romanized Hindi.

**These 25 IDs are the complete set.** `FW-REVENUE`, `FW-SALES`, `FW-AGENCY-GROWTH` and `FW-REAL-ESTATE` were cited by 275 hooks but never defined — those hooks now point at `FW-LEAD-LEAKAGE`, `FW-FOLLOWUP`, `FW-TEAM` and `FW-AUTHORITY` respectively. If you add a framework, define it here first or nothing can route to it.

**Two frameworks are switched off pre-launch.** `FW-CASE` and `FW-CUSTOMER` both need a real, consented customer. We have none. See [`../10-audience-and-voice/claims-and-proof-policy.md`](../10-audience-and-voice/claims-and-proof-policy.md).
...
## FW-CASE — Case Study — **DISABLED PRE-LAUNCH**
> **Precondition:** a real customer who has given written permission for their name, agency and numbers to be used. Until then this framework is not generated — a "real-style" story is a fabricated story.
- **Purpose:** Believable proof via a real customer story.
- **When to use:** Mid/bottom funnel, objection: "kya yeh kaam karta hai?". Post-launch only.
...
- **Purpose:** Position the brand as the expert worth following.
- **When to use:** Founder content, building trust over time.
- **Hook styles:** Contrarian, CRM, Real-Estate.
- **Script structure:** Bold claim → a sourced fact or a first-principles explanation → teach 1 insight → CTA (follow/comment). No unsourced statistic stands in for a credential.
...
- **Format:** text post, carousel, or AI-presenter voice-over. **The founder never appears on camera** — the founder's name in text is fine.
...
## FW-CUSTOMER — Customer Story (testimonial) — **DISABLED PRE-LAUNCH**
> **Precondition:** a real customer with written permission. An AI face may never deliver a testimonial or play a customer.
- **Purpose:** Real customer voice as proof.
- **When to use:** Bottom funnel trust. Post-launch only.
...
## FW-AI — AI Framework
- **Purpose:** Showcase the **WhatsApp AI Employee** as magic-but-real, from a real screen recording.
- **When to use:** Differentiation, wow-factor reach.
- **Hook styles:** AI, Curiosity, Fear.
- **Script structure:** "Raat ko lead aaya — dekho AI Employee ne kya kiya" → show the real WhatsApp thread → what the owner found next morning → CTA. Any creative that names the AI Employee carries its price and no-trial line.
> AI calling exists in the product (`agency-app/ai-calling/`, `agency-app/followup-agent/`) but is **not** on the approved-claims list, so marketing does not show it yet.
...
- **Script structure:** Hand-held, the AI presenter speaking **for the brand** → 1 benefit shown on the real product → casual CTA. Never first-person user testimony: the presenter does not say "main ek broker hoon" or claim to use the product as a customer.
...
| **Saves** | FW-MISTAKE, FW-CRM, FW-BAB |
| **Comments** | FW-CONTRARIAN, FW-MYTH, FW-NEWS |
| **Leads/Demos** | FW-PAS, FW-LEAD-LEAKAGE, FW-DEMO, FW-OBJECTION, FW-AI |
| **Trust** | FW-AUTHORITY, FW-FOUNDER, FW-DEMO (FW-CASE and FW-CUSTOMER are disabled pre-launch) |
...
Every framework ships with default **sound, grade and performance** so the pipeline picks the right mood automatically (override per piece if needed). Maps to [`visual-system.md`](visual-system.md) §10 / §12 / §14. Format: `MUSIC → GRADE → dominant PERF → SFX accent`.
...
- **FW-AUTHORITY** — `CORPORATE-INDIA` → `GRADE-BAZAAR` → `CONFIDENT` → subtle, clean
...
- **FW-CURIOSITY** — `TENSION (light)` → `BAZAAR` → `CONFUSED/SURPRISE` → whoosh on reveal
- **FW-CONTRARIAN** — `CORPORATE (bold)` → `CLEAN` → `CONFIDENT` → sharp hit
- **FW-NEWS** — `CORPORATE (news)` → `CLEAN` → `CONFIDENT` → ticker + stinger
- **FW-DEMO** — `CORPORATE-INDIA` → `GRADE-BAZAAR` → `CONFIDENT` → UI click per tap
...
- **FW-AI** — `CORPORATE (futuristic)` → `GRADE-BAZAAR` → `SURPRISE/EXCITED` → tech whoosh, glow
- **FW-CRM** — `CORPORATE-INDIA` → `BAZAAR/CLEAN` → `CONFIDENT` → UI clicks
- **FW-UGC** — `VP-MUSIC-NONE (ambient)` → `CLEAN (natural)` → authentic/`EXCITED` → raw, no SFX
- **FW-TEAM** — `CORPORATE` → `GRADE-BAZAAR` → `CONFIDENT` → dashboard-reveal swell
- **FW-PROPERTY** — `UPLIFT` → `WARM/GOLDEN` → `EXCITED/CONFIDENT` → whoosh on instant-share

> Camera, motion and `HF-*` defaults for each framework live in [`higgsfield-guide.md`](higgsfield-guide.md) §4. This table adds the *finish* (sound + grade + performance) so a piece is production-grade without manual mood-picking.
