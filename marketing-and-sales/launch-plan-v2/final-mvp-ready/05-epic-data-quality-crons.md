...
- Completeness rules (from `agency-app/api/crmDynamodbService.js` field patterns):
  ...
  - **Agreements**: entity `PROPERTY_AGREEMENT` (SK `AGREEMENT#{id}`) with `endDate` field. Also `customer.currentRental.leaseEndDate`. A `leaseEndingWithinDays` filter **already exists** in `agency-app/api/crmDynamodbService.js` — call it, do not re-implement the DynamoDB query.
- `agency-app/api/dataQualityService.js`: new root-level file (ships via `*.js` glob in deploy.sh).
- **Cron jobs:** All cron resources (EventBridge rules, Lambda functions, IAM roles) are now merged into the main `cfn-backend.yaml` template. Handler files in `agency-app/api/scripts/` (auto-included). Export: `handler` async function. One-click deployment via `./deploy.sh`.
- Messaging:
  - `agency-app/api/bailey.js` → `sendWhatsAppMessage(to, text, media?)` (EPIC 1, behind `BAILEY_ENABLED`)
  - `agency-app/api/emailService.js` → `sendEmail({ to, subject, html, text?, brevoTemplateId?, params?, from? })` (EPIC 3 fallback)
...
**Files**
- NEW `agency-app/api/dataQualityService.js`
...
**Files**
- NEW `agency-app/api/scripts/incomplete-data-cron.js` (handler)
- Cron resources merged into `agency-app/api/infra/cfn-backend.yaml` (schedule: `cron(30 3 * * ? *)` = 09:00 IST; env: CRM table, AUTH_SERVICE_URL, BAILEY, SES)
...
**Files**
- NEW `agency-app/api/scripts/expiring-agreements-cron.js` (handler)
- Cron resources merged into `agency-app/api/infra/cfn-backend.yaml` (schedule: `cron(0 3 * * ? *)` = 08:30 IST; env as E5-T2)
