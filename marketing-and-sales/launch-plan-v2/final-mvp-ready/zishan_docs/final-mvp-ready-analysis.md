...
- **RegisterAdmin page:** Already exists at `agency-app/web/src/pages/RegisterAdmin.tsx`; only route missing
...
- EventBridge Rule → Lambda (handler in `agency-app/api/scripts/*-cron.js`)
...
| 14 | `agency-app/api/agents/` is new directory; add to zip | Deploy fix | ✅ Documented |
...
- **File structure:** Root-level modules (`agency-app/api/creditService.js`, `agency-app/api/emailService.js`, etc.); no `services/` or `config/` directories
