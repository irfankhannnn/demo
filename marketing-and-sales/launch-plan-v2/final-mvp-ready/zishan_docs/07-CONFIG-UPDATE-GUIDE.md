...
## 1. Local Environment Config (`server/.env`)

✅ **Already done.** These are now in `server/.env`:
...
**Where:** `server/.env` (copied from `server/.env.example`)  
...
## 2. CloudFormation Main Stack (`server/infra/cfn-backend.yaml`)
...
**Where:** `server/infra/cfn-backend.yaml`  
**How:** YAML edits. Use `aws cloudformation validate-template` after changes.

---

## 3. Deployment Script (`server/infra/deploy.sh`)
...
## 6. Frontend Config (`real-estate-crm-app/.env`)
...
**Where:** `real-estate-crm-app/.env` (production build)  
...
Before deploying:
- [x] `server/.env` has all 8 new parameters
...
|--------|-------------|----------------|-------|
| Credit tables | `server/.env` | `CREDITS_TABLE_NAME`, `CREDIT_CONFIG_TABLE_NAME` | [x] |
| Email SES | `server/.env` | `AWS_SES_FROM_EMAIL`, `EMAIL_PROVIDER_PRIMARY` | [x] |
| Bailey | `server/.env` | `BAILEY_ENABLED=false`, keys blank | [x] |
| Cron jobs | `server/infra/cfn-backend.yaml` | All 10 cron jobs merged into main template | [x] |
| Agents | `server/.env` | `AGENTS_ENABLED=false` | [x] |
| CFN params | `server/infra/deploy.sh` | Add 8 parameters to both sections | [x] |
...
| Frontend | `real-estate-crm-app/.env` | API URLs, Razorpay key | [ ] |
