...
1. **Fix `server/infra/deploy.sh`** — add 8 missing parameters to `cfn-params.json` and `PARAM_OVERRIDES`:
   - `CreditsTableName`, `CreditConfigTableName`
   - `SesFromEmail`, `EmailProviderPrimary`
   - `BaileyEnabled`, `BaileyApiKey`, `BaileyWebhookSecret`
   - `AgentsEnabled`

2. **Fix `server/scripts/escalation-cron.js`** — add missing closing brace `}` at line 98.
...
set -e
# Validate syntax
node --check server/scripts/escalation-cron.js
