...
| **2.1** | Fix `server/scripts/escalation-cron.js` — missing closing `}` on handler function | ✅ Done | 05:12 |
...
| **4.1** | Create `server/scripts/lead-followup-cron.js` | ✅ Done | 05:18 |
| **4.2** | Create `server/scripts/lead-router-handler.js` | ✅ Done | 05:18 |
| **5** | Fix `server/scripts/expiring-agreements-cron.js` — group by member, per-member notifications, admin roll-up | ✅ Done | 05:22 |
| **6** | Fix `server/scripts/lead-qualifier-handler.js` — extract score from agent response, remove hardcoded `'WARM'`, add idempotency | ✅ Done | 05:27 |
| **7** | Complete `server/skillInvoker.js` — all 22 tools implemented, input validation + sanitization added | ✅ Done | 05:33 |
| **8** | Add `refundCredits()` to `server/creditService.js` + credit refund safety wrapper in `server/routes/leads.js` | ✅ Done | 05:36 |
| **9.1** | Add `gracePeriodActive` to `SubscriptionContextValue` interface + context value | ✅ Done | 05:42 |
| **9.2** | Add `BaileyApiEndpoint` to `cfn-backend.yaml` Parameters + Lambda env vars + `deploy.sh` + `server/.env` | ✅ Done | 05:42 |
...
| **16** | Add 3 new env vars to `server/.env` | ✅ Done | 06:34 |
...
The `server/infra/` folder now contains:
...
Ensure `server/.env` has all required values filled in (see template in `server/.env.example`).
...
1. Update `real-estate-crm-app/.env.production` with real values:
   ```
...
| `server/skillInvoker.js` syntax check | ✅ PASS |
| `server/creditService.js` syntax check | ✅ PASS |
| `server/routes/leads.js` syntax check | ✅ PASS |
...
### Backend
- `server/.env` — Added `BAILEY_API_ENDPOINT`, `POSTHOG_KEY_SERVER`, `BREVO_ESCALATED_TEMPLATE_ID`, `FOUNDER_WHATSAPP`
- `server/infra/deploy.sh` — Updated to pass 3 new parameters to CloudFormation
- `server/infra/cfn-backend.yaml` — **MAJOR REFACTOR**: Added 3 new parameters + merged all 10 cron resources (40 resources) + added 10 cron Lambda outputs
- `server/skillInvoker.js` — Full rewrite: 22 tools, input validation, sanitization
- `server/creditService.js` — Added `refundCredits()` function
- `server/routes/leads.js` — Added credit refund on failure wrapper
- `server/scripts/escalation-cron.js` — Fixed missing `}` closing handler function
- `server/scripts/expiring-agreements-cron.js` — Rewrote: member grouping, individual + admin notifications
- `server/scripts/lead-qualifier-handler.js` — Rewrote: score extraction, idempotency
- `server/scripts/lead-followup-cron.js` — Created new placeholder handler
- `server/scripts/lead-router-handler.js` — Created new placeholder handler
- `server/package.json` — Added 5 npm packages back

### CRM SPA
- `real-estate-crm-app/.env.production` — New production environment file
- `real-estate-crm-app/src/contexts/SubscriptionContext.tsx` — Added `gracePeriodActive`
- `real-estate-crm-app/src/types/crm.ts` — Added `seller?` to `ContactRoles`, added `priority`, `preferredArea`, `budget` to `CRMCustomer`
- `real-estate-crm-app/src/types/realEstate.ts` — Added `youtubeChannel?` to `Developer`
- `real-estate-crm-app/src/services/api.ts` — Added stubs for building, flat, developer, contact, agreement, document, verification methods
- `real-estate-crm-app/src/pages/admin/TeamAnalytics.tsx` — Fixed column `render` function signature
- `real-estate-crm-app/src/pages/crm/CustomerList.tsx` — Fixed priority index null guard
- `real-estate-crm-app/src/pages/BuildingDetail.tsx` — Fixed `getBuilding` setState cast
- `real-estate-crm-app/src/pages/crm/DeveloperDetails.tsx` — Fixed MediaAsset cast
- `real-estate-crm-app/src/components/CreditBalanceCard.tsx` — Removed unused `useState` import
