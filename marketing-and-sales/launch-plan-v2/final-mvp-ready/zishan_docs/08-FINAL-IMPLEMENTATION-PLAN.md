...
| **2.1** | Fix `apps/crm/server/scripts/escalation-cron.js` — missing closing `}` on handler function | ✅ Done | 05:12 |
...
| **4.1** | Create `apps/crm/server/scripts/lead-followup-cron.js` | ✅ Done | 05:18 |
| **4.2** | Create `apps/crm/server/scripts/lead-router-handler.js` | ✅ Done | 05:18 |
| **5** | Fix `apps/crm/server/scripts/expiring-agreements-cron.js` — group by member, per-member notifications, admin roll-up | ✅ Done | 05:22 |
| **6** | Fix `apps/crm/server/scripts/lead-qualifier-handler.js` — extract score from agent response, remove hardcoded `'WARM'`, add idempotency | ✅ Done | 05:27 |
| **7** | Complete `apps/crm/server/skillInvoker.js` — all 22 tools implemented, input validation + sanitization added | ✅ Done | 05:33 |
| **8** | Add `refundCredits()` to `apps/crm/server/creditService.js` + credit refund safety wrapper in `apps/crm/server/routes/leads.js` | ✅ Done | 05:36 |
| **9.1** | Add `gracePeriodActive` to `SubscriptionContextValue` interface + context value | ✅ Done | 05:42 |
| **9.2** | Add `BaileyApiEndpoint` to `cfn-backend.yaml` Parameters + Lambda env vars + `deploy.sh` + `apps/crm/server/.env` | ✅ Done | 05:42 |
...
| **16** | Add 3 new env vars to `apps/crm/server/.env` | ✅ Done | 06:34 |
...
The `apps/crm/server/infra/` folder now contains:

```
apps/crm/server/infra/
...
apps/crm/server/.env                        # Environment variables
...
Ensure `apps/crm/server/.env` has all required values filled in (see template in `apps/crm/server/.env.example`).
...
1. Update `apps/crm/real-estate-crm-app/.env.production` with real values:
   ```
...
| `apps/crm/server/skillInvoker.js` syntax check | ✅ PASS |
| `apps/crm/server/creditService.js` syntax check | ✅ PASS |
| `apps/crm/server/routes/leads.js` syntax check | ✅ PASS |
...
### Backend
- `apps/crm/server/.env` — Added `BAILEY_API_ENDPOINT`, `POSTHOG_KEY_SERVER`, `BREVO_ESCALATED_TEMPLATE_ID`, `FOUNDER_WHATSAPP`
- `apps/crm/server/infra/deploy.sh` — Updated to pass 3 new parameters to CloudFormation
- `apps/crm/server/infra/cfn-backend.yaml` — **MAJOR REFACTOR**: Added 3 new parameters + merged all 10 cron resources (40 resources) + added 10 cron Lambda outputs
- `apps/crm/server/skillInvoker.js` — Full rewrite: 22 tools, input validation, sanitization
- `apps/crm/server/creditService.js` — Added `refundCredits()` function
- `apps/crm/server/routes/leads.js` — Added credit refund on failure wrapper
- `apps/crm/server/scripts/escalation-cron.js` — Fixed missing `}` closing handler function
- `apps/crm/server/scripts/expiring-agreements-cron.js` — Rewrote: member grouping, individual + admin notifications
- `apps/crm/server/scripts/lead-qualifier-handler.js` — Rewrote: score extraction, idempotency
- `apps/crm/server/scripts/lead-followup-cron.js` — Created new placeholder handler
- `apps/crm/server/scripts/lead-router-handler.js` — Created new placeholder handler
- `apps/crm/server/package.json` — Added 5 npm packages back

### CRM SPA
- `apps/crm/real-estate-crm-app/.env.production` — New production environment file
- `apps/crm/real-estate-crm-app/src/contexts/SubscriptionContext.tsx` — Added `gracePeriodActive`
- `apps/crm/real-estate-crm-app/src/types/crm.ts` — Added `seller?` to `ContactRoles`, added `priority`, `preferredArea`, `budget` to `CRMCustomer`
- `apps/crm/real-estate-crm-app/src/types/realEstate.ts` — Added `youtubeChannel?` to `Developer`
- `apps/crm/real-estate-crm-app/src/services/api.ts` — Added stubs for building, flat, developer, contact, agreement, document, verification methods
- `apps/crm/real-estate-crm-app/src/pages/admin/TeamAnalytics.tsx` — Fixed column `render` function signature
- `apps/crm/real-estate-crm-app/src/pages/crm/CustomerList.tsx` — Fixed priority index null guard
- `apps/crm/real-estate-crm-app/src/pages/BuildingDetail.tsx` — Fixed `getBuilding` setState cast
- `apps/crm/real-estate-crm-app/src/pages/crm/DeveloperDetails.tsx` — Fixed MediaAsset cast
- `apps/crm/real-estate-crm-app/src/components/CreditBalanceCard.tsx` — Removed unused `useState` import
