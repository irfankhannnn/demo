...
| Area | Reality | Source |
|------|---------|--------|
| Server module system | **ESM** — `"type": "module"`, `import`/`export` everywhere | `agency-app/api/package.json`, `agency-app/api/server.js` |
| AWS SDK | **v3** (`@aws-sdk/client-dynamodb`, `client-s3`, `lib-dynamodb` @ ^3.669) in server; v2 only in auth microservice | `agency-app/api/package.json` |
| Bedrock | v3 `@aws-sdk/client-bedrock-agent-runtime` already used (RAG, tenant-isolated) | `agency-app/ai-calling/src/services/ragService.js` |
| Frontend | React 18.3, **React Router v7.8.2**, TailwindCSS 3.4, lucide-react, PostHog, Sentry | `agency-app/web/package.json` |
| Frontend API layer | Class-based `ApiService` (fetch); token `localStorage.auth_id_token` → `Authorization: Bearer`; `x-tenant-id` via `getTenantHeaders()` | `agency-app/web/src/services/api.ts`, `src/config/tenant.ts` |
| CRM data | Single-table `cloudberry-real-estate-crm`, PK=`TENANT#{t}#{ENTITY}#{id}`, SK=`PROFILE`/`NOTE#`/`AGREEMENT#`; GSI1 owner-property, GSI2 status, GSI3 search | `agency-app/api/crmDynamodbService.js`, `agency-app/api/infra/cfn-backend.yaml` |
| Users | **Separate auth microservice** `reality-flow-authentication`; DynamoDB `UsersTable`, PK=`TenantId`, SK=`USER#{userId}`; `listUsersByTenant()` | `platform/auth/src/models/usersModel.ts` |
| Subscriptions | DynamoDB `Subscriptions`, PK=`tenantId`; `createTrialSubscription()`; fields incl. `plan`, `seatsPaid`, `seatsUsed`, `trialEndsAt`, `isPaying`, `gracePeriodActive`, `paymentStatus`, `razorpaySubscriptionId` | `agency-app/api/subscriptionService.js`, `agency-app/api/routes/subscriptions.js` |
| Auth/tenant on server | `validateToken` (calls auth svc `/auth/me`, sets `req.user`, `req.tenantId`, 5s cache) → `extractTenantId` → `requireRole(...)` | `agency-app/api/middleware/validateToken.js`, `tenantMiddleware.js`, `middleware/requireRole.js` |
...
| Billing webhook | `POST /api/billing/webhook`, mounted **before** `express.json()`, uses `express.raw()`, HMAC-SHA256 timing-safe verify | `agency-app/api/routes/billing.js`, `agency-app/api/server.js` |
| Razorpay (frontend) | `openCheckout()` loads checkout.js, uses **subscription_id** flow; key `VITE_RAZORPAY_KEY_ID` | `agency-app/web/src/lib/razorpay.ts` |
...
| WhatsApp | **AiSensy** broadcast already wired in billing webhook (`sendAiSensyBroadcast`). No Bailey/inbound gateway. | `agency-app/api/routes/billing.js` |
...
| Crons | Each cron = its own CFN template in `/cron/*.yaml` (EventBridge Rule + Lambda + Permission). Handlers in `agency-app/api/scripts/*-cron.js`. | `cron/trial-reminder.yaml` |
| Deploy | `agency-app/api/infra/deploy.sh`: `npm ci` → zip `function.zip` (incl. `scripts/`) → S3 → `cloudformation deploy cfn-backend.yaml` → force API GW deploy. Lambda handler `lambda-handler.handler`. Syntax gate `agency-app/api/scripts/build.sh`. | `agency-app/api/infra/deploy.sh` |
...
10. **Lambda packaging already includes `scripts/`** — new cron handlers placed in `agency-app/api/scripts/` ship automatically. New top-level modules must be added to the `zip` include list in `deploy.sh` if outside `routes/middleware/utils/validation/lib/scripts`.
...
13. **E2-T3 code snippet has a duplicate import bug** — the original plan text contained leftover `import { deductCredits } from '../services/creditService.js'` and `import { getCosts } from '../config/creditConfig.js'` lines above the correct root-level imports. The ONLY correct imports in `agency-app/api/middleware/meterCredits.js` are `import { deductCredits } from '../creditService.js'` and `import { getCosts } from '../creditConfig.js'`. The `services/` and `config/` directories do not exist.

14. **`agency-app/api/agents/` is a new directory and must be explicitly added to the deploy.sh zip** — root `*.js` are auto-included but subdirectories need explicit listing. The current zip command must be extended to include `agents/` and to exclude `mcp-server/`. See `07-infra-cfn-deploy.md` §6.
...
16. **`RegisterAdmin.tsx` already exists** at `agency-app/web/src/pages/RegisterAdmin.tsx` — only the route in `App.tsx` is missing. Do not recreate the page component; only add the `<Route path="/onboarding/register-admin" ...>` entry.

17. **Two separate leads route files** — `agency-app/api/routes/crm.js` AND `agency-app/api/routes/leads.js` are both mounted under `/api/crm/leads` in server.js. E2-T4 must identify which file contains each create endpoint before adding metering. Confirm in server.js mount order before modifying.
