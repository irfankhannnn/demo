...
- Server ESM (`"type":"module"` in `agency-app/api/package.json`): use `import`/`export`. No `require()`.
- Middleware chain: `validateToken` (→ `agency-app/api/middleware/validateToken.js`) → `extractTenantId` (→ `agency-app/api/middleware/tenantMiddleware.js`) → `requireRole(...)` (→ `agency-app/api/middleware/requireRole.js`).
- Existing subscription service: `agency-app/api/subscriptionService.js` → `getSubscription(tenantId)` returns `{ plan, seatsPaid, seatsUsed, trialEndsAt, isPaying, paymentStatus, gracePeriodActive }`.
- Existing subscription routes: `agency-app/api/routes/subscriptions.js` → `GET /api/subscriptions/current`, `GET /api/subscriptions/trial-status`, `POST /api/subscriptions/check-seat`.
- Existing Razorpay subscription flow: `agency-app/web/src/components/PaywallModal.tsx` + `src/lib/razorpay.ts` → `openCheckout({ planId, ... })`. Plan IDs: `plan_team_*` (₹1999), `plan_teamplus_*` (₹4999).
- Billing webhook: `agency-app/api/routes/billing.js` → `POST /webhook` (full path: `POST /api/billing/webhook`) — mounted with per-route `express.raw()` before JSON parsing; HMAC timing-safe verify pattern to copy for credit purchase webhook.
- `req.tenantId` is the only trusted tenant source — **never** read tenant from request body.
- `InsufficientCreditsError` class: define inline in `agency-app/api/creditService.js` (not a separate file/dir). See `agency-app/api/expressError.js` for the existing error class style to match.
...
**Files**
- MODIFY `agency-app/api/infra/cfn-backend.yaml` (add tables + extend Lambda role Resource list + add env vars to `ApiLambdaFunction`)
- MODIFY `agency-app/api/infra/deploy.sh` (add new table-name params)
- NEW `agency-app/api/creditConfig.js` (root-level, matching `*Service.js` convention; loads config from table with hard-coded safe defaults as fallback)

> **File placement:** backend modules go at **server root** (`agency-app/api/creditService.js`, `agency-app/api/creditConfig.js`, etc.) — there is no `agency-app/api/services/` or `agency-app/api/config/` dir in this repo. Root `*.js` ships via the existing zip. See `07-infra-cfn-deploy.md` §6.
...
**Files**
- NEW `agency-app/api/creditService.js` (root-level)
- `InsufficientCreditsError` — define as an **inline class** in `creditService.js` (no `errors/` dir; match the style of `agency-app/api/expressError.js`)
...
**Files**
- NEW `agency-app/api/middleware/meterCredits.js`

**Detail**
```js
// agency-app/api/middleware/meterCredits.js
...
**Files**
- MODIFY `agency-app/api/routes/crm.js` (lead/contact/property/owner/tenant create; bulk import)
- MODIFY `agency-app/api/routes/khata.js` (entry create) — confirm path exists; if khata lives elsewhere, locate via route registration in `server.js`.

**Detail**
- **Before modifying any route file:** read `agency-app/api/server.js` to confirm which file handles each endpoint.
  - Leads create: check both `agency-app/api/routes/crm.js` AND `agency-app/api/routes/leads.js` — both mount under `/api/crm/leads`.
  - Contacts create: `agency-app/api/routes/contacts.js` → `POST /api/crm/contacts`.
  - Buyers create: `agency-app/api/routes/buyers.js` → `POST /api/crm/buyers`.
  - Khata entries: `agency-app/api/routes/khata.js` → `POST /api/khata`.
...
**Files**
- MODIFY `agency-app/api/routes/subscriptions.js` — add `POST /credits/purchase`
- MODIFY `agency-app/api/routes/billing.js` — in `payment.captured` (and/or `order.paid`), if `notes.credits` present, `grantCredits`
- NEW `agency-app/api/razorpayOrders.js` — create Orders via Razorpay REST (server key/secret env), since current frontend flow is subscription-only.
...
- Webhook: in `agency-app/api/routes/billing.js`, in the `payment.captured` handler, add: if `notes?.credits` present on the captured payment's order, call `grantCredits(tenantId, Number(notes.credits), 'purchase', { razorpayOrderId, razorpayPaymentId, amountPaise: payment.amount })`.
...
- `agency-app/api/razorpayOrders.js`: use Razorpay REST API (`https://api.razorpay.com/v1/orders`) with basic auth `{RAZORPAY_KEY_ID}:{RAZORPAY_KEY_SECRET}` env vars.
...
**Files**
- MODIFY `agency-app/api/routes/subscriptions.js`:
  - `GET /credits` → `{ balance, costs, packs, freeTier, resetDate }`
  - `GET /credits/ledger?limit&startKey` → paginated rows
- NEW `agency-app/api/routes/creditAdmin.js` (mounted `/api/credit-config`, `requireRole('ADMIN','FOUNDER','OWNER')`):
  - `GET /` → current config
  - `PUT /costs`, `PUT /packs`, `PUT /free-tier` → update config table
- MODIFY `agency-app/api/server.js` to mount the new router (after json + auth).
...
**Files**
- NEW `agency-app/api/scripts/credit-reset-cron.js` (handler `handler`, ships via zip)
...
**Files**
- MODIFY `agency-app/web/src/contexts/SubscriptionContext.tsx` — also fetch `GET /api/subscriptions/credits`; expose `creditBalance`, `creditCosts`, `packs`.
- NEW `agency-app/web/src/components/CreditBalanceCard.tsx` — "Credits: {balance}", reset date, "Buy more".
- NEW `agency-app/web/src/components/BuyCreditsModal.tsx` — calls `POST /credits/purchase`, then `openCheckout` with `order_id` (extend `src/lib/razorpay.ts` to accept an order flow alongside subscription flow).
- MODIFY `agency-app/web/src/pages/crm/BillingSettings.tsx` (E1-T3) and CRM dashboard header to mount `CreditBalanceCard`.
