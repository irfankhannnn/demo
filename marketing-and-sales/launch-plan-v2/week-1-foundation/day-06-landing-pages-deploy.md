...
9. **Wire signup → Brevo**: in `agency-app/api/routes/auth.js` registration handler, on success POST to Brevo `/contacts` API with list ID `Trial Signups` (env `BREVO_TRIAL_LIST_ID`).
