...
3. `agency-app/api/routes/leads.js` (route pattern to copy)
4. `agency-app/api/crmDynamodbService.js` (lines 1-100 for DDB patterns)
5. `agency-app/api/tenantMiddleware.js`
6. `agency-app/api/server.js` (understand the import + mount pattern)
7. `agency-app/web/src/App.tsx` (route + ProtectedRoute pattern)
8. `agency-app/web/src/pages/admin/InviteManagement.tsx` (admin page pattern to match)
...
### 1. `agency-app/api/grievanceDynamodbService.js`

Mirror the pattern of `agency-app/api/crmDynamodbService.js`. Import DDB via `awsClientWrapper.js`.
...
### 2. `agency-app/api/routes/grievance.js`
...
**PostHog server event:** Call `serverTrack` from `agency-app/api/lib/posthog.js` (this file is created by PR-E; for now, create a stub):
```js
...
### 3. `agency-app/web/src/pages/public/Grievance.tsx`
...
### 4. `agency-app/web/src/pages/admin/GrievanceList.tsx`
...
Playwright test. Read existing test config in `agency-app/web/test/` for base URL setup.
...
- `agency-app/api/lib/posthog.js` — leave as stub; PR-E creates the real version
- Any LP files
- Any existing CRM pages
- `agency-app/api/crmDynamodbService.js` — create a separate `grievanceDynamodbService.js`
...
Files created:
- agency-app/api/routes/grievance.js — POST /api/grievance (public, rate-limited, hCaptcha) + admin GET/PATCH
- agency-app/api/grievanceDynamodbService.js — Grievances DDB service
- agency-app/web/src/pages/public/Grievance.tsx — public form page
- agency-app/web/src/pages/admin/GrievanceList.tsx — admin triage UI
- tests/grievance.spec.ts — Playwright tests

Files modified:
- agency-app/api/server.js — added grievanceRoutes to LAUNCH ROUTES blocks
- agency-app/web/src/App.tsx — added /grievance + /admin/grievances to LAUNCH ROUTES blocks
