...
**Hard dependencies:**
- PR-F merged (`server/routes/billing.js` exists — you add to it)
- Batch 1 merged (`server/routes/auth.js` extension point must exist)
...
3. `server/routes/auth.js` (find the invite-creation handler)
4. `server/routes/billing.js` (created by PR-F — understand the webhook structure to add seat increment)
5. `real-estate-crm-app/src/pages/admin/InviteManagement.tsx` (you will modify this)
6. `real-estate-crm-app/src/pages/admin/MemberManagement.tsx` (you will modify this)
...
### 1. `server/subscriptionService.js`
...
### 3. Update `server/routes/auth.js` — invite creation handler ONLY
...
### 4. Update `server/routes/billing.js` — subscription.updated branch ONLY
...
### 5. `real-estate-crm-app/src/components/SeatCounter.tsx`
...
### 6. `real-estate-crm-app/src/components/SeatUpgradeModal.tsx`
...
### 8. `server/scripts/backfill-seats-paid.js`
...
- `server/aiEmployeeProvisioningService.js` (PR-F created this)
...
Files created:
- server/subscriptionService.js — Subscriptions DDB service
- server/routes/subscriptions.js — GET /current + GET /trial-status
...
Files modified:
- server/routes/auth.js — seat check in invite-creation handler (invite POST only)
- server/routes/billing.js — seat increment in subscription.updated case (one case only)
- real-estate-crm-app/src/pages/admin/InviteManagement.tsx — SeatCounter + SeatUpgradeModal mount
- real-estate-crm-app/src/pages/admin/MemberManagement.tsx — SeatCounter mount
- server/server.js — subscriptions route mount
