...
**Hard dependency:** PR-H merged (`agency-app/api/subscriptionService.js` + `subscriptions.js` must exist)
...
3. `agency-app/api/routes/subscriptions.js` (created by PR-H — you add trial-status endpoint)
4. `agency-app/api/subscriptionService.js` (created by PR-H — you call getSubscription)
5. `agency-app/web/src/App.tsx` (authenticated layout structure)
6. `agency-app/web/src/components/SeatCounter.tsx` (created by PR-H — component pattern to follow)
...
### 1. Update `agency-app/api/routes/subscriptions.js` — add trial-status endpoint
...
### 2. `agency-app/web/src/contexts/SubscriptionContext.tsx`
...
### 3. `agency-app/web/src/hooks/useSubscription.ts`
...
### 4. `agency-app/web/src/components/TrialCountdownBanner.tsx`
...
### 5. `agency-app/web/src/components/PaywallModal.tsx`
...
### 6. `agency-app/web/src/lib/razorpay.ts`
...
### 8. `agency-app/api/scripts/trial-reminder-cron.js` + `cron/trial-reminder.yaml`
...
- `agency-app/api/subscriptionService.js` functions (PR-H created them — only use `getSubscription`)
- `agency-app/api/routes/subscriptions.js` GET /current endpoint (PR-H created; only ADD trial-status)
...
Files modified:
- agency-app/api/routes/subscriptions.js — added GET /trial-status (new endpoint, no touch to /current)
- agency-app/web/src/App.tsx — SubscriptionContext.Provider + TrialCountdownBanner + PaywallModal
