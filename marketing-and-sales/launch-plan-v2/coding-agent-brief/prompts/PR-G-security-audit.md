...
2. All existing `agency-app/api/routes/*.js` files — enumerate every route
3. `agency-app/api/tenantMiddleware.js` — understand extractTenantId
4. `agency-app/api/middleware/validateToken.js`
5. `agency-app/api/server.js` — all mounted routes
6. `agency-app/api/crmDynamodbService.js` — understand how DDB calls use tenantId
...
Walk every `router.{get,post,put,delete,patch}` in every `agency-app/api/routes/*.js` file.
...
- Modify any source code (`agency-app/api/routes/*.js`, `src/**/*.tsx`)
...
- [ ] `route-tenant-coverage.csv` covers every route in `agency-app/api/routes/*.js`
