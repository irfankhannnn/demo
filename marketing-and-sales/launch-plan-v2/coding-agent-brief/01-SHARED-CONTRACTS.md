...
### PR-B: Grievance Routes (`server/routes/grievance.js`)
```
...
### PR-F: Billing Routes (`server/routes/billing.js`)
```
...
### PR-F: AI Employee Status (`server/routes/aiEmployeeStatus.js`)
```
GET    /api/ai-employee/status     — validateToken + extractTenantId
```

### PR-H: Subscription Routes (`server/routes/subscriptions.js`)
```
...
### PR-K: Feedback Routes (`server/routes/feedback.js`)
```
