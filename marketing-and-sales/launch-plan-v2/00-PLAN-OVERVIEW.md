...
Event names are **not** restated here. Every row's exact source is defined once, in `50-measurement/posthog-event-map.md`, which is built from the live contract in `apps/crm/real-estate-crm-app/src/types/analytics.ts`, `apps/crm/server/lib/posthog.js`, `apps/landing-pages/_partials/head-analytics.hbs` and `coding-agent-brief/01-SHARED-CONTRACTS.md` §3.4.

| Metric | Target | Measurement source |
|---|---|---|
| Trial signups | 30-50 | PostHog — see `50-measurement/posthog-event-map.md` |
| Paying customers | 3-5 | Razorpay billing webhook (`apps/crm/server/routes/billing.js`) |
| Activation rate | ≥40% | PostHog, within 7d of signup — definition open, see D27 |
| Cold reply rate | ≥10% | Manual log + Instantly metrics |
| NPS responses | ≥10 | DynamoDB `NPSResponses` (`apps/crm/server/routes/feedback.js`) |
