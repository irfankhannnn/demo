...
- All paths are repo-relative (e.g. `agency-app/api/routes/grievance.js`) — never a Windows user path
...
For decisions not yet logged, default to the founder. For technical conventions, follow `agency-app/api/routes/leads.js`, `agency-app/api/tenantMiddleware.js` and `agency-app/web/src/App.tsx`.
