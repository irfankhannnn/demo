...
- `agency-app/web/src/types/analytics.ts` — the declared `AnalyticsEvent` union and `UserTraits`.
- `agency-app/web/src/lib/analytics.ts` — `trackEvent` / `identifyUser`, client side.
- `agency-app/api/lib/posthog.js` — `serverTrack`, server side.
- `agency-app/landing-pages/_partials/head-analytics.hbs` — landing-page PostHog plus the consent-gated GA4 / Meta Pixel / LinkedIn / Hotjar tags.
...
|---|---|---|
| `signup_completed` | `agency-app/api/routes/auth.js:69` | account created |
...
| `ai_employee_escalated` | `agency-app/api/scripts/escalation-cron.js:80` | follow-up escalated to a human |
...
3. `lead_added` on lead creation, carrying `source` and `sourceAdapter` from `agency-app/api/leadIngestion.js` so channel mix is queryable.
...
| `first_ai_call` | — | not fired; AI call outcomes land in `agency-app/api/routes/aiCallingInternal.js` and are metered in credits |
...
- `agency-app/api/routes/billing.js:51` defines a **local** `serverTrack` rather than importing `lib/posthog.js`. Two wrappers, one event stream. Worth collapsing, but it does not change any number today.
