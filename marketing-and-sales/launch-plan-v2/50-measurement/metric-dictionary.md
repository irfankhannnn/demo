...
Source shorthand: **PH** = PostHog ([`posthog-event-map.md`](./posthog-event-map.md)) · **RZP** = Razorpay webhooks via `agency-app/api/routes/billing.js` · **NPS** = `NPSResponses` table via `agency-app/api/routes/feedback.js` · **LOG** = the founder's manual weekly log ([`weekly-scorecard.md`](./weekly-scorecard.md)) · **IG** = Instagram app insights, read by hand.
...
| ID | Metric | Formula | Source | Target |
|---|---|---|---|---|
| **M-F1** | Landing-page views | `count(page_view)` on `agency-app/landing-pages` | PH | — |
...
All of these are read by hand from the Instagram app, weekly. The Instagram service can serve followers, reach and views for a **connected** account (`agency-app/instagram-api/routes/insights.js`) — ours is not connected yet.
...
|---|---|---|---|
| **M-S1** | Tenant lead temperature | a *customer's* buyer leads, HOT / WARM / COLD | built, in code: `agency-app/api/utils/leadRubric.js`; Instagram enquiries map very_hot/hot/cold → hot/warm/cold in `agency-app/instagram-api/services/leadAnalyst.js` |
...
`Website · Referral · Walk-in · Google Ads · Social Media · Instagram · Property Portal · Broker Network · Other` (`agency-app/web/src/utils/leadConstants.ts`), plus `sourceAdapter` (`instagram`, `manychat`, …) set by `agency-app/api/leadIngestion.js`.
