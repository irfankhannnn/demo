...
This scores RealEstateFlow's own prospects. It is **not** the CRM's lead scoring. A customer's buyer leads are scored in code by `apps/crm/server/utils/leadRubric.js` (HOT / WARM / COLD from an AI call or from text) and, for Instagram enquiries, by `apps/instagram/backend_insta_sol_ms/services/leadAnalyst.js`. Those two are built and are not affected by anything on this page. The June document this replaces conflated the two populations throughout, which is how it came to claim "leads have no score today" while the CRM was scoring leads.
