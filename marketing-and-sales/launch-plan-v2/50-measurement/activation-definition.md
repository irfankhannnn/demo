...
- AI calls consume credits (`agency-app/api/creditConfig.js`), so even a paying tenant's activation has a cost attached to it.
...
- It segments on a Free / Starter / Growth / Pro plan ladder that **does not exist**. The real plans are Solo, Team, Team+ and the AI Employee add-on (`pricing.json`), so the solo/agency split should read "Solo plan" vs "3+ seats" (`agency-app/api/subscriptionService.js`).
- It needs a `scoringService` and a `SCORE` entity, neither of which exists. **Design only — not built.**
- It puts the aha on the AI call, which the current onboarding does not ask for. Onboarding asks the user to connect WhatsApp (`agency-app/web/src/pages/onboarding/ConnectWhatsApp.tsx`).
...
4. Choose plan (`src/pages/onboarding/ChoosePlan.tsx`) — `onboarding_plan_buy_clicked`, `onboarding_trial_started`; the trial record is created in `agency-app/api/subscriptionService.js`.

A trial email drip already runs at day 10, day 12, day 14 and after expiry (`agency-app/api/scripts/trial-reminder-cron.js`). Any activation nudge should extend that cron rather than start a parallel one.
...
| First AI call or AI-handled lead | the differentiator was experienced | AI call outcomes land in `agency-app/api/routes/aiCallingInternal.js`; no PostHog event |
