...
**Recommendation on the page, not a decision:** the day-30 spec. It is the most recent, it is costed against plans that actually exist, and it is the only one a solo founder can run by hand. The June scheme's cap refers to a ₹5,999 "Pro" plan that has never existed, and its "30-day extended trial" contradicts the 14-day trial in `pricing.json` and in `agency-app/api/subscriptionService.js`.
