...
|---|---|---|
| UTM on signup | `utm_source`, `utm_medium`, `utm_campaign` read from the signup URL into PostHog user traits | `apps/crm/real-estate-crm-app/src/pages/PhoneLogin.tsx:36-47`, `RegisterAdmin.tsx:38`, `src/types/analytics.ts` (`UserTraits`) |
| UTM on a landing-page view | `utm_source` on every `page_view` | `apps/landing-pages/_partials/head-analytics.hbs` |
| Lead source, on every lead | `source` (display value) and `sourceAdapter` (`instagram`, `manychat`, …), set at the single ingest point | `apps/crm/server/leadIngestion.js` |
| Originating reel, on Instagram leads | `reelRef { postId, permalink }` and `externalRef { igUsername, … }` | `apps/instagram/backend_insta_sol_ms/services/crmBridge.js`; `apps/crm/real-estate-crm-app/src/types/crm.ts` |
| Lead temperature and why | `score` (HOT/WARM/COLD), `scoreValue`, `scoreReasons`, `scoreSource` | `apps/crm/server/utils/leadRubric.js`; shown in `src/pages/crm/LeadDetails.tsx` |
| Per-reel enquiry, hot-lead and DM counts | for a connected Instagram account | `apps/instagram/backend_insta_sol_ms/routes/media.js`, `routes/insights.js` |
| Revenue events | Razorpay webhooks, mirrored to PostHog | `apps/crm/server/routes/billing.js` |
