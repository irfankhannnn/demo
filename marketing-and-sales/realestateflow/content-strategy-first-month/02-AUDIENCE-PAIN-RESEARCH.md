...
|------|--------------|-------------|
| A1 employee takes contacts | Multi-tenant CRM - leads/buyers/owners/tenants/contacts owned by the agency | `agency-app/api/agents`, tenant-scoped tables |
| A2 no pipeline visibility | `get_pipeline_summary`, `get_business_health`, `get_crm_metrics`, `get_dashboard_snapshot` | `agency-app/api/shared/toolDefinitions.js` |
| A3 duplicate calling | Lead assignment + ownership, `update_lead` | CRM lead module |
| A4 who do I owe | **Khata Book** - `TO_GIVE`/`TO_TAKE`, party type, per-property, settlement status, reminders | `agency-app/web/src/types/khata.ts` |
...
| B4 rent reminders | Khata `reminderAt` + notification service | `agency-app/api/notificationDynamodbService.js` |
