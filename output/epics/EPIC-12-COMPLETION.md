...
- `server/routes/auth.js` — `POST /api/auth/post-registration` endpoint: adds new trial user to Brevo "Trial Signups" list (via BREVO_API_KEY + BREVO_TRIAL_LIST_ID env vars), passes UTM attribution attributes
...
## Files Modified
1. `server/routes/auth.js` — added POST /post-registration with Brevo contact creation
2. `real-estate-crm-app/src/pages/PhoneLogin.tsx` — added UTM capture useEffect
3. `real-estate-crm-app/src/pages/RegisterAdmin.tsx` — added UTM pass-through + Brevo hook call + sessionStorage cleanup
