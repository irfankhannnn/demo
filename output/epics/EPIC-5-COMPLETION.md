...
- **Server PostHog** (`server/lib/posthog.js`): PostHog Node SDK wrapper — `serverTrack(distinctId, event, properties)` + `shutdownPostHog()`.
...
- Server Lambda Sentry init (`server/lambda-handler.js`) out of PR-E scope.
- Remaining event instrumentation: `trial_paywall_shown` (PR-J), `subscription_started` (PR-F), `seat_limit_hit` (PR-H).

## Rollback Notes
- Remove `src/lib/analytics.ts`, `src/types/analytics.ts`, `server/lib/posthog.js`.
