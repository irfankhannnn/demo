...
description: >
  ...
  Unbounded in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  # — Bazaar Signal v3. `fire-orange` is Marigold; `cream` is Paper. —
  # `canvas` is declared FIRST and explicitly: the assembler paints the video's base
  # ground from this role, and it is what shows through during every cross-frame
  # transition. Bazaar Signal is a DARK-ground system — the broadside preset's own
  # cream canvas would make all five cuts dip to light grey. Ground is ink.
  canvas: "#1C1512"
  ink-black: "#1C1512"      # Ink — page ground, warm near-black
  ink-black-alt: "#251C16"  # Ink 2 — card ground
  ink-card-alt: "#2E241D"   # Ink 3 — alternate card
  fire-orange: "#FF7A1A"    # Marigold — PRIMARY accent
  fire-orange-deep: "#E8620A"
  gulal: "#FF3D7F"          # SECONDARY pop — once per frame, never on the same card as marigold
  gulal-deep: "#E01F63"
  tulsi: "#1FAA59"          # checkmarks / verified ONLY — never a general accent
  cream: "#FBF2E4"          # Paper — khata-ledger cream, never stark white
  cream-alt: "#F3E6D2"      # Paper 2
  cream-muted: "#C9BBA8"    # Dust — secondary text on ink
  cream-hint: "#948575"     # Dust dim
  pain-red: "#EF4444"       # pain points / loss only
  border-dark: "#3A2E25"
...
typography:
  # — reading ramp —
  body:    { fontFamily: "Manrope", cqw: 1.2, weight: 500, lineHeight: 1.6 }
  lead:    { fontFamily: "Manrope", cqw: 1.6, weight: 600, lineHeight: 1.5 }
  caption: { fontFamily: "Manrope", cqw: 0.9, weight: 500, lineHeight: 1.5 }
  label:   { fontFamily: "Manrope", cqw: 0.72, weight: 800, tracking: "0.14em", upper: true }
  # — display / hero ramp (Unbounded 800/900 ONLY, sentence case, negative tracking) —
  h3:      { fontFamily: "Unbounded", cqw: 2.8, weight: 800, lineHeight: 1.2 }
  quote-text:{ fontFamily: "Unbounded", cqw: 3.8, weight: 800, lineHeight: 1.15, tracking: "-0.02em" }
  h2:      { fontFamily: "Unbounded", cqw: 4.5, weight: 800, lineHeight: 1.1, tracking: "-0.02em" }
  stat-value:{ fontFamily: "Manrope", cqw: 5.5, weight: 800, lineHeight: 1.0, tracking: "-0.03em" }
  h1:      { fontFamily: "Unbounded", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em" }
  fadelist-item:{ fontFamily: "Unbounded", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em" }
  quote-mark:{ fontFamily: "Unbounded", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Unbounded", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em" }
  display: { fontFamily: "Unbounded", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em" }
...
This is the **broadside** preset remixed onto **RealEstateFlow's v3 "Bazaar Signal"** brand system. The YAML frontmatter above (colors · typography · components) is **normative and already correct — use it verbatim.** The prose below is the ORIGINAL preset's intent; read it THROUGH the frontmatter and through the six overrides below.

- **Fonts** — **Unbounded 800/900 for display ONLY. Manrope 400–800 for everything else.** The preset gives Unbounded every text role; Bazaar Signal forbids that. Unbounded never sets body copy, a caption, a chat bubble, a label, or a paragraph — one line, maybe two, and never more. Ignore every "Unbounded body/lead/caption" reference in the prose below.
- **Colors** — use the frontmatter hex. Preset names map: **`fire-orange` = Marigold `#FF7A1A`**, **`cream` = Paper `#FBF2E4`** (khata-ledger cream, never stark white), **`ink-black` = Ink `#1C1512`** (warm near-black, never cold navy).

### The six Bazaar Signal overrides to the preset

1. **Display is SENTENCE CASE, not lowercase.** The preset's signature lowercase display is overridden — RealEstateFlow's shipped work sets display in sentence case ("Your AI Employee replied. You didn't lift a finger."). Capitals stay. Never uppercase a display line either.
2. **There IS a second accent.** The preset says fire-orange is the only color; Bazaar Signal adds **Gulal `#FF3D7F`** as a secondary pop — used **once per frame, and never on the same card as marigold.** Pick one accent per component so it reads confident, not chaotic.
3. **Tulsi `#1FAA59` is reserved.** Checkmarks and "verified" only — a deliberate wink at WhatsApp green. Never a general accent, never a ground.
4. **Three registers, not two.** Ink ground (paper text) · Marigold ground (ink text) · **Paper ground (ink text)** · Gulal ground (paper text). The preset's "no cream/paper register" is overridden — paper is a legitimate ground.
5. **Chrome is Manrope 800 uppercase tracked 0.14em**, not IBM Plex Mono. There is no mono face in this brand.
6. **Radius returns for two things only:** chat bubbles and cards (`rounded-lg` 14px / `rounded-xl` 18px). Everything else stays sharp per the preset. The plane is still flat — **no shadows, no gradients** except the one approved hero gradient `linear-gradient(100deg, #FF7A1A 15%, #FF3D7F 85%)` on display text over ink.

**Colour ratio per frame:** ~42% ink · ~18% paper · ~22% marigold · ~14% gulal · ~4% tulsi.


## Overview

Broadside at frame scale is a **protest-poster system where type is so large it stops reading as
text and becomes graphic primitive.** Unbounded `display` at 13cqw puts a single lowercase word
...
**Unbounded** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Unbounded 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Unbounded body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Unbounded `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Unbounded display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Unbounded `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Unbounded lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Unbounded `display`/`h1`
...
row of three top-border-only stat-cards — big Unbounded-900 numeral in `{colors.fire-orange}`, Unbounded
...
**Ground** ink-black. **Composes** fadelist (3 stacked Unbounded-900 words at 1.0/0.5/0.22), fadelist-title.
...
Unbounded `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Unbounded display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Unbounded display; never add a second accent color.
...
- No serif companion; chrome is never Unbounded.
...
- **Type** — Unbounded lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Unbounded + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
