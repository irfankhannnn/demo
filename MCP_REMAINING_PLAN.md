...
### What's DONE ✅
- `server/shared/toolDefinitions.js` — 54 tools, single source of truth
- `server/skillInvoker.js` — Dynamic handler lookup (switch/case removed)
- `server/mcp-server/tools.js` — Imports from shared definitions (13 lines)
- `server/oauth/tokenGenerator.js` — JWT access/refresh token generation
- `server/oauth/tokenValidator.js` — Token validation, tenantId extraction
- `server/routes/oauth.js` — OAuth endpoints (authorize, token, revoke)
...
- OAuth routes are **NOT registered** in `server/server.js`
...
- **Existing CFN:** `server/infra/cfn-backend.yaml` (1500+ lines, full CRM stack)
...
**What:** Create `server/mcp-server/httpServer.js` that uses `StreamableHTTPServerTransport` instead of `StdioServerTransport`.
...
**What:** Create `server/mcp-server/lambdaHandler.js` that adapts the Express app to Lambda via `@vendia/serverless-express`.
...
**What:** Create `server/mcp-server/localServer.js` for local development and testing.
...
**Implementation:**
1. Create `server/mcp-server/resources.js`
...
**Implementation:**
1. Create `server/mcp-server/prompts.js`
...
**Implementation:**
1. In `server/routes/agentTools.js`, change:
   ```javascript
...
3. Use existing `logger` from `server/logger.js`
...
3. Package MCP Lambda code:
   ```bash
   zip -r mcp-function.zip server/mcp-server/ server/shared/ server/oauth/ server/crmDynamodbService.js server/skillInvoker.js server/logger.js server/agents/ server/userCategoryService.js server/aiDtoMiddleware.js node_modules package.json
   ```
4. Upload to S3
...
**What:** Create `real-estate-crm-app/src/pages/crm/AiIntegrations.tsx`
...
**What:** Create `server/routes/aiIntegrations.js` with endpoints for managing connected apps.
...
**Implementation:**
1. Add `GET /oauth/callback` route in `server/routes/oauth.js`
...
**Implementation:**
1. Create Zod schemas for each tool in `server/shared/toolSchemas.js`
...
2. Create `server/mcp-server/metrics.js`
