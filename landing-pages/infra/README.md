...
`real-estate-crm-app/infra/README.md` for the sibling implementation this
one is modeled on.
...
`cfn-templates-cicd/landing-pages/deploy.sh <dev|prod>` — it delegates to
this script and then records the build.

## Why this isn't just a copy of `real-estate-crm-app/infra/cfn-frontend.yaml`
...
- Routing is a `AWS::CloudFront::Function` (`UrlRewriteFunction` in
  `cfn-landing-pages.yaml`) that reimplements `landing-pages/netlify.toml`'s
