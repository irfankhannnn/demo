...
| 2 | Send retry missed 5xx status codes | `server/bailey.js` | `isRetryableSendError` checks `err.response.status` as number (500–599) |
| 3 | Chunk retry duplicate-delivery risk | `server/bailey.js` | Documented in JSDoc; partial success tracked |
...
| 7 | Chunk send partial success not tracked | `server/bailey.js` | Returns `sentChunks`, `totalChunks`, `messageIds` |
...
2. **Server propagates queued status**
   - `server/bailey.js` `sendWhatsAppMessage` returns `queued: true`.
   - `sendWhatsAppMessageChunks` throws if nothing was actually delivered.

3. **Processor releases claim on failure**
   - Added `markMessageProcessingFailed` in `server/whatsappConversationService.js`.
