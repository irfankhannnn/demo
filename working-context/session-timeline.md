...
2. **5xx status code retry** in `server/bailey.js` `isRetryableSendError`.
...
### Files changed
- `server/skillInvoker.js` — added all new tools and tool schemas
- `server/skillInvoker.test.js` (new) — unit tests for tool routing and validation
- `server/agents/responseFormatter.js` — formatting for new result types
- `server/agents/prompts.js` — instructions for new tools

### Test results
- `server/skillInvoker.test.js`: all passed
...
### Implementation
1. Created `server/agents/responseFormatter.js` with format templates for:
...
- `server/agents/agentRuntime.js` — applies `sanitizeAndFormatReply` before returning text
- `server/agents/prompts.js` — updated whatsapp prompt to prefer structured bullets/numbered lists

### Test results
- `server/agents/responseFormatter.test.js`: all passed
- Full `server` test suite: passed
...
### Test results after fix
- `server/agents/agentRuntime.test.js`: all passed
