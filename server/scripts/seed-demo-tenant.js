 ...
 * USAGE
 * -----
 *   node server/scripts/seed-demo-tenant.js [--reset] [--tenant=DEMO_REALESTATEFLOW]
 *
...
// Load server/.env (same pattern as crmDynamodbService.js)
...
if (!CRM_TABLE) {
  console.error(
    'ERROR: CRM_DYNAMODB_TABLE_NAME is not set. Configure it in server/.env (or the Lambda env).',
  );
  process.exit(1);
}
