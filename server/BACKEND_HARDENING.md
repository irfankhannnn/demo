...
Middleware: `server/middleware/requirePlatformOperator.js`
...
Single source of truth: `server/utils/corsOrigins.js`
...
CFN: `server/infra/launch-tables-cfn.yaml` defines `keyHash-index` on `TenantApiKeys`.

## DynamoDB hardening

- Main list getters in `crmDynamodbService.js` paginate via `collectAllPages` (`server/utils/dynamoPagination.js`)
...
- `server/middleware/requirePlatformOperator.js`
- `server/middleware/apiKeyAuth.js`
- `server/utils/corsOrigins.js`
- `server/utils/dynamoPagination.js`
- `server/expressError.js`
- `server/lambda-handler.js`
- `server/infra/apigw-explicit-routes.yaml`
