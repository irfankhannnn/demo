...
Scope: `public-app/property-pages/routes/pages.js` (the POST handler),
...
The CRM's existing limiter (`agency-app/api/middleware/rateLimiter.js`) keeps
...
`agency-app/api/publicListingService.js`'s allowlist bounds *what* is ever exposed
