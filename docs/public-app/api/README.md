...
|---|---|---|---|
| Consumer web | `public-app/web/` | Vite + React 18 + Tailwind, S3 + CloudFront | `infra/cicd/public-app/web/deploy.sh <env>` |
| Marketplace API | `public-app/api/` | Express on Lambda, own REST API GW, own DynamoDB table | `infra/cicd/public-app/api/deploy.sh <env>` |
| Consumer auth | `public-app/auth/` | TS Express on Lambda, own Cognito pool + tables | `infra/cicd/public-app/auth/deploy.sh <env>` |
| CRM changes | `agency-app/api`, `agency-app/web` | existing | existing `infra/cicd/agency-app/api`, `infra/cicd/agency-app/web` |
...
- **Visibility is a set of attributes on the CRM's own PROPERTY item**, set or
  removed by `agency-app/api/marketplaceIndexing.js` at the end of every
...
- Create the vector index once per environment, after the backend stack:
  `agency-app/api/infra/create-vector-index.sh <env> marketplace` and wait for
  `check-vector-index.mjs` to report ACTIVE / not backfilling.
- Backfill keys for listings published before this shipped:
  `node agency-app/api/scripts/backfill-marketplace-keys.js --dry-run` then without.
