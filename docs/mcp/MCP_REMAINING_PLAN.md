...
### What's DONE ✅
- `apps/crm/server/shared/toolDefinitions.js` — 54 tools, single source of truth
- `apps/crm/server/skillInvoker.js` — Dynamic handler lookup (switch/case removed)
- `apps/crm/server/mcp-server/tools.js` — Imports from shared definitions (13 lines)
- `apps/crm/server/oauth/tokenGenerator.js` — JWT access/refresh token generation
- `apps/crm/server/oauth/tokenValidator.js` — Token validation, tenantId extraction
- `apps/crm/server/routes/oauth.js` — OAuth endpoints (authorize, token, revoke)
...
- OAuth routes are **NOT registered** in `apps/crm/server/server.js`
...
- **Existing CFN:** `apps/crm/server/infra/cfn-backend.yaml` (1500+ lines, full CRM stack)
...
**What:** Create `apps/crm/server/mcp-server/httpServer.js` that uses `StreamableHTTPServerTransport` instead of `StdioServerTransport`.
...
**What:** Create `apps/crm/server/mcp-server/lambdaHandler.js` that adapts the Express app to Lambda via `@vendia/serverless-express`.
...
**What:** Create `apps/crm/server/mcp-server/localServer.js` for local development and testing.

**Implementation:**
1. Load `.env` from `apps/crm/server/.env`
...
**Implementation:**
1. Create `apps/crm/server/mcp-server/resources.js`
...
**Implementation:**
1. Create `apps/crm/server/mcp-server/prompts.js`
...
**Implementation:**
1. In `apps/crm/server/routes/agentTools.js`, change:
   ```javascript
...
3. Use existing `logger` from `apps/crm/server/logger.js`
...
3. Package MCP Lambda code:
   ```bash
   zip -r mcp-function.zip apps/crm/server/mcp-server/ apps/crm/server/shared/ apps/crm/server/oauth/ apps/crm/server/crmDynamodbService.js apps/crm/server/skillInvoker.js apps/crm/server/logger.js apps/crm/server/agents/ apps/crm/server/userCategoryService.js apps/crm/server/aiDtoMiddleware.js node_modules package.json
   ```
4. Upload to S3
...
**Implementation:**
1. In `apps/crm/server/server.js`, add:
   ```javascript
...
**What:** Create `apps/crm/real-estate-crm-app/src/pages/crm/AiIntegrations.tsx`
...
**What:** Create `apps/crm/server/routes/aiIntegrations.js` with endpoints for managing connected apps.
...
**Implementation:**
1. Add `GET /oauth/callback` route in `apps/crm/server/routes/oauth.js`
...
**Implementation:**
1. Create Zod schemas for each tool in `apps/crm/server/shared/toolSchemas.js`
...
2. Create `apps/crm/server/mcp-server/metrics.js`
