...
straight into `services/ai-calling-service/.env.prod` (gitignored) — **never paste secret
...
| 12 | CRM caller key | `CRM_CALLER_API_KEY` | Generate — step E. **Same value in BOTH `services/ai-calling-service/.env.prod` and `apps/crm/server/.env.prod`** |
...
3. **System prompt:** paste the entire block from
   `services/ai-calling-service/elevenlabs-agent-prompt.md` (the fenced block under
...
- **`SERVER_TOOL_API_KEY`** → `services/ai-calling-service/.env.prod`. The same value goes
  into ElevenLabs later as the server tools' secret header.
- **`CRM_CALLER_API_KEY`** → the same value in **both**
  `services/ai-calling-service/.env.prod` **and** `apps/crm/server/.env.prod`. This is what lets
...
2. Add the **six server tools** to the prod agent per
   `services/ai-calling-service/elevenlabs-agent-tools.md`, pointing at
...
Full context on the service: `docs/services/ai-calling-service/GO-LIVE-RUNBOOK.md`.
