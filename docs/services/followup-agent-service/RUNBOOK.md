...
```
`AI_CALLING_CALLER_API_KEY` is the existing `CRM_CALLER_API_KEY` from `services/ai-calling-service/.env.dev`.

## 1. ai-calling-service (extended)

1. `services/ai-calling-service/.env.dev`: add `EXOTEL_CALLER_ID=<ExoPhone in E.164>` (blank keeps click-to-call disabled).
...
1. `apps/crm/server/.env.dev`: `FOLLOWUP_SERVICE_DOMAIN_NAME=services-api.cloudberrysolutions.in`, `FOLLOWUP_SERVICE_BASE_PATH=devrealestatefollowup`, `FOLLOWUP_CALLER_API_KEY`, `FOLLOWUP_INTERNAL_API_KEY` (values from step 0).
2. Deploy the server through `infra/cicd/server/deploy.sh dev` (SSM sync picks up the four new keys; API Gateway gets the new routes).
...
`infra/cicd/followup-agent-service/deploy.sh rollback-full dev <build>`. Disabling calls without a deploy: set `followupCallsEnabled: false` on the tenant, or disable the `<env>-realestateflow-followup-dispatch` EventBridge rule.
