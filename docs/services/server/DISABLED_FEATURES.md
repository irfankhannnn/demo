...
- **Re-enable steps:**
  1. Uncomment imports in `apps/crm/server/server.js`
  2. Uncomment route mounts in `apps/crm/server/server.js`
  3. Uncomment imports in `apps/crm/real-estate-crm-app/src/App.tsx`
  4. Uncomment Routes in `apps/crm/real-estate-crm-app/src/App.tsx`

### 2. Buildings/Flats Hierarchy
- **Server routes:** `routes/areasBuildings.js`, `routes/flats.js`, `routes/areas.js`, `routes/publicAreas.js`
- **Re-enable steps:**
  1. Uncomment imports and route mounts in `apps/crm/server/server.js`
...
- **Service:** `services/ai-calling-service/` (standalone microservice) — still needs its
...
- **Remaining steps to go live per tenant:**
  1. Deploy `services/ai-calling-service/` CloudFormation stack (`services/ai-calling-service/cfn-template.yaml`)
  ...
  2. Set the new `AiCallingInternalApiKey` / `AiCallingServiceDomainName` + `AiCallingServiceBasePath` CFN
     parameters (`apps/crm/server/infra/cfn-backend.yaml`) — or `AI_CALLING_INTERNAL_API_KEY`
     / `AI_CALLING_SERVICE_DOMAIN_NAME` + `AI_CALLING_SERVICE_BASE_PATH` in `apps/crm/server/.env` for local dev
