...
Middleware: `apps/crm/server/middleware/requirePlatformOperator.js`
...
Single source of truth: `apps/crm/server/utils/corsOrigins.js`
...
CFN: `apps/crm/server/infra/launch-tables-cfn.yaml` defines `keyHash-index` on `TenantApiKeys`.

## DynamoDB hardening

- Main list getters in `crmDynamodbService.js` paginate via `collectAllPages` (`apps/crm/server/utils/dynamoPagination.js`)
...
- `apps/crm/server/middleware/requirePlatformOperator.js`
- `apps/crm/server/middleware/apiKeyAuth.js`
- `apps/crm/server/utils/corsOrigins.js`
- `apps/crm/server/utils/dynamoPagination.js`
- `apps/crm/server/expressError.js`
- `apps/crm/server/lambda-handler.js`
- `apps/crm/server/infra/apigw-explicit-routes.yaml`
