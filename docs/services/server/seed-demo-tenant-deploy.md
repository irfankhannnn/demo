...
> **NOTE:** This document references `cron/reset-demo.yaml` which is now obsolete. The demo reset cron has been merged into `apps/crm/server/infra/cfn-backend.yaml` (2026-06-21). For current deployment, run `./deploy.sh` from `apps/crm/server/infra/` to deploy all cron jobs including demo reset.

## Files

| File | Purpose |
|---|---|
| `apps/crm/server/scripts/seed-demo-tenant.js` | Idempotent seeder (20 buyers, 15 owners, 10 properties, 8 pipeline leads, 2 team members, 5 khata entries, 5 AI transcripts) |
| `apps/crm/server/scripts/reset-demo-tenant.js` | Thin `--reset` wrapper + Lambda `handler()` for the cron |
| `cron/reset-demo.yaml` | EventBridge rule + Lambda spec (daily 02:00 IST) |
| `apps/crm/real-estate-crm-app/src/components/DemoBanner.tsx` | Yellow "DEMO TENANT" banner (gated on `VITE_IS_DEMO=true`) |
...
Frontend demo build (`apps/crm/real-estate-crm-app/.env`):
...
```bash
node apps/crm/server/scripts/seed-demo-tenant.js --tenant=DEMO_TEST          # seed
node apps/crm/server/scripts/seed-demo-tenant.js --reset --tenant=DEMO_TEST  # purge + re-seed
```
...
node apps/crm/server/scripts/seed-demo-tenant.js --reset --tenant=DEMO_TEST
```

Verify: the script logs row counts (should be 20/15/10/8/2/5/5 = 65 total) and
writes `apps/crm/server/scripts/node-trace.json` with elapsed time + counts. It exits
non-zero on any error.

## 3. Deploy the reset Lambda

1. Package the `apps/crm/server/` tree into the Lambda (or reuse the existing API Lambda
   bundle, which already contains `apps/crm/server/scripts/*`).
2. Create a Lambda `realestateflow-demo-reset` with handler
   `apps/crm/server/scripts/reset-demo-tenant.handler`, runtime `nodejs22.x`,
