...
Contract: `docs/services/followup-agent-service/CONTRACTS.md` sections 2.2, 5 and 7.
Design: `docs/services/followup-agent-service/APPROVAL-PLAN.md` sections 3.5 and 3.6.

## 1. Phone masking

Code: `apps/crm/server/middleware/phoneMasking.js` (tests: `phoneMasking.test.js`).
...
Code: `apps/crm/server/routes/clickToCall.js` (tests: `clickToCall.test.js`).

### Flow

```
Browser                    CRM (apps/crm/server/)                       ai-calling-service            Exotel
...
CRM (`apps/crm/server/`), already used by `routes/aiCalling.js`:
...
### Mounting in `apps/crm/server/server.js`
