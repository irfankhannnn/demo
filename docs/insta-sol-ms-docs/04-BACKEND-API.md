...
| POST | `/accounts/:igUserId/sync` | runs profile (first time), conversations and analysis now; `{ account, summary }`; 409 if not connected |
...
| DELETE | `/rules/:ruleId` | |
| GET | `/overview?days=` | `{ counters, accounts, series[{date, enquiries, followers, reach, views}] }` |
