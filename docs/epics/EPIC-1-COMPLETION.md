...
- Self-contained, idempotent demo-tenant seeder (`agency-app/api/scripts/seed-demo-tenant.js`) — deterministic IDs so re-running overwrites instead of duplicating.
...
- Lambda cron wrapper (`agency-app/api/scripts/reset-demo-tenant.js`) exporting `resetDemo()` + `handler()` for EventBridge.
...
## APIs Added
- None. PR-A is CLI/script + UI only; no `agency-app/api/server.js` routes added (per anti-conflict rules).
...
- `cron/reset-demo.yaml` — EventBridge rule `realestateflow-demo-daily-reset`, schedule `cron(30 20 * * ? *)` (UTC) = **2:00 AM IST daily**, target Lambda `realestateflow-demo-reset` (handler `agency-app/api/scripts/reset-demo-tenant.handler`, Node 22.x, 60s timeout, 256 MB) + `lambda:InvokeFunction` permission for `events.amazonaws.com`.
- `docs/agency-app/api/seed-demo-tenant-deploy.md` — deployment runbook (env vars, local testing, Lambda packaging, cron wiring, CloudWatch alarm, demo frontend build).
- `agency-app/api/.gitignore` — ignores the seeder's `scripts/node-trace.json` runtime artifact.
