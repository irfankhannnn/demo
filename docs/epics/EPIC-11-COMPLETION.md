...
- `apps/crm/server/routes/feedback.js` — `POST /api/feedback/nps` (authenticated, validates score 0-10, freeText required for ≤6, stores to NPSResponses DDB) + `GET /api/nps` (public, HMAC-validated email-link NPS)
...
3. `apps/crm/real-estate-crm-app/src/pages/public/NpsEmailLanding.tsx`
4. `tests/nps.spec.ts`

## Files Modified
- `apps/crm/server/server.js` — feedback route import + mount (`/api/feedback`, `/api/nps`)
