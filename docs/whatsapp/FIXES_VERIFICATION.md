...
**File 1:** `apps/crm/real-estate-crm-app/src/utils/whatsappConnection.ts` (lines 180-187)
```typescript
if (!res.ok) {
...
**File 2:** `apps/crm/server/routes/auth.js` (lines 138-148)
```javascript
...
**File:** `apps/crm/real-estate-crm-app/src/utils/whatsappConnection.ts` (lines 46-75)
```typescript
...
1. `apps/crm/real-estate-crm-app/src/utils/whatsappConnection.ts` — 2 fixes applied
2. `apps/crm/server/routes/auth.js` — 1 fix applied
3. `apps/crm/server/bailey.js` — No changes needed (already correct)
