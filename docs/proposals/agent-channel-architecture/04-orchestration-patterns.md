...
The codebase already contains the correct instinct, written by whoever built Call Intelligence. `server/services/callIntelligence/actionPlanner.js` opens with:
...
|---|---|
| Tool definitions and schemas | `server/shared/toolDefinitions.js` |
| Tool execution + authorisation | `server/skillInvoker.js` → `canUserAccessTool()` |
| Data access | `server/crmDynamodbService.js` |
| Semantic retrieval | `05-retrieval-and-vector-search.md` (proposed) |
| Audit trail | `server/agents/agentAuditService.js` |
