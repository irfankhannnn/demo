...
Research before writing any code confirmed the blast radius was smaller than it might sound: in production, only **two** places ever call into `conversationStateService.js` — `apps/crm/server/scripts/whatsapp-message-processor.js` (the real Lambda handler) and `apps/crm/server/agents/agentRuntime.js` — both using the same already-normalized phone value consistently. That made a careful, fully-tested re-key tractable in one slice.
...
### 1. `apps/crm/server/conversationStateService.js`
...
### 2. `apps/crm/server/utils/whatsapp.js` — new `buildWhatsAppPrincipal(phone)`
...
### 4. `apps/crm/server/agents/agentRuntime.js`
...
### 5. `apps/crm/server/scripts/clear-whatsapp-conversation.js` (ops utility)
