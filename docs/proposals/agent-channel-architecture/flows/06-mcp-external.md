...
Claude Desktop / ChatGPT / other MCP client
    ...
    ▼
reality-flow-mcp/            ← standalone TypeScript service, own deployment
...
server/routes/agentTools.js
    ...
    ▼
server/skillInvoker.js  →  canUserAccessTool()  →  crmDynamodbService  →  DynamoDB
```
...
`reality-flow-mcp/src/services/toolDefinitions.ts` is an explicit copy. Its own header says so:

> *"Defines all 54 CRM tools in a neutral format... This is a self-contained copy for the isolated MCP microservice. The canonical source lives in `server/shared/toolDefinitions.js`."*

Measured at `f6b557e`:

| Registry | Unique tools |
|---|---|
| `server/shared/toolDefinitions.js` (canonical) | **87** |
| `reality-flow-mcp/src/services/toolDefinitions.ts` (copy) | **74** |
...
```
server/shared/toolDefinitions.js          ← single source of truth
            │
    ...
    generate-mcp-tools.mjs
            ...
            ▼
reality-flow-mcp/src/services/toolDefinitions.ts   ← GENERATED, committed, never hand-edited
```
...
There is already a related helper (`server/scripts/dump-tools.js`) and a docs generator (`docs/current_design/_generate-tools.mjs`), so this pattern is established in the repo.
...
So the single-shot limitation is a property of `server/agents/llm/planTurn.js`, **not** of the tools. Two consequences:
