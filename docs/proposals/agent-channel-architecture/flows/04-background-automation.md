...
| Script | Trigger | Job |
|---|---|---|
| `agency-app/api/scripts/lead-qualifier-handler.js` | EventBridge `lead.created` | Hot/Warm/Cold temperature from intake data |
| `agency-app/api/scripts/lead-router-handler.js` | EventBridge `lead.qualified` | Assign the lead to a team member |
| `agency-app/api/scripts/lead-followup-cron.js` | Scheduled | Generate follow-up nudges |

---

## The problem

All three call `invokeAgent()` from `agency-app/api/agents/agentRuntime.js` — **the same entry point the WhatsApp chat agent uses**. That means they inherit the full Mode A machinery: domain routing, open function-calling over the CRM tool registry, and the ability to write to the CRM with nobody reading the result.
