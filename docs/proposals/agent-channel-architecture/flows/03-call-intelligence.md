...
`agency-app/api/services/callIntelligence/actionPlanner.js` states the principle outright:
...
Added to `agency-app/api/shared/toolDefinitions.js`, which means it becomes available to **Flow 01, Flow 02 and Flow 06 automatically** — one registry, every consumer. An agency owner can then ask on WhatsApp: *"pichhle mahine ke khata dispute wale calls dikhao"*.
