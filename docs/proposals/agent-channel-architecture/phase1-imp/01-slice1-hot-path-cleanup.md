...
Every inbound WhatsApp message runs through `agency-app/api/scripts/whatsapp-message-processor.js` before a reply goes out. Two things on that path were pure overhead:
...
2. **Tenant resolution ran its own inline DynamoDB `Scan`**, duplicating a helper that already existed. `agency-app/api/agencyConfigService.js` already exports `getTenantIdByConnectedWhatsAppPhone(phone)` — same table, same filter, same Scan — and its own docstring even (incorrectly) claimed the processor already used it. Two copies of the same query is how logic drifts: a fix applied to one stops covering the other. `agency-app/api/routes/webhooks.js` was already calling the shared helper; the processor was the odd one out.
...
Nothing replaces this — it was dead weight, not a guard protecting anything. `BAILEY_API_ENDPOINT` is still read elsewhere (in `agency-app/api/bailey.js`, where the actual send happens), so nothing else references the removed usage.
...
| Manual check: `BAILEY_API_ENDPOINT` still referenced correctly elsewhere | ✅ Only used in `agency-app/api/bailey.js`, unaffected by this change |
