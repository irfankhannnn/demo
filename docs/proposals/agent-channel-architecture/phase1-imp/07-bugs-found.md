...
### 6. `~360 lines of dead code` in `agency-app/api/skillInvoker.js`
...
### 7. `USER_CATEGORIES` (`agency-app/api/userCategoryService.js`) allowlists referenced tools that no longer exist, and never granted the new archive tools
...
### 9. `toTitleCase()` (`agency-app/api/utils/titleCase.js`) mangles common building/area names typed in caps
...
- **`agency-app/api/agents/prompts.js` (the live WhatsApp system prompt)** — three separate instructions telling the model that delete tools exist and to gate them behind confirmation:
...
- **`agency-app/api/agents/llm/plannerPrompt.js`** — *"Delete: if user has not confirmed after you asked, do NOT call delete_* — reply asking them to confirm (haan/yes)."*
...
- **`agency-app/api/agents/responseFormatter.js:104`** — `if (toolName.startsWith('delete_')) return 'note deleted';` inside note handling. Unreachable (no `delete_*` tool can be dispatched), and it concerns the phantom note tools from #11.
- **`agency-app/api/agents/responseFormatter.js:349`** — still branches on `mode === 'confirm' || mode === 'cancelled'`, two `decideInteraction` modes removed in Slice 5. Unreachable.
- **`agency-app/api/agents/responseFormatter.test.js:281`** — a `formats delete confirmation` test calling `formatToolResult('delete_lead', …)`. It passes (the formatter doesn't check tool existence) but tests a tool that no longer exists.
...
### 12. `agency-app/api/agents/toolContextBuilder.js` is entirely dead code
...
**Verified:** all eight of `delete_lead_note`, `update_lead_note`, `delete_owner_note`, `update_owner_note`, `delete_tenant_note`, `update_tenant_note`, `delete_buyer_note`, `update_buyer_note` are absent from `agency-app/api/shared/toolDefinitions.js` — *none* of them exist. They are nonetheless referenced in two files:

- **`agency-app/api/userCategoryService.js`** — listed in `USER_CATEGORIES.admin.allowedTools`. Harmless (granting permission to a tool that can't be called is a no-op).
- **`agency-app/api/aiDtoMiddleware.js`** — listed in the `LEAD_TOOLS`/`OWNER_TOOLS`/`TENANT_TOOLS` sets (~lines 50/55/60) **and** given dedicated handling branches (`if (toolName === 'delete_lead_note')` ~141, `delete_owner_note` ~199, `delete_tenant_note` ~258). Those branches are unreachable dead code.
