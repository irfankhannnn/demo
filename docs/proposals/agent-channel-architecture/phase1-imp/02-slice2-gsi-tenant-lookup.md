...
Verified against `agency-app/api/infra/cfn-backend.yaml`: **`AgencyConfigTable` has no GSI at all today** — only its `TenantId` hash key. So "switch to a Query" isn't a same-file code change; it requires adding an index first.

## What will change

### 1. Add a GSI in `agency-app/api/infra/cfn-backend.yaml`
...
3. **Deploy verification (non-prod first) — pending, user-owned:**
   ...
   - Specifically exercise a request through `agency-app/api/routes/webhooks.js` (the main API Lambda), not just the WhatsApp processor Lambda — that's the path the IAM gap above would have broken.
