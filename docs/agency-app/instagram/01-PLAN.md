...
- `agency-app/api/routes/webhooks.js` — the ManyChat Instagram lead webhook stays exactly as
  it is and keeps running in parallel.
- `agency-app/api/agencyConfigService.js` and the `AgencyConfig` table — no new fields.
- The existing `Leads` table, `createLead`, `notifyNewLead`, or the `lead.created`
  EventBridge flow.
- `agency-app/api/infra/cfn-backend.yaml` — new tables go in their own stack.
...
**Total edit to existing code: 2 lines in `agency-app/api/server.js`** (one import, one
...
agency-app/api/routes/instagramAgent.js      new router, /api/ig-agent/*
server/instagramAgentService.js      new DynamoDB access layer
agency-app/api/infra/cfn-instagram-agent.yaml   own CFN stack, own tables
agency-app/web/src/pages/instagram/  new CRM page
```

**Existing files edited (2 lines + 1):**
```
agency-app/api/server.js          import + app.use('/api/ig-agent', validateToken, igAgentRoutes)
