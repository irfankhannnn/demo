...
| Folder | What |
|---|---|
| `agency-app/instagram-api/` | The microservice. Express on Lambda + EventBridge worker, API Gateway, two DynamoDB tables |
| `agency-app/instagram-web/` | The console. React 18 + TS + Vite, served at `/insta/` |
| `infra/cicd/agency-app/instagram-api/` | CI/CD wrapper with build tracking and rollback |
| `infra/cicd/agency-app/instagram-web/` | Same, plus per-build `dist/` archives |
