...
straight into `agency-app/ai-calling/.env.prod` (gitignored) — **never paste secret
...
| 12 | CRM caller key | `CRM_CALLER_API_KEY` | Generate — step E. **Same value in BOTH `agency-app/ai-calling/.env.prod` and `agency-app/api/.env.prod`** |
...
3. **System prompt:** paste the entire block from
   `agency-app/ai-calling/elevenlabs-agent-prompt.md` (the fenced block under
...
- **`SERVER_TOOL_API_KEY`** → `agency-app/ai-calling/.env.prod`. The same value goes
  into ElevenLabs later as the server tools' secret header.
- **`CRM_CALLER_API_KEY`** → the same value in **both**
  `agency-app/ai-calling/.env.prod` **and** `agency-app/api/.env.prod`. This is what lets
...
2. Add the **six server tools** to the prod agent per
   `agency-app/ai-calling/elevenlabs-agent-tools.md`, pointing at
...
Full context on the service: `docs/agency-app/ai-calling/GO-LIVE-RUNBOOK.md`.
