...
`agency-app/ai-calling/` places outbound AI voice calls to real estate leads: the
...
| CI/CD wrapper | ✅ Done, `infra/cicd/agency-app/ai-calling/deploy.sh dev\|prod` |
...
`agency-app/api/leadIngestion.js`, `agency-app/api/meetingReminderRecipients.js`,
`agency-app/api/scripts/meeting-reminder-cron.js`.
...
|---|---|
| `CRM_INTERNAL_API_KEY` | Must equal `AI_CALLING_INTERNAL_API_KEY` in `agency-app/api/.env.dev` |
...
| `CRM_CALLER_API_KEY` | Generate: `openssl rand -hex 32`. **Same value required in both `agency-app/ai-calling/.env.<env>` and `agency-app/api/.env.<env>`** — it authenticates the CRM backend to the calling service's management API. Tenant-crossing credential, server-side only |
...
```bash
cd infra/cicd/agency-app/ai-calling
./deploy.sh dev
```
...
1. **Two things must be switched on that currently are not.**
   ...
   - `CRM_CALLER_API_KEY` must hold the **same value** in
     `agency-app/ai-calling/.env.<env>` and `agency-app/api/.env.<env>`. If they disagree
...
2. **The vector index must be created and properties backfilled** before
   ...
   `agency-app/api/infra/cfn-backend.yaml`; existing properties need
   `node agency-app/api/scripts/backfill-property-embeddings.js --tenant <id> --dry-run`
...
5. ~~**`answer_policy_question` returns nothing useful.**~~ **BUILT.** Policy
   ...
   (`agency-app/api/services/knowledge/`).
   ...
   - have an agency save at least one policy document (or run
     `node agency-app/api/scripts/reindex-policies.js`)
...
6. ~~**There is no vector/semantic property search.**~~ **BUILT.** `match_properties`
   ...
   `node agency-app/api/scripts/backfill-property-embeddings.js` for pre-existing rows.
...
| Thing | Where |
|---|---|
| Agent system prompt (paste-ready) | `agency-app/ai-calling/elevenlabs-agent-prompt.md` |
| Server tool definitions (paste-ready) | `agency-app/ai-calling/elevenlabs-agent-tools.md` |
| CFN template | `agency-app/ai-calling/infra/cfn-ai-calling.yaml` |
| Deploy | `infra/cicd/agency-app/ai-calling/deploy.sh dev\|prod` |
| Env contract | `agency-app/ai-calling/.env.example` |
| Architecture + known gaps | `agency-app/ai-calling/README.md` |
