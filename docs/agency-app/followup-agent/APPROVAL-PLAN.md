...
### 2.1 Voice calling: `agency-app/ai-calling/` (reusable for dialing)
...
- Stray file `agency-app/ai-calling/--version` is an accidental empty zip; it will be removed.
...
| B. `instagram-local-agent/` → `agency-app/instagram-api/` | Graph API on the agency laptop | DynamoDB `<env>-realestateflow-insta-data` | Yes, via `POST /api/internal/adapters/leads` |
| C. ManyChat webhook `agency-app/api/routes/webhooks.js:245` | ManyChat External Request | CRM Leads table | Yes (direct) |
...
- All three paths converge on the CRM `ingestLead()` (`agency-app/api/leadIngestion.js`), which emits EventBridge `crm.leads` / `lead.created` (`{tenantId, leadId, leadType, name, phone}`) on the default bus. The event has no `sourceAdapter` and no "call requested" flag.
- Property references in DM data are free text (locality, building name). Only the CRM Meeting entity can carry a `propertyId`.

### 2.3 CRM (`agency-app/api/`) hooks that will be reused
...
- Scheduling precedents: EventBridge cron/rate rules plus Lambdas in `agency-app/api/infra/cfn-backend.yaml` (e.g. `meeting-reminder-cron` every 5 min); SQS plus DLQ for the call-recording worker.
...
- The CRM's API Gateway enumerates routes explicitly (`agency-app/api/infra/apigw-explicit-routes-part*.yaml`). Every new `/api/crm/...` path needs a template entry (helper: `agency-app/api/scripts/generate-missing-cfn-routes.js`).

### 2.4 Roles, phone visibility, Exotel

- Roles actually issued by the auth service: **`ADMIN` and `MEMBER` only** (`platform/auth/src/models/usersModel.ts:31`). `FOUNDER/OWNER/MANAGER` are accepted by the CRM middleware but never minted. So "agency owner" = `ADMIN`, "team member" = `MEMBER`. Role arrives via `/auth/me`, not as a JWT claim (`agency-app/api/middleware/validateToken.js:89-118`).
...
- Precedent to copy: `agency-app/api/routes/aiCalling.js:121-162` (browser sends an entity id, server resolves the number, number never returns to the client) and `publicListingService.js:206` (allowlist serializer).
...
### 3.1 New service `agency-app/followup-agent/`
...
### 3.2 Changes to `agency-app/ai-calling/` (backward compatible)
...
### 3.3 Changes to CRM `agency-app/api/`
...
### 3.7 Frontend (`agency-app/web/`), kept minimal
...
- [ ] `agency-app/followup-agent/`: src, tests (`node --test`), `infra/cfn-followup.yaml`, `infra/deploy.sh`, `infra/config-deploy.sh`, `infra/config-only-allowed-params.json`, `infra/lib/*`, `.env.example`, `.gitignore`, README, this document.
- [ ] `infra/cicd/agency-app/followup-agent/`: `deploy.sh`, `README.md`, `.gitignore`.
- [ ] `agency-app/ai-calling/`: purposes, context, three new tools, `call.ended` events, connect endpoint, prompt and tools docs, template IAM and params, tests.
- [ ] `agency-app/api/`: bugs A to D, meeting events, follow-up internal and proxy routes, agency config fields, adapters `followUp` hint, phone masking middleware, click-to-call route, API Gateway route entries, template/SSM/env plumbing, tests.
- [ ] `hp-insta-lead-automation/`: `push_leads_to_crm.py`, analyst contract fields, workbook columns.
- [ ] `agency-app/web/`: follow-up UI, masked phones plus Call button, settings.
...
| 9 | Service folder name | `agency-app/followup-agent/` (wrapper folder named identically, as the deploy skill requires). |
...
|---|---|---|
| `agency-app/followup-agent/` + `infra/cicd/agency-app/followup-agent/` | Complete | 42 unit tests pass; `aws cloudformation validate-template` OK; `cfn-readiness-auditor` verdict READY_TO_DEPLOY (secrets blank by design) |
| `agency-app/ai-calling/` (purposes, context, 3 tools, `call.ended` events, `/calls/connect`, `EXOTEL_CALLER_ID`, docs) | Complete | 54 tests pass |
...
| `agency-app/web/` (PhoneNumber component, Call button, AI follow-up card, settings) | Complete | typecheck: no new errors (84 pre-existing on this branch) |
...
Verification fixes (2026-09-15): (1) `agency-app/api/middleware/phoneMasking.js` now strips masked phone values and `phoneMasked` from inbound `/api/crm` bodies (property/developer/customer pages had no frontend strip); (2) `agency-app/followup-agent/src/handlers/worker.js` logs a `dispatch result` line per due job and an `event handled` line per event so the test brief's log checks are real. Known UI limits found: the meeting scheduler has no meeting-type or property picker (site visits are recognised by title), the calendar shows no AI-confirmation badge, and the enquiries page is not routed.
