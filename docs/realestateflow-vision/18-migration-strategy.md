...
The serverless CRM (`apps/crm/server/`, `apps/crm/real-estate-crm-app/`) stays the core. New capabilities are wrapped around it and reach the business through the same domain services. DynamoDB stays the only source of truth. No design deletes CRM data; old data is archived.
...
flowchart TB
    subgraph Channels
        ...
        MC["ManyChat adapter"]
        PP["Property pages + visit booking<br/>apps/property-pages-ms/"]
        WA["WhatsApp command channel<br/>services/whatsapp-platform/ (Baileys)"]
        WEB["In-CRM web chat"]
        EXT["Claude / ChatGPT via MCP<br/>services/reality-flow-mcp/"]
    end
    ...
    WA --> AG["Agent core<br/>apps/crm/server/agents/"]
    WEB --> AG
    ...
    ING --> CRM
    RT --> CRM
    EB -->|lead.created, meeting.completed| FU["services/followup-agent-service"]
    FU --> VC["services/ai-calling-service<br/>ElevenLabs + Exotel"]
```
...
| — Fine-grained RBAC | Partly done | Auth service issues `ADMIN`/`MEMBER` only (`services/reality-flow-authentication/src/controllers/phoneAuthCustomController.ts`); CRM backend also accepts `MANAGER`/`FOUNDER`/`OWNER` (`apps/crm/server/middleware/requireRole.js`); role-based phone masking (`middleware/phoneMasking.js`). No team/region scoping. Next (D15): ADMIN/MEMBER for M1; MANAGER + "members see only their own leads" before selling Team plans (Phase B). |
...
| — Tenant-scoped reads (MED-1) | Partly done | `getLeads` queries `search-index` (GSI3); other list functions still scan (`TODO(MED-1)` in `apps/crm/server/crmDynamodbService.js`). Phase B. |
| — Conversation tables | Done differently | WhatsApp conversation state and history in `apps/crm/server/conversationStateService.js`, `whatsappConversationService.js` |
| — Wrap domain endpoints as MCP tools | Done differently | One MCP server, `services/reality-flow-mcp/`, 72 tools generated from `apps/crm/server/shared/toolDefinitions.js`, own OAuth, Streamable HTTP. Not AgentCore Gateway. |
| — Re-enable disabled routes | Partly done | `aiCallingInternal` is mounted (`apps/crm/server/server.js`). `routes/projects.js`, `developers.js`, `buildings.js`, `areas.js` exist but are not mounted. |
| **2. Conversation backbone** | Done differently | EventBridge (`lead.created`, `lead.qualified`, `call.ended`) and SQS (call-recording queue, follow-up DLQ). No Redis: dedup via a DynamoDB idempotency log (`logEventIfNotProcessed` in `apps/crm/server/webhookLogService.js`, used by `leadIngestion.js`). No Chatwoot (dropped, D9). Channels are adapters into `leadIngestion.js`. |
| **3. Reasoning over tools** | Done differently | Qualifier and router are EventBridge-triggered Lambdas (`apps/crm/server/scripts/lead-qualifier-handler.js`, `lead-router-handler.js`). The WhatsApp/web agent is an in-house classify → plan → execute → compose pipeline with an optional bounded tool loop, on Gemini behind a model gateway (`apps/crm/server/agents/`). Off unless `AGENTS_ENABLED=true`. |
| **4. Engines one by one** | Partly done | Qualification (Hot/Warm/Cold rubric, `apps/crm/server/utils/leadRubric.js`) and routing are built. The founder is building a next-generation lead engine separately; scoring/assignment docs will be updated when it lands (D11). |
| **5. Voice + follow-up journeys** | Done differently | Voice rebuilt on ElevenLabs' native Exotel integration (`services/ai-calling-service/`), outbound only (D16). Follow-up calls scheduled, retried and escalated by `services/followup-agent-service/` on EventBridge + DynamoDB, not Step Functions. |
...
| Keep | Knowledge retrieval | Now DynamoDB vector search + Titan v2 embeddings (`apps/crm/server/services/embeddings/`, `services/knowledge/`), not the Bedrock KB pattern |
...
| Evolve | "AI Employee" from human SLA to agent | Partly: WhatsApp agent + follow-up agent with `draft`/`autosend` modes (`apps/crm/server/routes/aiEmployeeConfig.js`), still set up by a concierge (`aiEmployeeProvisioningService.js`) |
| Retire | Hardcoded-secret deploy script | Done (deleted) |
| Retire | In-memory rate limiter | Not done: still used (`apps/crm/server/middleware/rateLimiter.js`); add API Gateway throttling (Phase A, D17) |
...
- **Additive only.** New tables were added beside the CRM table (credits, credit config, agent audit, knowledge chunks, push tokens and others in `apps/crm/server/infra/cfn-backend.yaml`). No bulk migration was needed and none is planned.
...
Today WhatsApp runs on self-hosted Baileys (`services/whatsapp-platform/`, ECS Fargate) and the CRM only acts on the agency's own self-chat or whitelisted admin senders (`apps/crm/server/routes/webhooks.js`). Baileys is an unofficial client, so it is not safe for customer messaging (number-ban risk, `19-risk-analysis.md`).
...
- **Flags:** per-category feature toggles (`apps/crm/server/featureToggleService.js`) plus env flags (`AGENTS_ENABLED`, `AGENT_TOOL_LOOP_ENABLED`). There is no per-tenant flag service yet.
