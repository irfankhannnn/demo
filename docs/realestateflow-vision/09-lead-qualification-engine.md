...
Requirement data lives on the Lead as sub-objects: `buyerRequirement`, `tenantRequirement`, `sellerProperty` and `ownerProperty` (`agency-app/web/src/types/crm.ts`). The buyer object carries `requirement`, `budget`, `preferredArea`, `city`, `bhk`, `propertyType`, `propertySubType` and `timeline`. There are no `purpose` (end use vs investment) or `financing` fields.

Every intake adapter (`manychat`, `instagram`, `insta-agent`, `bailey`, `website`) creates leads through the same server-side ingestion path, which publishes an EventBridge `lead.created` event (`agency-app/api/leadIngestion.js`, `agency-app/api/routes/leads.js`).
...
|---|---|---|---|
| **CRM lead qualifier** | EventBridge `lead.created` | One Gemini call per lead scores it HOT / WARM / COLD against a written rubric, writes `score`, `scoreValue`, `scoreReasons`, `scoredAt`, `scoreSource: 'llm_text'`, then publishes `lead.qualified` | `agency-app/api/scripts/lead-qualifier-handler.js`, `agency-app/api/utils/leadRubric.js` |
| **Instagram DM analyst** | New or updated DM thread in the Instagram service | Pulls phone, budget, area, intent and property type with rules (or Gemini when a key is set), scores the lead, and drafts a Hinglish reply for a human to send | `agency-app/instagram-api/services/leadAnalyst.js`, `services/extract.js` |
| **AI qualification call** | A user presses "qualify call" (`POST /api/crm/leads/:id/qualify-call`) | The voice agent classifies the lead on the same rubric during a call capped at 180 s and submits the verdict silently; the CRM overwrites the text score with `scoreSource: 'ai_call'` | `agency-app/api/routes/leads.js`, `agency-app/api/routes/aiCallingInternal.js`, `agency-app/ai-calling/README.md` |
...
- The model call goes through `invokeAgent(tenantId, 'qualifier', ...)` in `agency-app/api/agents/agentRuntime.js`, which reads `GEMINI_API_KEY` and `GEMINI_MODEL` (CFN default `gemini-3.8-flash`).
...
|---|---|---|
| **AI voice call** | The lead | The only customer-facing conversational AI. ElevenLabs agent with nine server tools: `search_properties`, `get_property_details`, `schedule_site_visit`, `answer_policy_question`, `submit_qualification`, `request_human_handoff`, `confirm_site_visit`, `record_visit_feedback`, `request_callback` (`agency-app/ai-calling/elevenlabs-agent-tools.md`). Built, not live yet (see `12`). |
| **WhatsApp (Baileys)** | Agency staff | Staff command channel into the CRM agent, not a customer assistant (`agency-app/api/bailey.js`, `docs/proposals/agent-channel-architecture/flows/01-whatsapp-agent.md`). |
| **Web CRM chat** | Agency staff | Same agent runtime, operator-facing (`agency-app/api/agents/channels/webChannel.js`). |
| **Instagram** | Human sends | Drafted Hinglish replies only (`leadAnalyst.js`). |

Where the grounded facts come from:

- Property data: the CRM internal API (`agency-app/api/routes/aiCallingInternal.js`) and semantic matching through `match_properties` on DynamoDB vector search (`agency-app/api/services/embeddings/`).
- Policy answers: passages from the agency's own policy text (`agency-app/api/services/knowledge/policySearchService.js`, see `14`).
- Site visits and callbacks: `POST /api/internal/site-visits`, the voice tools above, public site-visit booking on property pages (`agency-app/api/siteVisitBooking.js`, `public-app/property-pages`), and "Schedule AI follow-up" jobs (`agency-app/api/services/followupService.js`).
- Brochure and floor-plan tools do not exist (none in `agency-app/api/shared/toolDefinitions.js`).

Prompt caching: the chat pipeline keeps a stable prompt prefix so Gemini's implicit caching applies; the ordering is locked by `agency-app/api/agents/llm/plannerPrompt.caching.test.js`. Explicit context caching is not used.

## 4. Follow-up as built

Follow-up is built in two independent pieces. Neither uses Step Functions.

**a) Follow-up call jobs: `agency-app/followup-agent`.** A DynamoDB job table plus an EventBridge tick every 5 minutes. Two job types:
...
Each job gets up to 2 call attempts, 45 minutes apart, inside the tenant's business hours. After that, or when the customer asks for a human, it escalates to the lead's assignee and the agency admins (`agency-app/followup-agent/README.md`).

**b) Stale-lead nudge: `agency-app/api/scripts/lead-followup-cron.js`.** Runs daily at 04:00 IST (`${Env}-realestateflow-lead-followup`). It finds leads in `new`, `contacted`, `qualified` or `negotiating` with no activity for 1 to 7 days, and either writes a draft note for review (`followupAgentMode: 'draft'`, the default) or sends a message over WhatsApp or email (`autosend`).

Other reminders that exist:

- Meeting reminders 15 minutes before a CRM meeting, drained every 5 minutes by `agency-app/api/scripts/meeting-reminder-cron.js`.
...
- `agency-app/api/routes/aiCallingInternal.js` is enabled and mounted at `/api/internal`. It serves lead context, property lookups, semantic property matching, policy answers, site visits and call outcomes.
- The regex intent classifier in the calling service was deleted in Sep 2026 (commit `bdff45e`). The ElevenLabs agent's own model now decides when to call server tools. `INTENT_TYPES` remain only as labels (`agency-app/ai-calling/src/config/constants.js`). On the chat side, `agency-app/api/agents/domainRouter.js` routes with rules first and a Gemini classifier as fallback.
- Lead conversion logic lives in `agency-app/api/services/leadConversionService.js`.
