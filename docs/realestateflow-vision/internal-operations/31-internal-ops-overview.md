...
Evidence for the product box: `agency-app/api/agents/`, `platform/mcp/`, `agency-app/api/leadIngestion.js`, `scripts/lead-qualifier-handler.js`, `lead-router-handler.js`, `agency-app/ai-calling/`, `agency-app/followup-agent/`.
...
- `landing-page-builder`: landing pages deploy to S3 + CloudFront through `infra/cicd/agency-app/landing-pages/deploy.sh`. No Vercel.
- `sdr`: email is Amazon SES with Brevo as fallback (`agency-app/api/emailService.js`). AiSensy appears only as a founder-side broadcast in the Razorpay webhook (`agency-app/api/routes/billing.js`).
...
| `nabi-crm` | The product MCP server (`platform/mcp/`), 72 CRM tools |
...
|---|---|---|
| Razorpay | Revenue, subscriptions, credit packs | REST API from the CRM backend (`agency-app/api/routes/billing.js`, `razorpayOrders.js`) |
| PostHog | Product analytics | SDK in the CRM app and server |
| SES / Brevo | Outbound email | `agency-app/api/emailService.js` |
