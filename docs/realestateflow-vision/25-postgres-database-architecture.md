...
### CRITICAL: Your `agency-app/api/` is JavaScript, not TypeScript

**Finding from codebase audit:**
- `agency-app/api/` has **zero TypeScript** — all `.js` files, no `tsconfig.json`, no `typescript` dependency, no build step.
- Only `platform/auth/` and `agency-app/web/` are TypeScript.
- Introducing a TypeScript ORM (`Drizzle`) means either:
  1. Rewrite all of `agency-app/api/` to TypeScript (massive, separate effort), **or**
...
| File | Purpose |
|---|---|
| `agency-app/api/db/schema.ts` | Drizzle schema definition (leads, contacts, properties, etc.) |
| `agency-app/api/db/migrations/001_init.sql` | Initial schema + RLS policies |
| `agency-app/api/db/client.ts` | Drizzle + RDS Proxy connection pool |
| `agency-app/api/db/middleware.ts` | Middleware to set `app.tenant_id` per request |
| `agency-app/api/services/leadService.ts` | Lead CRUD (Drizzle) |
| `agency-app/api/services/contactService.ts` | Contact CRUD + merge logic |
| `agency-app/api/services/propertyService.ts` | Property CRUD + search |
| `agency-app/api/services/visitService.ts` | Visit scheduling + tracking |
| `agency-app/api/services/taskService.ts` | Task management |
| `agency-app/api/services/khataService.ts` | Accounting transactions (ACID) |
| `agency-app/api/services/projectService.ts` | Project/developer management |
| `agency-app/api/services/agentActionService.ts` | Agent action audit log |
| `agency-app/api/services/conversationMetaService.ts` | Conversation thread metadata |

### Modified Files

| File | Changes |
|---|---|
| `agency-app/api/infra/cfn-backend.yaml` | Add Aurora cluster, RDS Proxy, VPC config; keep DynamoDB tables |
| `agency-app/api/db.js` or `agency-app/api/index.js` | Require new `agency-app/api/db/client.ts` + middleware |
| `agency-app/api/routes/*.js` (23 files) | Swap service imports (e.g., `crmDynamodbService` → `leadService`); routes unchanged |
...
- Frontend (`agency-app/web/`) — API contract identical
- Auth service (`platform/auth/`) — No changes
