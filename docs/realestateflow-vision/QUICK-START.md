...
|---|---|---|
| CRM (leads, contacts, properties, meetings, khata, billing) | Yes, ~270 mounted endpoints in 42 route files | `apps/crm/server/`, `apps/crm/real-estate-crm-app/` |
| Agent runtime (WhatsApp command channel + web chat) | Yes: in-house classify → plan → execute → compose pipeline, model gateway with Gemini as the one adapter, optional bounded tool loop. Off by default (`AGENTS_ENABLED`) | `apps/crm/server/agents/` |
| Lead qualification (Hot/Warm/Cold) and routing | Yes, EventBridge `lead.created` / `lead.qualified` handlers | `apps/crm/server/scripts/lead-qualifier-handler.js`, `lead-router-handler.js` |
| Lead ingestion pipeline | Yes: ManyChat + Instagram adapters → `ingestLead()` | `apps/crm/server/leadIngestion.js`, `docs/lead-adapter-architecture.md` |
...
| WhatsApp | Self-hosted Baileys, agency's own command channel only (non-self messages are dropped). Customer-facing WhatsApp not built | `services/whatsapp-platform/`, `apps/crm/server/routes/webhooks.js` |
| AI voice | Outbound calls (ElevenLabs + Exotel) and scheduled follow-up calls | `services/ai-calling-service/`, `services/followup-agent-service/` |
| MCP for Claude/ChatGPT | One server, 72 tools, own OAuth 2.1 | `services/reality-flow-mcp/` |
| Knowledge / RAG | DynamoDB vector search + Titan v2 embeddings (Bedrock, working in dev) | `apps/crm/server/services/embeddings/`, `services/knowledge/` |
| Credits | Ledger, metering on CRM writes and agent actions, Razorpay credit packs, monthly reset | `apps/crm/server/creditService.js`, `middleware/meterCredits.js` |
| Public property pages + visit booking | Yes | `apps/property-pages-ms/` |
| Mobile app | Capacitor app, store launch in prep | `apps/crm/real-estate-crm-app/capacitor.config.ts`, `docs/launch/MOBILE_LAUNCH_YOUR_TASKS.md` |
...
- **Grace period:** `payment.failed` already sets grace and `subscription.cancelled`/`halted` already update the DB. Still open: `subscription.charged` does not clear grace; the expiry script `apps/crm/server/scripts/grace-period-expiry-cron.js` is not wired into CFN; no server-enforced read-only mode after grace. See `29`.
...
| **One MCP server** | 72 tools generated from one registry (`apps/crm/server/shared/toolDefinitions.js`) shared by the WhatsApp agent, CRM backend and MCP server. |
...
2. Wire the grace-period expiry Lambda + rule into `apps/crm/server/infra/cfn-backend.yaml`, clear grace on `subscription.charged`, add server-side read-only.
