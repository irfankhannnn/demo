...
Code: `agency-app/api/scripts/lead-router-handler.js`. Lambda `${Env}-realestateflow-lead-router`, triggered by the EventBridge rule on `lead.qualified` (source `crm.leads`, published by the qualifier in `09`). Defined in `agency-app/api/infra/cfn-backend.yaml`.
...
- `'assigned'` is not a value of `LeadStatus` (`agency-app/web/src/types/crm.ts`: `new | contacted | qualified | site_visit | negotiating | converted | lost | spam`), so leads the router touches carry a status the UI does not recognise.
...
- A feature toggle `AI_ROUTING` exists in `agency-app/api/featureToggleService.js`; the router handler does not check it.

## 2. Manual assignment

- Users change the assignee on a lead through `PUT /api/crm/leads/:id` (`agency-app/api/routes/leads.js`). The new assignee gets a notification (`notifyLeadAssigned`).
...
- The auth service issues only `ADMIN` and `MEMBER` (`platform/auth/src/controllers/phoneAuthCustomController.ts`).
- The CRM middleware also accepts `MANAGER`, `OWNER` and `FOUNDER` strings, but nothing issues them (`agency-app/api/middleware/requireRole.js`).
...
| SLA timer with escalation / reassignment on a lead | Not built. `agency-app/api/scripts/escalation-cron.js` is about AI Employee *provisioning* SLA, not lead response. The nearest thing is `agency-app/followup-agent` escalating unreached follow-up calls to the assignee and admins. |
