...
**Account caveats.** The prod account is not verified for CloudFront: new distributions fail there, so `ENABLE_CLOUDFRONT=false` is the documented workaround (`apps/property-pages-ms/README.md`, `docs/insta-sol-ms-docs/09-CLOUDFRONT-INTEGRATION.md`). Bedrock invocation was refused on the same verification case; on 17 Sep 2026 it succeeded again in **dev**, and prod has not been re-tested (D12, `14`).
...
|---|---|---|---|
| CRM backend | `apps/crm/server/infra/cfn-backend.yaml` | `infra/cicd/server` | 13 Lambdas, 2 REST APIs + 4 nested route stacks, 18 DynamoDB tables, 11 EventBridge rules, 2 SQS queues, 1 S3 bucket, 8 alarms, 1 dashboard, SNS topic |
...
| CRM launch tables | `apps/crm/server/infra/launch-tables-cfn.yaml` | `infra/cicd/launch-tables` | 6 tables: grievances, webhook-log, tenant-api-keys, subscriptions, nps-responses, beta-invites |
| CRM frontend | `apps/crm/real-estate-crm-app/infra/cfn-frontend.yaml` | `infra/cicd/real-estate-crm-app` | S3 + CloudFront + OAC + CloudFront Function + Route 53 record |
| Marketing site | `apps/landing-pages/infra/cfn-landing-pages.yaml` | `infra/cicd/landing-pages` | S3 + CloudFront + response-headers policy (CSP, HSTS) + 2 Route 53 records |
| Property pages | `apps/property-pages-ms/infra/cfn-property-pages.yaml` | `infra/cicd/property-pages-ms` | Lambda, REST API, CloudFront with cache/origin-request policies, `pages-guard` table |
| Instagram API | `apps/instagram/backend_insta_sol_ms/infra/cfn-insta-sol-ms.yaml` | `infra/cicd/backend_insta_sol_ms` | Lambda, REST API (throttled), 2 tables, 1 scheduled sync rule |
| Instagram console | `apps/instagram/frontend_insta_sol_ms/infra/cfn-insta-frontend.yaml` | `infra/cicd/frontend_insta_sol_ms` | S3 bucket only — served at `/insta/` from the CRM frontend's distribution |
| Auth | `services/reality-flow-authentication/infra/cfn-backend.yaml` (+ `auth-explicit-routes.yaml`) | `infra/cicd/reality-flow-authentication` | Cognito user pool, domain, client, IdP, branding; 4 Lambdas; 3 tables; REST API with access logging |
| MCP server | `services/reality-flow-mcp/infra/cfn-backend.yaml` | `infra/cicd/reality-flow-mcp` | 2 Lambdas, REST API with a Lambda authorizer, 2 OAuth tables |
| AI calling | `services/ai-calling-service/infra/cfn-ai-calling.yaml` | `infra/cicd/ai-calling-service` | Lambda, REST API, 1 table, knowledge + recordings buckets, 1 Secrets Manager secret |
| Follow-up agent | `services/followup-agent-service/infra/cfn-followup.yaml` | `infra/cicd/followup-agent-service` | 2 Lambdas, REST API, 1 table, 4 EventBridge rules, SQS DLQ, 2 alarms, 1 secret |
| WhatsApp platform | `services/whatsapp-platform/infra/cfn-platform.yaml` | `infra/cicd/whatsapp-platform` | ECS cluster + Fargate service + task definition, session table, session bucket, KMS key, 6 alarms, 4 log metric filters, 3 rules, 1 secret |
...
`apps/crm/server/infra/apigw-explicit-routes.yaml` is a source file for the part1/part2 splitter, not a deployed stack. `apps/onboarding` has no template: it is a local-only tool that writes a tenant row into the AgencyConfig table.
...
- **Lambda** runs everything except the WhatsApp transport: the CRM API (one Express handler behind both REST APIs), event handlers, scheduled jobs, the MCP server, auth, the calling service, the follow-up worker, the Instagram sync worker and the property-pages renderer. Agents run **inside** the CRM API Lambda and inside the qualifier / router Lambdas, calling CRM tools in-process through `apps/crm/server/skillInvoker.js` — not over MCP.
- **ECS Fargate** runs only `services/whatsapp-platform`, the self-hosted Baileys workers, on a `FARGATE_SPOT` / `FARGATE` capacity-provider strategy in the shared VPC's private subnets. The voice bridge needs no container: ElevenLabs hosts the agent and connects to Exotel directly (`services/ai-calling-service/README.md`).
...
**Not built:** Step Functions (the follow-up engine is a DynamoDB job table plus the 5-minute tick) and Redis / ElastiCache. Deduplication is done with DynamoDB conditional writes instead (`apps/crm/server/whatsappConversationService.js`). The shared VPC template reserves a cache subnet tier and security group, so adding ElastiCache later is a stack change, not a redesign.
...
`netlify.toml` files still sit in `apps/crm/real-estate-crm-app/` and `apps/landing-pages/`. Netlify was the original host and is not the deploy path any more; CloudFront is. The landing-pages CFN response-headers policy reimplements what `netlify.toml` used to set.
...
- **Custom metrics** from `apps/crm/server/observability/cloudwatch.js` and `phase2Metrics.js`.
- **Sentry** (`apps/crm/server/lib/sentry.js`) and **PostHog** (`lib/posthog.js`).
...
| Server-enforced read-only after grace | `apps/crm/server/scripts/grace-period-expiry-cron.js` exists but no EventBridge rule schedules it | Before paid launch |
