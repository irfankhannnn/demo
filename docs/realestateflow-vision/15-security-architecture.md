...
- Cognito-based auth microservice (`platform/auth`): phone OTP, Google sign-in, PKCE, refresh token in an httpOnly cookie.
- Tenant id derived on the server, not taken from the client.
- CRM Express hardening: CORS allowlist (`agency-app/api/utils/corsOrigins.js`), security headers, auth rate limiting, timing-safe API-key compares, structured logging.
- Phone numbers masked in every `/api/crm/*` JSON response (`agency-app/api/middleware/phoneMasking.js`).
- DPDP grievance route (`agency-app/api/routes/grievance.js`).
...
**Rotation status: unconfirmed.** `docs/agency-app/ai-calling/GO-LIVE-RUNBOOK.md` still lists rotation as pending.
...
- **Service to service today:** shared API keys plus an `x-tenant-id` header, compared with timing-safe checks (for example `agency-app/api/routes/adapterIngestionInternal.js`). Examples:
  ...
  (`agency-app/followup-agent/README.md`, `agency-app/ai-calling/README.md`, `agency-app/api/infra/cfn-backend.yaml`)
- **External AI clients:** the MCP server runs its own OAuth, validating Cognito user tokens (`platform/mcp/src/routes/oauth.ts`, `src/middleware/validateToken.ts`).
- **Later:** per-service machine identities (for example Cognito M2M clients) with scopes, replacing shared keys. Not scheduled.

## 4. Roles (D15)

Today the auth service issues only `ADMIN` and `MEMBER` (`platform/auth/src/controllers/phoneAuthCustomController.ts`). The CRM middleware already accepts `MANAGER`, `OWNER` and `FOUNDER` (`agency-app/api/middleware/requireRole.js`), but nothing issues them. Members can see every lead in the tenant.
...
| Dimension | As built |
|---|---|
| **CRM data** | `tenantId` attributes with tenant-scoped GSIs (for example `search-index`, `tenant-index`) in the CRM table; some list functions still scan and filter by tenant (`agency-app/api/crmDynamodbService.js`, TODO MED-1) |
| **Calling data** | Keys prefixed `TENANT#{tenantId}#CALL#...` (`agency-app/ai-calling`) |
| **Vector search** | `tenantId` is the HASH key of both vector search schemas; AWS rejects a search without it (`agency-app/api/infra/create-vector-index.mjs`, see `14`) |
| **Voice tools** | Tenant from a bound header, never from a model parameter |
| **Instagram** | Per-tenant OAuth connection; tokens AES-256-GCM encrypted in DynamoDB (`agency-app/instagram-api/services/metaSecurity.js`); the service's IAM role reaches only its own two tables and talks to the CRM over the internal API |
| **Files** | S3 object keys prefixed with tenant id (`<tenantId>/call-recordings/...`) |
| **Credits / usage** | Per-tenant credit ledger (`agency-app/api/creditService.js`) |

Not built: per-tenant KMS keys, per-tenant secrets, automated cross-tenant isolation tests in CI.

## 6. Secrets and encryption

- **CRM backend:** about 100 config values, keys included, live in SSM Parameter Store (SecureString) and are loaded at cold start (`agency-app/api/config/ssmBootstrap.js`, synced by `agency-app/api/infra/sync-ssm-params.sh`).
...
- **KMS:** one customer-managed key, in `platform/whatsapp-platform/infra/cfn-platform.yaml`. Everything else uses AWS-managed encryption.
- **S3 documents / recordings bucket** (`agency-app/api/infra/cfn-backend.yaml`): public access blocked, SSE-S3, no expiration rule (customer records are never deleted), versioning explicitly `Suspended`.

> **Open question:** re-enable versioning on the documents / recordings bucket? Not decided.

## 7. Audit and observability

- `AgentAuditTable` records agent actions (`agency-app/api/agents/agentAuditService.js`), but rows carry a **90-day TTL**, so DynamoDB deletes them. **D17:** archive audit rows instead of letting TTL delete them.
...
|---|---|---|
| API throttling on the CRM API | Not built. Rate limiting is an in-memory Express limiter (`agency-app/api/middleware/rateLimiter.js`), which doesn't hold across Lambda containers. API Gateway throttling exists only on the Instagram and property-pages stacks. | **Before paid launch** |
| API Gateway access logs on the CRM API | Not built | **Before paid launch** |
| CORS on API Gateway gateway responses | Still `Access-Control-Allow-Origin: '*'` on 5 gateway responses (`agency-app/api/infra/cfn-backend.yaml`); the Express allowlist is correct | **Before paid launch** |
| Server-enforced read-only after the grace period | `agency-app/api/scripts/grace-period-expiry-cron.js` exists but no EventBridge rule schedules it | **Before paid launch** |
...
- **WhatsApp:** Baileys (self-hosted, `platform/whatsapp-platform`) is used only as a staff command channel. Customer messaging moves to the official WhatsApp Business Cloud API (D9, `39`). Chatwoot is dropped.
...
| Runaway cost | Credit checks before AI calls and agent actions (`agency-app/api/middleware/meterCredits.js`) |
