...
**Done:** the leaking `deploy-lambda.ps1` is deleted and the AI calling stack takes NoEcho parameters into Secrets Manager (`agency-app/ai-calling/infra/cfn-ai-calling.yaml`, `src/config/secretsBootstrap.js`); env files untracked; SSM SecureString sync (`agency-app/api/infra/sync-ssm-params.sh`); DynamoDB PITR on all CRM tables and S3 versioning (`agency-app/api/infra/cfn-backend.yaml`); a CORS allowlist in Express (`agency-app/api/server.js`).
...
**Done:** `payment.failed` starts a 7-day grace period and `subscription.cancelled`/`halted` update the Subscriptions table (`agency-app/api/routes/billing.js`); the paywall respects grace (`components/PaywallModal.tsx`); the expiry script exists (`agency-app/api/scripts/grace-period-expiry-cron.js`).
...
**Done differently:** a DynamoDB credit ledger instead of Lago — 1 credit = ₹1, 1,000 free monthly credits, `agent_action` 15 credits, `ai_call_per_minute` 15, manual CRM actions 0; atomic deduction with HTTP 402 on a short balance; refund on a failed agent turn; Razorpay one-time Orders for top-up packs; admin cost editor; balance and ledger in the UI (`agency-app/api/creditConfig.js`, `creditService.js`, `middleware/meterCredits.js`, `razorpayOrders.js`, `routes/creditAdmin.js`, `routes/subscriptions.js`, `components/CreditBalanceCard.tsx`).
...
**Done:** Capacitor 8 wrapper with real Android and iOS projects, native camera, mobile layouts, push device tokens (`agency-app/web/capacitor.config.ts`, `android/`, `ios/`, `agency-app/api/services/push/`).
...
**Done:** outbound calling only (D16), inside tenant business hours, with role-based phone masking (`agency-app/ai-calling/`, `agency-app/followup-agent/`, `agency-app/api/middleware/phoneMasking.js`).
...
**Done:** the CRM backend checks ADMIN/MANAGER/FOUNDER/OWNER/MEMBER (`agency-app/api/middleware/requireRole.js`) and masks phone numbers by role; MCP clients carry OAuth scopes (`platform/mcp/scripts/check-scopes.ts`).
**Not started:** the auth service still issues only ADMIN/MEMBER (`platform/auth/src/models/usersModel.ts`); no MANAGER role, no "members see only their own leads", no team or region scoping; two mounted DELETE routes have no role check (`routes/aiIntegrations.js`, `routes/notifications.js`).
...
**Done differently:** one MCP service exposing 72 tools generated from the canonical registry that the WhatsApp agent and CRM backend share, with tenant derived from the token, not from arguments (`platform/mcp/`, `agency-app/api/shared/toolDefinitions.js`, `scripts/generate-mcp-tools.mjs`). Not AgentCore Gateway (D10).
**Partly done:** tenant-scoped reads (MED-1) — `getLeads` queries `search-index`, other list functions still scan (`TODO(MED-1)` in `agency-app/api/crmDynamodbService.js`).
**Done differently:** conversation state lives in `agency-app/api/conversationStateService.js` and `whatsappConversationService.js` rather than in new generic conversation tables.
...
**Open question:** the website visitor chat widget and Meta Lead Ads sync are not built and are not in the D14 scope list — are they still wanted? Public property pages with site-visit booking (`public-app/property-pages/`) cover part of the website need today.

### REF-E12 · AI sales assistant — **Partly done**
**Done:** an in-house agent core (classify → plan → execute tools → compose, optional bounded tool loop) on Gemini behind a model gateway, serving the WhatsApp command channel and in-CRM web chat; grounded property and policy search that returns nothing on a weak match; `create_meeting` and site-visit booking; follow-up `draft` mode as a light approval step (`agency-app/api/agents/`, `services/knowledge/`, `siteVisitBooking.js`, `routes/aiEmployeeConfig.js`).
...
**Done differently:** `agency-app/followup-agent/` schedules, retries and escalates follow-up calls on EventBridge + DynamoDB — site-visit confirmation and post-visit feedback — with `draft` and `autosend` modes. Not Step Functions.
...
**Done differently:** DynamoDB vector search with Titan v2 embeddings and tenant id as the index partition key, instead of Bedrock Knowledge Bases on S3 Vectors (`agency-app/api/services/embeddings/`, `services/knowledge/`, `infra/create-vector-index.sh`).
...
**Done:** outbound qualification and follow-up calls on ElevenLabs agents through their native Exotel integration, with mid-call CRM tools, semantic property search and Hinglish intents (`agency-app/ai-calling/`).
...
**Done:** golden-conversation tests and a tool-choice eval harness (`agency-app/api/agents/goldenConversations.test.js`, `eval/whatsapp-tool-choice.eval.js`); Call Intelligence actions require approval (`services/callIntelligence/actionExecutor.js`).
...
An agent-based router assigns qualified leads (`agency-app/api/scripts/lead-router-handler.js`). There is no rule chain, no capacity or availability model and no SLA timers. Same note as REF-E14: superseded by the next-generation lead engine (D11).

### REF-E33 · Reporting & analytics — **Partly done**
**Done:** a team analytics dashboard over DynamoDB with Excel export (`agency-app/web/src/pages/admin/TeamAnalytics.tsx`, `agency-app/api/routes/admin.js`, `utils/excel.js`), plus PostHog product analytics.
...
### REF-E43 · Partner / product MCP surface — **Done**
`platform/mcp/` is usable from Claude and ChatGPT with its own OAuth 2.1 (`docs/MCP_AGENCY_GUIDE.md`, `37-product-agents-mcps-guide.md`).
...
| Work | Where |
|---|---|
| Public property pages + site-visit booking microservice | `public-app/property-pages/` |
| Call Intelligence: recording upload → transcription → AI review → approved actions | `agency-app/api/services/callIntelligence/`, `routes/callRecordings.js` |
| Click-to-call with phone masking | `agency-app/api/routes/clickToCall.js` |
| Khata (ledger) | `agency-app/api/routes/khata.js` |
| Seat caps, paywall, trial countdown, trial reminders | `agency-app/api/routes/subscriptions.js`, `components/PaywallModal.tsx`, `SeatCounter.tsx` |
...
| Landing pages build pipeline + 12 pages with JSON-LD | `agency-app/landing-pages/` |
