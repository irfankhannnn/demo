...
> **Status (17 Sep 2026):** As built + roadmap. Checked against the code on main. The migration this document planned has happened: the SyncBot `npx tsx` CLI is gone and the CRM's in-process tool registry (`apps/crm/server/shared/toolDefinitions.js`, 72 tools) is the product surface, republished as MCP by `services/reality-flow-mcp` (`05`). Paths, counts and `skills-lock.json` are corrected; the tenant marketing agent is dropped.
...
- **Registry hygiene.** `services/reality-flow-mcp/README.md` still says 54 tools (`05 §2`). The root `CLAUDE.md` skills registry is a catalogue, not an inventory, and says so nowhere.
