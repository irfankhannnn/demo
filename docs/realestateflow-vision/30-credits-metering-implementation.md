...
| Part | File |
|---|---|
| Balance + ledger (grant, deduct, refund, monthly reset, paginated ledger) | `agency-app/api/creditService.js` |
| Costs, packs, free allotment (defaults + runtime config, 60 s cache) | `agency-app/api/creditConfig.js` |
| Metering helpers (`precheckCredits`, `chargeCreditsForAction`, `meterCredits`, 402 response) | `agency-app/api/middleware/meterCredits.js` |
| AI turn charge and refund | `agency-app/api/agents/agentRuntime.js` (`invokeAgent`) |
| AI call charge and pre-call check | `agency-app/api/aiCallBilling.js`, called from `routes/aiCallingInternal.js`, `routes/aiCalling.js`, `routes/leads.js` |
| Tenant API | `agency-app/api/routes/subscriptions.js` (`/api/subscriptions/credits*`) |
| Operator API | `agency-app/api/routes/creditAdmin.js` (`/api/credit-config`) |
| Pack purchase → grant | `agency-app/api/routes/billing.js` (`payment.captured`), `razorpayOrders.js` |
| Monthly reset | `agency-app/api/scripts/credit-reset-cron.js` |
| Balance repair (manual script) | `agency-app/api/scripts/credit-balance-reconcile.js` |
| Tables | `CreditsTable`, `CreditConfigTable` in `agency-app/api/infra/cfn-backend.yaml` |
| Frontend | `src/contexts/CreditsContext.tsx`, `src/hooks/useCredits.ts`, `src/components/BuyCreditsModal.tsx`, `src/pages/crm/BillingSettings.tsx` (under `agency-app/web/`) |
...
| # | Issue | Evidence |
|---|---|---|
| C1 | **Pack prices differ between server and UI.** The server charges ₹500 / ₹2,000 / ₹5,000; `BuyCreditsModal` shows ₹499 / ₹1,799 / ₹3,999. The customer pays the server price. The modal should read packs from `GET /api/subscriptions/credits`. | `creditConfig.js` `creditPacks`; `agency-app/web/src/components/BuyCreditsModal.tsx` `PACK_OPTIONS` |
