...
Two vector indexes, one embedding model, one shared query path. All code is in `apps/crm/server/services/`.
...
| Backfill / repair | `apps/crm/server/scripts/backfill-property-embeddings.js` | `apps/crm/server/scripts/reindex-policies.js`, or `POST /api/crm/agency-policies/reindex` |
...
**Index creation:** CloudFormation cannot declare DynamoDB vector indexes. `KnowledgeChunksTable` is in `apps/crm/server/infra/cfn-backend.yaml`; both indexes are created by `apps/crm/server/infra/create-vector-index.sh <dev|prod>` (`create-vector-index.mjs`). Whether the indexes and backfills have been run in each environment is not recorded in the repo.

## 3. Who uses it

| Consumer | Uses | Path |
|---|---|---|
| AI voice agent | Property matching and policy answers | ElevenLabs tools → `services/ai-calling-service/src/services/crmApiService.js` → `POST /api/internal/properties/match`, `POST /api/internal/policies/answer` (`apps/crm/server/routes/aiCallingInternal.js`) |
| WhatsApp staff agent, web CRM chat | Property matching | `match_properties` tool from `apps/crm/server/shared/toolDefinitions.js`, run in-process |
| External AI clients (MCP) | Property matching | Same tool, exposed by `services/reality-flow-mcp` |
| Agency admins | Write and re-index policies | `apps/crm/server/routes/agencyPolicies.js` (read: members; save and re-index: admin/manager) |

Policy answers are only wired to the voice agent today; there is no policy tool in the chat tool registry. `services/ai-calling-service` has no Bedrock grant and reaches knowledge only through the CRM.

File uploads to the calling service (`/api/ai-calling/knowledge/*`) are stored but **not indexed**: `/confirm` returns 501 and the API tells users to enter policy text in the CRM (`services/ai-calling-service/src/routes/knowledge.js`).
...
- On 3 Sep 2026 Bedrock `InvokeModel` was refused on the AWS account, including for Titan embeddings, while an account-verification case was open (`docs/services/ai-calling-service/AWS-CASE-178749035000906-BEDROCK-REPLY.md`).
