...
Customers are **not billed per token**. The rule in code is **"humans work for free, AI costs credits"** (`apps/crm/server/creditConfig.js`, comment above `COSTS`):
...
|---|---|---|
| Subscriptions | Razorpay subscriptions; webhook handles charge, failure, cancel, halt, seat changes; events de-duplicated through a webhook log | `apps/crm/server/routes/billing.js`, `apps/crm/server/webhookLogService.js` |
| Trial | 14 days by default (`TRIAL_DAYS`), created on first visit, with 1,000 starter credits | `apps/crm/server/subscriptionService.js`, `apps/crm/server/routes/subscriptions.js` (`/trial-status`) |
| Paywall + trial banner | Shown after trial expiry unless paying or in grace | `apps/crm/real-estate-crm-app/src/components/PaywallModal.tsx`, `TrialCountdownBanner.tsx` |
| Seat caps | Seat check endpoint; seats paid/used kept in step with Razorpay quantity changes | `apps/crm/server/routes/subscriptions.js` (`/check-seat`), `routes/billing.js` |
| Grace period | `payment.failed` gives a paying tenant 7 days (`GRACE_PERIOD_DAYS`) and emails the founder | `apps/crm/server/routes/billing.js` |
| Grace expiry | Script that ends grace and suspends AI Employee provisioning | `apps/crm/server/scripts/grace-period-expiry-cron.js` (**not scheduled**, see §7) |
| Credit ledger | Balance row + ledger rows per tenant in one DynamoDB transaction; HTTP 402 when balance is short | `apps/crm/server/creditService.js`, `CreditsTable` in `apps/crm/server/infra/cfn-backend.yaml` |
| Metering | Per-route middleware and helpers; a cost of 0 short-circuits | `apps/crm/server/middleware/meterCredits.js` |
| Agent turn charge | 15 credits deducted before the turn, refunded if the turn fails without a CRM write | `apps/crm/server/agents/agentRuntime.js` (`AGENT_ACTION_CREDITS`, `refundCredits`) |
| AI call charge | 15 credits per started minute, from the settled duration on the post-call webhook | `apps/crm/server/aiCallBilling.js`, `routes/aiCallingInternal.js` |
| Monthly reset | Scheduled Lambda resets monthly credits on the billing anniversary | `apps/crm/server/scripts/credit-reset-cron.js`, `CreditResetFunction` schedule in `cfn-backend.yaml` |
| Top-up packs | One-time Razorpay Orders; `payment.captured` grants the credits | `apps/crm/server/razorpayOrders.js`, `routes/subscriptions.js` (`/credits/purchase`), `routes/billing.js` |
...
| Admin cost editor | Platform operators change costs, packs and free tier without a deploy (60 s config cache) | `apps/crm/server/routes/creditAdmin.js` (mounted at `/api/credit-config`), `CreditConfigTable` |
...
| AI Employee add-on | ₹7,999/mo, no trial, concierge setup | Provisioned by hand (`apps/crm/server/aiEmployeeProvisioningService.js`) |
...
- Cost safety today: hard stop at zero balance (402), tool-loop step cap of 6 (`AGENT_TOOL_LOOP_MAX_STEPS` in `apps/crm/server/agents/llm/runToolLoop.js`), refund on a failed agent turn.
...
- **Cheap model first:** Gemini Flash for planning/compose and a lighter Flash model for classification (`GEMINI_MODEL`, `GEMINI_CLASSIFIER_MODEL` in `apps/crm/server/infra/cfn-backend.yaml`); a rules fast-path skips the classifier call when it can (`apps/crm/server/agents/domainRouter.js`). Models sit behind a gateway so a cheaper provider can be added (`apps/crm/server/agents/modelGateway/index.js`).
- **Stable prompt prefix** for provider-side caching (commit `cb67418`).
- **Agents off unless enabled** (`AGENTS_ENABLED`, default `false`).
- **WhatsApp today** runs on self-hosted Baileys (`services/whatsapp-platform/`), so there is no Meta per-message fee; the cost is ECS Fargate. Moving customer messaging to the official WhatsApp Business Cloud API (`39-whatsapp-official-api-plan.md`) adds Meta per-message charges; keep replies inside the free 24-hour service window and prefer Utility over Marketing templates.
- **Voice** is ElevenLabs + Exotel per minute, recovered at 15 credits per minute (`services/ai-calling-service/`).
- **Serverless scale-to-zero;** no OpenSearch or EKS. Vector search runs on DynamoDB (`apps/crm/server/services/embeddings/`, `services/knowledge/`).
