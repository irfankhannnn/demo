...
| Question | Answer |
|---|---|
| What is live today? | One self-hosted Baileys gateway (`services/whatsapp-platform`). It powers the **owner's self-chat command channel** — the owner messages their own number and the AI Employee runs CRM tools. Customer messages are **dropped** (`apps/crm/server/routes/webhooks.js:162-176`). |
| Is the landing-page claim true? | Not yet. `apps/landing-pages/main/index.html:28,217` says RealEstateFlow "connects to the WhatsApp Business API". Fix the copy or ship the adapter. |
...
| Fact | Evidence |
|---|---|
| Multi-tenant self-hosted service on `@whiskeysockets/baileys` 6.7.23, ECS Fargate (Spot with on-demand fallback), up to 100 sessions per task | `services/whatsapp-platform/README.md:1-4`, `package.json:21`, `infra/cfn-platform.yaml:504-577` |
...
- Tenant ↔ number: exactly one `AgencyConfig.connectedWhatsAppPhone` per tenant, GSI-indexed (`cfn-backend.yaml:1072-1095`), linked by QR scan (`apps/crm/real-estate-crm-app/src/pages/onboarding/ConnectWhatsApp.tsx:49-208`). There are **no** `wabaId`, `phoneNumberId`, token, template or opt-in fields.
...
2. **Business Verification for Cloudberry IT Solutions is not done** (`docs/insta-sol-ms-docs/10-APP-REVIEW.md:24,50-54`).
...
2. `apps/crm/server/whatsapp/transport/` with `WhatsAppTransport` (`sendText`, `sendTemplate`, `sendInteractive`, `markRead`, `getWindowState`) and two adapters, `baileysTransport` and `cloudApiTransport`. Route all 10 `bailey.js` importers through it.
...
2. Staff command channel v2: staff message the platform number from their own phones; sender phone → user/tenant via `GET /internal/users/by-whatsapp` (`services/reality-flow-authentication/src/routes/internal.ts:60-64`); multi-tenant users pick a workspace. Digests become utility templates.
...
| 17 | Regression | Baileys self-chat still works while `provider=baileys` | `apps/crm/server` jest suites green |
