...
The domain is already exposed as **one tool registry** (`agency-app/api/shared/toolDefinitions.js`) that the in-house agent runtime runs in-process via `agency-app/api/skillInvoker.js`. The older SyncBot CLI skills (now reference only in `tools/openclaw_workspace_reference/skills/*`) were retired in its favour. MCP publishes the same tools through a **standard, discoverable, auth-aware interface** that Claude, ChatGPT and partner tools can use.

## 2. As Built: One Server

| Property | Today |
|---|---|
| Server | `platform/mcp` (TypeScript, Express), Streamable HTTP, on Lambda + API Gateway (`platform/mcp/infra/cfn-backend.yaml`) |
...
| Customer use | Agencies can connect Claude/ChatGPT today (`docs/MCP_AGENCY_GUIDE.md`, `agency-app/api/routes/aiIntegrations.js`) |

Known gap: `platform/mcp/README.md` still says 54 tools.
...
| Voice | `agency-app/ai-calling` (not in MCP) | `start_call`, `get_transcript` |
...
| Knowledge | `agency-app/api/services/knowledge/*` + vector search (not in MCP) | `query_knowledge` |
