...
- **Web only.** The mobile builds show no prices and no checkout (App Store 3.1.1 / Google Play billing rules); `agency-app/web/src/lib/razorpay.ts` refuses to open checkout inside the native app.

**Pricing** is not defined here. Current (pre-launch) pricing is in `marketing-and-sales/launch-plan-v2/pricing.json` and CRM `agency-app/web/src/lib/plans.ts`, which disagree on Team+; it is being replaced by the proposal in `docs/realestateflow-vision/38-pricing-plan-contacts-and-credits.md`.
...
| Part | File |
|---|---|
| Webhook receiver (all Razorpay events) | `agency-app/api/routes/billing.js`, mounted at `/api/billing` before `express.json()` with its own rate limit (`agency-app/api/server.js`) |
| Subscription row helpers | `agency-app/api/subscriptionService.js` |
| Tenant-facing subscription and credit API | `agency-app/api/routes/subscriptions.js`, mounted at `/api/subscriptions` |
| Razorpay Orders client (credit packs) | `agency-app/api/razorpayOrders.js` |
| Webhook idempotency log | `agency-app/api/webhookLogService.js` (table `${Env}-realestateflow-webhook-log`, 30-day TTL) |
| AI Employee provisioning | `agency-app/api/aiEmployeeProvisioningService.js` |
| Tables | `SubscriptionsTable`, `WebhookLogTable` in `agency-app/api/infra/launch-tables-cfn.yaml` (deployed via `infra/cicd/agency-app/launch-tables`) |
| Scheduled jobs | `agency-app/api/scripts/trial-reminder-cron.js`, `grace-period-expiry-cron.js`, `credit-reset-cron.js`, `escalation-cron.js` |
| Frontend | `src/lib/razorpay.ts`, `src/components/PaywallModal.tsx`, `src/components/TrialCountdownBanner.tsx`, `src/components/BuyCreditsModal.tsx`, `src/pages/crm/BillingSettings.tsx`, `src/lib/plans.ts` (all under `agency-app/web/`) |

Secrets (`RazorpayKeySecret`, `RazorpayWebhookSecret`, `BrevoApiKey`, …) reach the CRM Lambda as `NoEcho` CloudFormation parameters synced from SSM (`agency-app/api/infra/sync-ssm-params.sh`, `ssm-param-map.txt`). Moving them to Secrets Manager is a goal, not the current state.
...
- **Trial:** created on post-registration (`agency-app/api/routes/auth.js`) or auto-created by `GET /api/subscriptions/trial-status`. Reminder emails at day 10, 12 and 14 and 3 days after expiry, from `TrialReminderFunction` on `cron(30 3 * * ? *)` UTC (`agency-app/api/infra/cfn-backend.yaml`).
...
| 3 | Grace expiry cron | **Half done.** Script exists; not scheduled. | Add `GracePeriodExpiryFunction` + rule to `agency-app/api/infra/cfn-backend.yaml` (handler `scripts/grace-period-expiry-cron.handler`) |
...
6. **CORS fix** (allowlist is in `agency-app/api/utils/corsOrigins.js`).
7. **Archive audit-log rows instead of TTL delete** (agent audit rows currently expire after 90 days, `agency-app/api/agents/agentAuditService.js`).
