...
|---|---|---|---|
| CRM SPA | `apps/crm/real-estate-crm-app/` | Mature (63 page files, 68 component files) | Keep and extend |
| CRM backend API | `apps/crm/server/` | Mature (42 route files, ~270 mounted endpoints) | Keep; fix access patterns |
| Auth microservice | `services/reality-flow-authentication/` | Mature (Cognito, phone OTP, Google, invites) | Keep; add MANAGER role |
| Onboarding tool | `apps/onboarding/` | Local tool, still used | Keep as internal tool |
| Agent runtime | `apps/crm/server/agents/` | Built, flag-gated | Direction for all agents (`04`) |
| Tool registry | `apps/crm/server/shared/toolDefinitions.js`, `apps/crm/server/skillInvoker.js` | Built (72 tools) | Single source for agent + MCP tools |
| Lead ingestion | `apps/crm/server/leadIngestion.js` (`POST /api/internal/adapters/leads`) | Built (ManyChat, Instagram) | Add portal and WhatsApp adapters |
| Instagram service | `apps/instagram/{backend,frontend}_insta_sol_ms/` | Built, dev only | Prod after Meta App Review |
| WhatsApp platform | `services/whatsapp-platform/` | Built (Baileys on ECS Fargate) | Agency command channel only; customer messaging moves to official API (`39`) |
| AI calling | `services/ai-calling-service/` | Built, routes mounted in CRM | Outbound at launch |
| Follow-up calls | `services/followup-agent-service/` | Built | Keep |
| MCP server | `services/reality-flow-mcp/` | Built (72 tools) | Keep one server (`05`) |
| Property pages | `apps/property-pages-ms/` | Built | Keep |
| Landing pages | `apps/landing-pages/` | Built | Keep; fix stale claims |
| Mobile app | `apps/crm/real-estate-crm-app/` (Capacitor, `android/`, `ios/`) | Store launch in prep | Keep |
...
| Crons | `apps/crm/server/scripts/*-cron.js` | 8 scheduled rules in the backend stack | Wire grace-period expiry (not scheduled) |
| Tests | `tests/playwright/` (31 specs), jest in `apps/crm/server` | Jest in CI; Playwright not in CI | Run E2E in CI |

## 3. CRM Core

### 3.1 Backend (`apps/crm/server/`)
...
- **API Gateway REST APIs** on `services-api.cloudberrysolutions.in`, explicit route templates in `apps/crm/server/infra/apigw-explicit-routes*.yaml`.
...
- **Not mounted:** developers, projects, realEstateAreas, buildings/flats, areas (`docs/services/server/DISABLED_FEATURES.md`).
...
- WhatsApp conversations and state are rows in the CRM table (`apps/crm/server/whatsappConversationService.js`, `conversationStateService.js`), not separate tables.

### 3.3 Frontend (`apps/crm/real-estate-crm-app/`)
...
- Instagram console served at `/insta/*` from `apps/instagram/frontend_insta_sol_ms`.
...
- **Roles:** the auth model has only `'ADMIN' | 'MEMBER'` (`services/reality-flow-authentication/src/models/usersModel.ts`). The server also accepts FOUNDER/OWNER/MANAGER (`apps/crm/server/middleware/requireRole.js`) and PLATFORM_OPERATOR/SUPER_ADMIN for credit admin only (`requirePlatformOperator.js`).
...
- **Pricing:** current pre-launch pricing is in `marketing-and-sales/launch-plan-v2/pricing.json` and CRM `apps/crm/real-estate-crm-app/src/lib/plans.ts`, which disagree on Team+. It is being replaced by the proposal in `38-pricing-plan-contacts-and-credits.md` (Proposed).
- **Credits:** `CreditsTable` with ledger and `CreditConfigTable` (`apps/crm/server/creditService.js`), metering on CRM write routes (`middleware/meterCredits.js`) and per agent action (`agents/agentRuntime.js`), Razorpay credit-pack orders (`razorpayOrders.js`), per-minute AI call billing (`aiCallBilling.js`), monthly reset cron. Detail: `30`.
...
**Bedrock:** on 17 Sep 2026 Bedrock `invoke-model` succeeded in the dev account (ap-south-1) for `amazon.titan-embed-text-v2:0` and Claude Haiku 4.5. The earlier access block (`docs/services/ai-calling-service/AWS-CASE-178749035000906-BEDROCK-REPLY.md`) is resolved for dev; the prod account has not been re-tested.

## 4. Agent Runtime and Channels

- **Pipeline** (`apps/crm/server/agents/agentRuntime.js`): classify (rules fast-path, then `GEMINI_CLASSIFIER_MODEL`) → plan (one scoped tool call, or chat/clarify) → execute through the in-process tool registry (`skillInvoker.js`) → compose a channel-shaped reply.
...
- **WhatsApp is the agency owner's command line only.** The CRM webhook skips any message that is not self-chat/`fromMe` as `unauthorized_sender` (`apps/crm/server/routes/webhooks.js`). Customer-facing WhatsApp is not built.
- **Audit and metrics:** `AgentAuditTable` (90-day TTL), CloudWatch metrics (`apps/crm/server/observability/`). No LLM tracing.
- **Evals:** `agents/goldenConversations.test.js` checks the deterministic formatter; `apps/crm/server/eval/whatsapp-tool-choice.eval.js` runs against live Gemini outside `npm test`.
...
Paths: `apps/crm/server/leadIngestion.js`, `scripts/lead-qualifier-handler.js`, `scripts/lead-router-handler.js`, `docs/lead-adapter-architecture.md`. Website and WhatsApp adapters are planned, not built.

> The founder is building a next-generation lead engine separately; these docs will be updated when it lands.

## 6. AI Voice

- `services/ai-calling-service` places outbound calls through ElevenLabs' native Exotel integration; server tools pull live CRM data mid-call, and policy answers use the CRM's vector search. Own CFN stack, NoEcho parameters into Secrets Manager.
- `services/followup-agent-service` schedules site-visit confirmation and post-visit feedback calls from `lead.created` / `meeting.completed`, with retries and escalation to humans.
...
- **Property pages** (`apps/property-pages-ms/`): public tenant-branded listing pages; "Schedule a visit" creates a CRM lead + meeting.
- **MCP** (`services/reality-flow-mcp/`): one Streamable HTTP server on Lambda with its own OAuth 2.1 (dynamic client registration + PKCE). 72 tools generated from the CRM registry, proxied to `POST /api/crm/agent/tool`. Agencies can connect Claude/ChatGPT today (`docs/MCP_AGENCY_GUIDE.md`). The service README still says 54 tools.

## 8. Knowledge / RAG

- DynamoDB vector search, tenant id as the required search partition: `property-vector-index` on the CRM table and `knowledge-vector-index` on `KnowledgeChunksTable` (`apps/crm/server/infra/create-vector-index.mjs`).
...
- Agency policies: `apps/crm/server/services/knowledge/*`. This replaced the Bedrock Knowledge Base path; uploaded files in the calling service are stored but not indexed.
...
- **DynamoDB:** 18 tables in `apps/crm/server/infra/cfn-backend.yaml` + 6 in `launch-tables-cfn.yaml`, all with PITR and `DeletionPolicy: Retain`, plus per-service tables (Instagram, follow-up, WhatsApp platform, AI calling).
...
- **Frontends:** CloudFront stacks for the CRM SPA (`apps/crm/real-estate-crm-app/infra/cfn-frontend.yaml`, served at `app.realestateflow.in` with the Instagram console at `/insta`) and the marketing site (`apps/landing-pages/infra/cfn-landing-pages.yaml`, apex `realestateflow.in`).
