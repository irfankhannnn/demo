...
|---|---|---|
| Outbound voice ("re-enable the disabled AI Calling service") | AI calling is **live**, not disabled. Outbound calls go through ElevenLabs' native Exotel integration; the agent pulls CRM data mid-call through server tools. CRM routes are mounted at `/api/internal`; the CRM page is `/crm/ai-calling`. | `agency-app/ai-calling/README.md`; `agency-app/api/server.js` (mounts `aiCallingInternalRoutes`); `agency-app/api/routes/aiCallingInternal.js`; `agency-app/web/src/App.tsx` |
| Call billing | Per started minute, charged once after the call settles, de-duplicated per call session. Pre-call balance check before dialling from the CRM. | `agency-app/api/aiCallBilling.js`; doc 30 |
| Follow-up automation (Step Functions + Chatwoot) | **`agency-app/followup-agent`**: EventBridge schedule tick + event rules, a DynamoDB jobs table and two Lambdas. It schedules AI **calls** for `site_visit_confirmation` (from `lead.created` hints or the CRM button) and `post_visit_feedback` (after `meeting.completed`), with 2 attempts 45 minutes apart inside business hours, then escalation to the assignee and admins. | `agency-app/followup-agent/README.md`, `infra/cfn-followup.yaml`, `src/domain/jobEngine.js`; CRM proxy `agency-app/api/routes/followups.js` |
| Stale-lead follow-up messages | Daily cron (`cron(30 22 * * ? *)` UTC) finds leads with no activity for 1–7 days and asks the agent for a follow-up. Per-tenant mode `draft` (default: saves a note for review) or `autosend` (WhatsApp via Baileys, or email). | `agency-app/api/scripts/lead-followup-cron.js`; `LeadFollowupRule` in `agency-app/api/infra/cfn-backend.yaml` |
| Lead qualification, scoring, assignment | `lead.created` → qualifier agent scores Hot/Warm/Cold against a fixed schema → `lead.qualified` → router agent picks a team member by workload. AI calls can also score a lead (`scoreSource: 'ai_call'`). | `agency-app/api/scripts/lead-qualifier-handler.js`, `scripts/lead-router-handler.js`, `routes/aiCallingInternal.js` |
| Multi-channel lead capture | One ingestion pipeline for every adapter: `POST /api/internal/adapters/leads` → `ingestLead()` (dedupe, create, notify, `lead.created`). Instagram DMs are the first adapter. | `agency-app/api/routes/adapterIngestionInternal.js`, `agency-app/api/leadIngestion.js`; `docs/lead-adapter-architecture.md` |
| Call recording analysis | Agency uploads call recordings; they are transcribed, analysed with Gemini, and turned into proposed CRM actions that a person approves before they run. | `agency-app/api/services/callIntelligence/`; page `agency-app/web/src/pages/crm/CallRecordings.tsx` |
| Agent audit, analytics, credits (June "Phase 2") | All on DynamoDB: agent audit table, credit ledger, team and business analytics. | `agency-app/api/agents/agentAuditService.js`, `creditService.js`, `teamAnalyticsService.js`; page `src/pages/crm/BusinessAnalytics.tsx` |
...
- Customer messaging moves to the **official WhatsApp Business Cloud API** (possibly through AiSensy as the BSP). Plan: `docs/realestateflow-vision/39-whatsapp-official-api-plan.md` (D9). The self-hosted Baileys link (`platform/whatsapp-platform`) stays for the staff command channel.
...
**Today:** outbound calls only. There is no inbound call handler; the agent prompt lives in `agency-app/ai-calling/elevenlabs-agent-prompt.md`.
...
**Today:** projects accept a brochure PDF upload (`POST /api/crm/projects/:projectId/brochure`, `agency-app/api/routes/projects.js`), and properties have documents (`get_property_documents`, `create_property_document` in the tool registry, `agency-app/api/shared/toolDefinitions.js`).
...
| Telegram channel | **Dropped** | No transport code exists. Remove it from customer-facing copy (for example `agency-app/web/src/pages/crm/AIEmployeeStatus.tsx` and the legal pages). |
...
| Strands agents, AgentCore (Gateway, Browser Tool), Bedrock agents, Nova Sonic | Considered, not adopted (D7) | The product runs an in-house pipeline (domain router → planner or bounded tool loop → composer) behind a model gateway, Gemini by default (`agency-app/api/agents/`). Voice is ElevenLabs + Exotel. |
