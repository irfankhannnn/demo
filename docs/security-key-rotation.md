...
**On the fixed OTP — checked, and it is not a bypass by default.** It only applies when `TEST_OTP_ENABLED === 'true'` (`platform/auth/src/models/phoneLinkOtpModel.ts:48-52`, and the Cognito custom-auth trigger at `infra/cfn-backend.yaml:671`). It defaults to `false` in both `infra/deploy.sh:160` and `sample.env:46`, and is surfaced as the `TestOtpEnabled` CloudFormation parameter.
