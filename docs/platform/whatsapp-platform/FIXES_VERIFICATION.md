...
**File 1:** `agency-app/web/src/utils/whatsappConnection.ts` (lines 180-187)
```typescript
if (!res.ok) {
...
**File 2:** `agency-app/api/routes/auth.js` (lines 138-148)
```javascript
...
**File:** `agency-app/web/src/utils/whatsappConnection.ts` (lines 46-75)
```typescript
...
```
✓ agency-app/api/bailey.js — Syntax OK
✓ agency-app/api/routes/auth.js — Syntax OK
```
...
1. `agency-app/web/src/utils/whatsappConnection.ts` — 2 fixes applied
2. `agency-app/api/routes/auth.js` — 1 fix applied
3. `agency-app/api/bailey.js` — No changes needed (already correct)
