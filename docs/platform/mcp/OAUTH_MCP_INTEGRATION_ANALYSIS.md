...
- **Frontend:** `tools/mcp-oauth-debug/patch_mcp_remote.py` patches mcp-remote to also read `x-amzn-remapped-www-authenticate` and case variants
...
```python
# tools/mcp-oauth-debug/patch_mcp_remote.py (lines 16-17)
...
- **Frontend:** `tools/mcp-oauth-debug/patch_mcp_remote.py` patches discovery URL builder to try stage-relative paths first
- **Status:** ✅ Verified — mcp-remote now tries correct stage paths

**Code Changes:**

```python
# tools/mcp-oauth-debug/patch_mcp_remote.py (lines 49-69)
urlsToTry.push({
...
**Solution:**
- **Frontend:** `tools/mcp-oauth-debug/patch_mcp_remote.py` preserves `protectedResourceMetadata.resource` in transport constructors
- **Status:** ✅ Verified — mcp-remote maintains metadata URL across transport recreations

**Code Changes:**

```javascript
// tools/mcp-oauth-debug/patch_mcp_remote.py (lines 82-84)
...
### Test Script: `tools/mcp-oauth-debug/test_oauth_flow.py`
...
### Patch Script: `tools/mcp-oauth-debug/patch_mcp_remote.py`
...
- [ ] **6. Patch mcp-remote**
  ```bash
  python tools/mcp-oauth-debug/patch_mcp_remote.py
  ```

- [ ] **7. Test End-to-End**
  ```bash
  python tools/mcp-oauth-debug/test_oauth_flow.py
  ```
...
**Solution:**
1. Verify OAuth flow completed: `python tools/mcp-oauth-debug/test_oauth_flow.py`
...
**Solution:**
1. Re-run `tools/mcp-oauth-debug/patch_mcp_remote.py` to apply patches
2. Verify patches applied: Check `chunk-65X3S4HB.js` for `protectedResourceMetadata` code
3. Restart mcp-remote: `npm install -g mcp-remote && python tools/mcp-oauth-debug/patch_mcp_remote.py`
...
| File | Lines | Purpose |
|------|-------|---------|
| `tools/mcp-oauth-debug/test_oauth_flow.py` | 133 | End-to-end OAuth + MCP test |
| `tools/mcp-oauth-debug/patch_mcp_remote.py` | 133 | Patches mcp-remote for AWS API Gateway |
