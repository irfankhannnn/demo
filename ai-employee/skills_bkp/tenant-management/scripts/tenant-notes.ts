...
async function main() {
  ...
  if (action === 'list') {
    ...
    await axios.put(`${BASE}/api/crm/customers/${customerId}/notes/${noteId}`, { content }, { headers });
    console.log(`Note [${noteId}] updated.`);

  } else if (action === 'delete') {
    if (!noteId) { console.error('Error: noteId required for delete.'); process.exit(1); }
    await axios.delete(`${BASE}/api/crm/customers/${customerId}/notes/${noteId}`, { headers });
