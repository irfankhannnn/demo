...
function renderSummary(properties: any[], total: number, hasMore: boolean) {
  console.log(`Found ${total} propert${total === 1 ? 'y' : 'ies'}\n`);
...
function renderCompact(properties: any[], total: number, hasMore: boolean) {
  console.log(`${total} propert${total === 1 ? 'y' : 'ies'} found, showing ${properties.length}:\n`);
  properties.forEach((p, i) => {
    const bhk = p.bhk ? `${p.bhk}BHK ` : '';
...
function renderFull(properties: any[], total: number, hasMore: boolean) {
  ...
  properties.forEach((p) => {
    ...
    if (p.carpetArea) console.log(`  Area: ${p.carpetArea} sqft`);
    const rent = p.rentAmount || p.rentalInfo?.expectedRent;
    if (rent) console.log(`  Rent: ₹${rent}/mo`);
    const sale = p.saleInfo?.listedPrice || p.salePrice;
    if (sale) console.log(`  Sale: ₹${sale}`);
...
async function main() {
  ...
  const limit = res.data.limit ?? properties.length;
