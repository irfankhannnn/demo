...
async function main() {
  ...
  if (!raw) { console.error('Usage: owner-notes.ts \'<json with action and ownerId>\''); process.exit(1); }

  const payload = JSON.parse(raw);
  const { action, ownerId, noteId, content, _meta } = payload;

  if (!ownerId) { console.error('Error: ownerId is required.'); process.exit(1); }

  const base = `/api/crm/owners/${ownerId}/notes`;
