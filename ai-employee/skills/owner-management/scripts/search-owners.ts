...
async function main() {
  ...
  if (!raw) { console.error('Usage: search-owners.ts \'<json>\''); process.exit(1); }
  ...
  lastRequestLog = { method: 'GET', url: '/api/crm/owners', params: { search: q, limit: String(limit) } };
  const res = await crmClient.get('/api/crm/owners', { params: { search: q, limit: String(limit) } });
  ...
  switch (responseMode as ResponseMode) {
    case 'compact': renderCompact(owners, total, hasMore, q); break;
    case 'details': renderDetails(owners, total, hasMore, q); break;
    case 'full': renderFull(owners, total, hasMore, q); break;
    case 'summary':
    default: renderSummary(owners, total, hasMore, q);
  }

  logExecution({ skill: SKILL_NAME, script: SCRIPT_NAME, status: 'success', duration_ms: timer.end(), userMessage: _meta?.userMessage, intent: _meta?.intent, request: lastRequestLog, response: { total, returned: owners.length, query: q } });
}
...
main().catch(e => {
  ...
  else { console.error('Error searching owners:', e.response?.data?.error || e.message); }
  process.exit(1);
});