...
async function main() {
  ...
  switch (responseMode as ResponseMode) {
    case 'compact':
      renderCompact(buyers, total, hasMore);
      break;
    case 'details':
      renderDetails(buyers, total, hasMore);
      break;
    case 'full':
      renderFull(buyers, total, hasMore);
      break;
    case 'summary':
    default:
      renderSummary(buyers, total, hasMore);
  }
}

main().catch(e => {
  logApiError(SCRIPT_NAME, lastRequestLog, e.response?.data || e.message);
  console.error('Error fetching buyers:', e.response?.data?.error || e.message);
  process.exit(1);
});
