...
async function main() {
  ...
  if (!raw) {
    console.error('Usage: contact-notes.ts \'<json with action and contactId>\'');
    process.exit(1);
  }

  const { action, contactId, noteId, content } = JSON.parse(raw);
  if (!contactId) { console.error('Error: contactId is required.'); process.exit(1); }

  if (action === 'list') {
    lastRequestLog = { method: 'GET', url: `${BASE}/api/crm/contacts/${contactId}/notes` };
    const res = await axios.get(`${BASE}/api/crm/contacts/${contactId}/notes`, { headers });
    ...
    lastRequestLog = { method: 'POST', url: `${BASE}/api/crm/contacts/${contactId}/notes`, body: { content } };
    const res = await axios.post(`${BASE}/api/crm/contacts/${contactId}/notes`, { content }, { headers });
    logApiCall(SCRIPT_NAME, lastRequestLog, res.data);
    ...
    lastRequestLog = { method: 'PUT', url: `${BASE}/api/crm/contacts/${contactId}/notes/${noteId}`, body: { content } };
    const updRes = await axios.put(`${BASE}/api/crm/contacts/${contactId}/notes/${noteId}`, { content }, { headers });
    ...
    lastRequestLog = { method: 'DELETE', url: `${BASE}/api/crm/contacts/${contactId}/notes/${noteId}` };
    const delRes = await axios.delete(`${BASE}/api/crm/contacts/${contactId}/notes/${noteId}`, { headers });
