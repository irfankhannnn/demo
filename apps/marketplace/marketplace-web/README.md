...
`docs/services/marketplace-api/API-CONTRACT.md`:

| Service | Env var | Local |
|---|---|---|
| marketplace-api (`apps/marketplace/marketplace-api`) | `VITE_MARKETPLACE_API_URL` | `http://localhost:3006` |
| marketplace-authentication (`services/marketplace-authentication`) | `VITE_MARKETPLACE_AUTH_URL` | `http://localhost:3007` |
...
`infra/cicd/marketplace-web/deploy.sh` at the repo root wraps it with numbered
builds and rollback. The public domain is a placeholder — leave the `WEB_*`
domain vars blank to serve from `*.cloudfront.net`.

```bash
./infra/deploy.sh dev
../../../infra/cicd/marketplace-web/deploy.sh dev
```
