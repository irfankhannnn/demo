...
#
# Loads apps/marketplace/marketplace-api/.env.dev or .env.prod (never a plain
...
# Windows Git Bash compatibility (same approach as apps/property-pages-ms/infra/deploy.sh)
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy apps/marketplace/marketplace-api/.env.sample to $ENV_FILE and fill in the values."
  exit 1
fi
