...
// apps/crm/server/agents/modelGateway/index.js for why this is a thin delegation,
// not a rewrite.
...
// Moved here from apps/crm/server/scripts/whatsapp-message-processor.js (Phase 2 Slice 2b)
 ...
 * (e.g. "open the second one"). Archive tools (apps/crm/server/shared/toolDefinitions.js)
 ...
 * apps/crm/server/utils/whatsapp.js buildWhatsAppPrincipal(); a future `web:<userId>`
...
export async function invokeAgent(tenantId, agentId, prompt, context = {}) {
  ...
  try {
    ...
    // was re-keyed to a principal (e.g. `wa:<phone>`) -- see apps/crm/server/utils/whatsapp.js
...
//
// apps/crm/server/scripts/whatsapp-message-processor.js used to do all of this inline
 ...
 * @param {string} params.principal     - e.g. `wa:<phone>`, see apps/crm/server/utils/whatsapp.js
