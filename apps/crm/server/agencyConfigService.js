...
if (!AGENCY_CONFIG_TABLE_NAME) {
  throw new Error('AGENCY_CONFIG_DYNAMODB_TABLE_NAME is not set. Please configure it in apps/crm/server/.env');
}
 ...
 *
 * Queries the connectedWhatsAppPhone-index GSI (apps/crm/server/infra/cfn-backend.yaml)
...
export async function getTenantIdByInstagramWebhookToken(token) {
  ...
  // Queries instagramWebhookToken-index (apps/crm/server/infra/cfn-backend.yaml)
