...
if (!CRM_TABLE_NAME) {
  throw new Error('CRM_DYNAMODB_TABLE_NAME is not set. Please configure it in apps/crm/server/.env');
}
...
export async function createProperty(tenantId, data) {
  ...
  const property = {
    ...
    // Read by apps/crm/server/publicListingService.js — see the allowlist rule there.
 ...
 *
 * Handler for the `match_properties` tool — see apps/crm/server/shared/toolDefinitions.js.
 */
