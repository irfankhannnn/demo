...
#   Loads apps/crm/server/.env.dev or apps/crm/server/.env.prod (never a plain apps/crm/server/.env) and
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy apps/crm/server/.env.sample to $ENV_FILE and fill in the values."
  exit 1
fi
...
# (infra/cicd/server/deploy.sh) to read afterward — it needs these
