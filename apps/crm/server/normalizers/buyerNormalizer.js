/**
 * BuyerNormalizer — strip internal fields and normalize buyer records.
 */
...
export function normalizeBuyer(buyer) {
  ...
  const cleaned = removeInternalFields(buyer);
  return {
    ...cleaned,
    ...
    nextFollowUpDate: normalizeTimestamp(cleaned.nextFollowUpDate),
    hasNotes: Array.isArray(cleaned.notes) && cleaned.notes.length > 0,
  };
}

export function normalizeBuyers(buyers) {
  if (!Array.isArray(buyers)) return [];
  return buyers.map(normalizeBuyer).filter(Boolean);
}

export default { normalizeBuyer, normalizeBuyers };
