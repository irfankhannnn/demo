 ...
 *
 * `services/reality-flow-mcp/src/services/generatedToolDefinitions.ts` is now GENERATED
...
const MCP_TOOLS_PATH = path.resolve(__dirname, '../../../../services/reality-flow-mcp/src/services/generatedToolDefinitions.ts');
...
function main() {
  ...
  console.log(`canonical (apps/crm/server/shared/toolDefinitions.js): ${canonical.size} tools`);
  console.log(`mcp copy  (services/reality-flow-mcp/.../toolDefinitions.ts): ${mcp.size} tools`);
