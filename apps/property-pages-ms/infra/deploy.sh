...
#
# Loads apps/property-pages-ms/.env.dev or .env.prod (never a plain .env) and FORCES
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy apps/property-pages-ms/.env.sample to $ENV_FILE and fill in the values."
  exit 1
fi
