...
# =============================================================================
# RealEstateFlow Landing Pages — Config-Only Deploy
# =============================================================================
# Usage: ./infra/config-deploy.sh <dev|prod>
#
# For a pure CFN-parameter change (custom domain, ACM cert, price class,
# WAF, www alias settings) — skips the build + s3 sync entirely, delegating
# to infra/deploy.sh's own --skip-build mode after this script's safety
# gate passes. Same safety model as services/reality-flow-authentication/infra/
# config-deploy.sh (the reference implementation).
#
# This is the "category 1" axis. For a pure content-only change (the
# {{TOKEN}} substitution build, LP_ENV), see infra/content-deploy.sh
# instead — see docs/proposals/config-only-deploy/context.md for why these
...
echo "============================================="
echo " RealEstateFlow Landing Pages — Config-Only Deploy"
echo "============================================="
