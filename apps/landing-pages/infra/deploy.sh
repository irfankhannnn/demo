...
# =============================================================================
# RealEstateFlow Landing Pages — Deployment Script (S3 + CloudFront)
# =============================================================================
...
#   <dev|prod>-realestateflow-landing-pages-*, matching the naming already
#   live on real-estate-crm-app's frontend stack. dev and prod are separate
#   stacks, separate S3 buckets, separate CloudFront distributions.
#
# This is a static-site deploy, same shape as real-estate-crm-app's:
# build -> deploy the S3+CloudFront stack -> sync dist/ -> invalidate cache.
# The live S3 bucket content IS the deployed artifact; S3 versioning
# (enabled on the bucket in cfn-landing-pages.yaml) is the content-level
# rollback mechanism, backed up by the CI/CD wrapper's build archive
# (infra/cicd/landing-pages/deploy.sh).
# =============================================================================
...
# apps/crm/real-estate-crm-app/infra/deploy.sh — see infra/config-deploy.sh and
...
# (<env>-realestateflow-landing-pages), overridable via FRONTEND_S3_BUCKET_NAME
