...
**Preservation Requirement:** The existing webhook signature verification in `server/bailey.js` (`verifyBaileySignature`) MUST be preserved and remain enforced throughout the refactor. Any new endpoints added must also enforce signature verification. No unauthenticated webhook endpoints may be introduced.

#### 3. server/scripts/whatsapp-message-processor.js
**Current State:**
...
- **CRM-side notification:**
  - `server/routes/auth.js` (modify): Add `POST /webhooks/bailey/fresh-link-required` endpoint
...
- **File:** `server/services/userCategoryService.js` (new)
- **Features:**
...
- **File:** `server/services/featureToggleService.js` (new)
- **Features:**
...
- **File:** `server/services/whatsappAccessControl.js` (new)
...
- **File:** `server/scripts/whatsapp-message-processor.js` (modify)
- **Changes:**
...
- **File:** `server/agents/whatsapp-agent-prompt.js` (new)
- **Features:**
...
- **File:** `server/agents/whatsapp-tool-executor.js` (new)
- **Features:**
...
- **File:** `server/agents/agentRuntime.js` (modify)
- **Changes:**
...
0. **PreKey Exhaustion Recovery** (ROOT CAUSE FIX — do this FIRST)
   ...
   - `server/routes/auth.js`: `POST /webhooks/bailey/fresh-link-required` endpoint
...
5. **No authentication on pairing routes**
   ...
   - **Fix:** Add an `x-api-key` or JWT middleware to all pairing routes. The key should be shared only between `server/bailey.js` and `baileys-service`. Add rate limiting (e.g., max 5 QR requests per phone per minute) and reject requests without the secret.

### A.3 Backend — `server/routes/aiEmployeeConfig.js`
...
7. **No verification that the phone is actually connected**
   ...
   - **Fix:** When `connectedWhatsAppPhone` is updated, call `getConnectionStatus()` from `server/bailey.js` and reject the update if the session is not connected. Return a clear 400 error with the actual connection state.

### A.4 Backend — `server/routes/whatsappConversations.js`
...
### A.5 Backend — `server/whatsappConversationService.js`
...
16. **Duplicate `classifyWhatsAppId` logic**
    - **Problem:** The classification logic for phone/LID/group is duplicated in `server/utils/whatsapp.js` and `real-estate-crm-app/src/components/WhatsAppConversationList.tsx`.
...
### A.7 Frontend — `real-estate-crm-app/src/services/api.ts`
...
### A.8 Frontend — `real-estate-crm-app/src/pages/onboarding/ConnectWhatsApp.tsx`
...
### A.9 Frontend — `real-estate-crm-app/src/pages/crm/CRMDashboard.tsx`
...
### A.10 Frontend — `real-estate-crm-app/src/pages/crm/WhatsAppInbox.tsx`
...
### A.11 Frontend — `real-estate-crm-app/src/components/WhatsAppChatThread.tsx`
...
### A.12 Frontend — `real-estate-crm-app/src/pages/crm/AiEmployee.tsx`
...
29. **No unit tests for phone number utilities**
    - **Problem:** `server/utils/whatsapp.js` contains critical normalization and classification logic that is currently untested.
    ...
    - **Fix:** Add unit tests in `server/utils/whatsapp.test.js` covering E.164, JID, LID, group IDs, device suffixes, international numbers, and malformed inputs. Add equivalent tests for the frontend helper if it remains.
...
3. **Revert or version the `/api/whatsapp` endpoint path change** in `api.ts` and `server/routes/whatsappConversations.js`.
4. **Fix silent API failures** in `ConnectWhatsApp.tsx`.
5. **Add unit tests** for `server/utils/whatsapp.js`.
