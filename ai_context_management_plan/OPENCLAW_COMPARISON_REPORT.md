...
1. **Implement Session Key Construction** (`server/services/sessionKeyService.js`) - enables flexible DM collapsing and isolation
2. **Add Group Chat Support with Isolation** - remove group blocking, add group context
3. **Implement Agent Route Resolution** (`server/services/agentRouteService.js`) - enables multi-agent routing
4. **Enrich Inbound Context** - add BodyForAgent, CommandBody, ReplyTo context, Sender metadata
5. **Add Echo Detection** (`server/utils/echoDetection.js`) - prevents agent responding to itself
...
1. **Implement Agent Routing Layer** (`server/agents/agentRouter.js`) - channel/intent/category-based routing
...
2. **Standardize on TypeScript** - migrate `server/` from JS to TS with strict mode
...
1. **Use Structured Output for the WhatsApp Agent** (High Priority)
   ...
   - **Files:** `server/agents/agentRuntime.js`, `server/agents/prompts.js`
...
2. **Enforce "One Message Per Turn" in the Reply Pipeline** (High Priority)
   ...
   - **Files:** `server/messaging/replyPipeline.js`, `server/scripts/whatsapp-message-processor.js`
...
3. **Clean the Inbound Body Before Sending to the LLM** (High Priority)
   ...
   - **Files:** `server/scripts/whatsapp-message-processor.js`, `server/utils/whatsapp.js`
...
4. **Improve Context Depth and Add Summarization** (Medium Priority)
   ...
   - **Files:** `server/whatsappConversationService.js`, `server/conversationStateService.js`
...
5. **Add Echo Detection** (High Priority)
   ...
   - **Files:** `server/utils/echoDetection.js`, `server/scripts/whatsapp-message-processor.js`
...
7. **Add Output Validation Before Sending** (Medium Priority)
   ...
   - **Files:** `server/messaging/replyValidator.js`, `server/scripts/whatsapp-message-processor.js`
...
8. **Inject Conversation State into System Prompt** (Medium Priority)
   ...
   - **Files:** `server/agents/agentRuntime.js`, `server/conversationStateService.js`
...
9. **Separate Reasoning from Output (for models that support it)** (Low Priority)
   ...
   - **Files:** `server/agents/agentRuntime.js`
...
10. **Add A/B Testing for Prompt Variants** (Low Priority)
    ...
    - **Files:** `server/agents/prompts.js`, `server/agents/agentAuditService.js`
...
| 26 | Standardize on TypeScript across server | Medium | 2-3 weeks | `server/` migration |
...
**Evidence:**
- `server/agents/agentRuntime.js:279-327` (Bedrock loop) and `:246-274` (Gemini loop) accept any `text` from the model and then run a heuristic cleanup.
- `server/agents/prompts.js:81-113` tells the WhatsApp agent to "Reply with ONLY the final message text" but provides no JSON schema or structured output contract.
- `server/agents/agentRuntime.js:93` sets `max_tokens: 1024`, leaving room for rambling, reasoning, and multiple sentences.
...
**Evidence:**
- `server/agents/agentRuntime.js:120-231` contains `scoreResponseCandidate()` and `sanitizeAgentReply()`.
...
**Evidence:**
- `server/agents/prompts.js:186-202` defines `buildSystemPromptWithContext()`, which loads business context and team members from `.devin/ai-employee/tenant-templates/`.
- `server/agents/agentRuntime.js:10` imports `buildSystemPrompt`, **not** `buildSystemPromptWithContext`.
- `server/agents/agentRuntime.js:393` calls `buildSystemPrompt(agentId, tenantId, personality)` — the tenant context is never injected.
...
**Evidence:**
- `server/agents/prompts.js:87-94` and `:139-150` contain 15+ "NEVER" instructions.
...
**Evidence:**
- `server/scripts/whatsapp-message-processor.js:164` passes raw `text` to `invokeAgent`.
...
**Evidence:**
- `server/scripts/whatsapp-message-processor.js:164` hardcodes `agentId = 'whatsapp'`.
- `server/agents/prompts.js:50-80` defines specialized agents (`qualifier`, `router`, `followup`, `mcp`), but they are only used in separate Lambda handlers, not the main WhatsApp flow.
- `server/skillInvoker.js:175` exposes all 24 tools to every agent via `ALLOWED_TOOLS`.
- `server/agents/agentRuntime.js:39-60` builds the same tool definitions for every agent.
...
**Evidence:**
- `server/agents/agentRuntime.js:254` (Gemini) and `:297` (Bedrock) only process the **first** tool call per turn.
- `server/agents/agentRuntime.js:268-273` and `:314-321` feed raw tool results back to the LLM without checking `toolResult.ok`.
- `server/agents/agentRuntime.js:21` hardcodes `MAX_TOOL_TURNS = 5`.
...
**Evidence:**
- `server/conversationStateService.js:29-64` defines a rich state object with `intent`, `topic`, `status`, `messageCount`, and `context`.
- `server/agents/agentRuntime.js:392-409` loads only the last 5 messages; it never imports or injects `getConversationState`.
- `server/whatsappConversationService.js:374-381` maps roles using the unreliable `fromMe` boolean.
...
**Evidence:**
- `server/scripts/whatsapp-message-processor.js:58-74` has no echo detection and no Lambda-level idempotency.
- `server/routes/webhooks.js:126-129` deduplicates at the webhook layer but does not propagate to EventBridge or the Lambda.
- `server/scripts/whatsapp-message-processor.js:200-228` sends a reply without checking whether a reply was already sent for this inbound `messageId`.
- `server/infra/cfn-backend.yaml:2308-2322` has no EventBridge `RetryPolicy` or DeadLetterQueue (default = 185 retries over 24 hours).
...
**Evidence:**
- `server/scripts/whatsapp-message-processor.js:9-34` parses only two commands (`lead:...` and `search leads...`).
- If parsing fails, the raw command string is still passed to the AI.
- `server/whatsappAccessControl.js:43-74` checks `canReceiveMessage()` and `canAutoReply()`, but there is no `canExecuteCommands()` or `CommandAuthorized` flag.
- `server/scripts/whatsapp-message-processor.js:140` passes `userId: 'whatsapp'` as a hardcoded placeholder.
