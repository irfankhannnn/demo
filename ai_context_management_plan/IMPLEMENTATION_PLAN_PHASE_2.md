...
**Changes to `server/infra/cfn-backend.yaml`:**
...
### Key Files to Modify
- `server/agents/prompts.js` - Personality injection
- `server/agents/agentRuntime.js` - Context loading
- `server/whatsappConversationService.js` - Conversation history
- `server/skillInvoker.js` - Tool enhancement
- `server/routes/aiEmployeeConfig.js` - Configuration
...
### New Files to Create
- `server/conversationStateService.js` - State management
- `.devin/ai-employee/` - Knowledge base directory
- `server/infra/cfn-conversation-state.yaml` - CloudFormation
