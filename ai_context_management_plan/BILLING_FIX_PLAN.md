...
**Also add to `server/subscriptionService.js` (append after `setBillingAnniversaryDay`):**
```javascript
/**
...
**Add to `server/subscriptionService.js` `createTrialSubscription` item (line 114-128):**
```javascript
// Add gracePeriodEndsAt field to schema (initially null)
gracePeriodEndsAt: null,
```

**Add env var to `server/.env.example`:**
```bash
...
**File:** `server/scripts/grace-period-expiry-cron.js` (NEW FILE)  
...
# Run cron
node server/scripts/grace-period-expiry-cron.js
...
**File:** `server/scripts/credit-reset-cron.js`  
**Lines:** 93-101  
...
node server/scripts/credit-reset-cron.js &
node server/scripts/credit-reset-cron.js &
wait
...
- `server/routes/crm.js` (search for `refundCredits`)
- `server/routes/khata.js` (if applicable)
...
**File:** `server/scripts/credit-balance-reconcile.js` (NEW FILE)  
...
# Run reconciliation
node server/scripts/credit-balance-reconcile.js
...
**File:** `server/routes/creditAdmin.js`  
**Lines:** 24-42  
...
**Apply to credit-charging routes in `server/routes/leads.js`:**
```javascript
...
**Files:**
- `real-estate-crm-app/src/pages/crm/BillingSettings.tsx` — Add "Transaction History" tab
- `real-estate-crm-app/src/services/api.ts` — Add `getCreditLedger()` method
...
**File:** `real-estate-crm-app/src/contexts/SubscriptionContext.tsx`  
...
**File:** `real-estate-crm-app/src/components/PaywallModal.tsx`  
...
**File:** `server/subscriptionService.js`  
...
**File:** `server/infra/launch-tables-cfn.yaml` (or wherever Subscriptions table is defined)  
...
**Then update `server/scripts/trial-reminder-cron.js` to use GSI:**
```javascript
...
**File:** `server/infra/launch-tables-cfn.yaml`  
...
### Post-Deployment
1. Run credit balance reconciliation: `node server/scripts/credit-balance-reconcile.js`
...
4. Run grace period expiry cron manually first time: `node server/scripts/grace-period-expiry-cron.js`
...
| File | Fixes | Phase |
|------|-------|-------|
| `server/routes/billing.js` | 1.1, 1.2, 1.3, 1.8, 2.2 | 1, 2 |
| `server/subscriptionService.js` | 1.2, 1.3, 3.4 | 1, 3 |
| `server/scripts/credit-reset-cron.js` | 1.5, 2.6 | 1, 2 |
| `server/scripts/grace-period-expiry-cron.js` | 1.4 (NEW) | 1 |
| `server/scripts/credit-balance-reconcile.js` | 1.9 (NEW) | 1 |
| `server/routes/leads.js` | 1.6 | 1 |
| `server/bailey.js` | 1.7 | 1 |
| `server/server.js` | 2.1 | 2 |
| `server/routes/subscriptions.js` | 2.3 | 2 |
| `server/routes/creditAdmin.js` | 2.4 | 2 |
| `server/middleware/rateLimiter.js` | 2.5, 2.7 | 2 |
| `server/logger.js` | 2.8 | 2 |
| `server/creditConfig.js` | 3.5 | 3 |
| `server/infra/cfn-backend.yaml` | 4.1 | 4 |
| `server/infra/launch-tables-cfn.yaml` | 4.2, 4.3 | 4 |
| Frontend components | 3.1, 3.2, 3.3 | 3 |
| `server/.env.example` | All env vars | All |
