...
**Files:**
- `reality-flow-authentication/src/utils/cognito.ts`
- `reality-flow-authentication/src/middleware/requireAuth.ts` (NEW)
- `reality-flow-authentication/src/app.ts`
...
**File:** `reality-flow-authentication/src/middleware/authMiddleware.ts`
...
**Files:**
- `reality-flow-authentication/src/controllers/tokenController.ts`
- `real-estate-crm-app/src/utils/authStorage.ts`
- `real-estate-crm-app/src/services/api.ts`
...
- `server/routes/buyers.js`, `server/routes/khata.js`, `server/routes/enquiries.js`, `server/routes/notifications.js`

**Actions:**
1. `cd server && npm install zod`
2. Create `server/validation/crmSchemas.js` with Zod schemas for each entity.
...
**File:** `server/routes/b2bLeads.js:168-191`
...
**File:** `server/routes/auth.js:16-21`
...
**File:** `server/server.js` (or new middleware)
...
**Files:**
- `reality-flow-authentication/src/controllers/tokenController.ts`
- `reality-flow-authentication/src/controllers/phoneAuthCustomController.ts`
...
**File:** `server/routes/grievance.js:162`
...
**Files:** `server/routes/khata.js`, `server/routes/notifications.js`, `server/routes/enquiries.js`
...
**File:** `real-estate-crm-app/src/lib/analytics.ts`
...
**File:** `server/routes/grievance.js`
...
| # | File | Action |
|---|------|--------|
| LOW-1 | `server/subscriptionService.js:78` | Make trial days configurable via env var |
| LOW-2 | `server/package.json` | `npm uninstall bcryptjs jsonwebtoken` |
| LOW-3 | `real-estate-crm-app/package.json` | `npm uninstall @types/react-router-dom` |
| LOW-4 | `reality-flow-authentication/package.json` | Move `dotenv` from `devDependencies` to `dependencies` |
...
**Files:** `server/middleware/auth.js`, all imports

**Action:** Delete `server/middleware/auth.js`. Replace all `authenticateToken` imports/usages with `validateToken`.
...
2. **Stage 2 (CRM Backend):** Deploy `server/` fixes (CRIT-2, CRIT-7, CRIT-8, CRIT-9, HIGH fixes).
