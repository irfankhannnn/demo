...
2. Create `server/agents/prompts.js` → compose system prompts
3. Refactor `agentRuntime.js` → add tenant opt-in checks, Bedrock loop, error handling

**Files Modified/Created:**
- `server/agents/agentAuditService.js`
- `server/agents/prompts.js` (NEW)
- `server/agents/agentRuntime.js`
...
**Tasks:**
1. Update `server/routes/leads.js` → publish `lead.created` event
...
- `server/scripts/lead-qualifier-handler.js`
- `server/scripts/lead-router-handler.js`
- `server/scripts/lead-followup-cron.js`
...
**Tasks:**
1. Create `server/routes/agentTools.js` → `/api/crm/agent/tool` endpoint with JWT validation
2. Update `server/mcp-server/index.js` → sign JWT, call endpoint
3. Update `.mcp.json` → register `nabi-crm` server
4. **CRITICAL:** Mount routes in `server/server.js`

**Files Modified/Created:**
- `server/routes/agentTools.js` (NEW)
- `server/mcp-server/index.js`
- `.mcp.json`
- `server/server.js` (add route mounts)
...
**Tasks:**
1. Update `server/routes/billing.js` → on `subscription.activated` with `plan_ai_employee_monthly`, set `aiEmployeeEnabled=true`
...
**Files Modified/Created:**
- `real-estate-crm-app/src/components/AgentActivityLog.tsx` (NEW)
- `server/routes/admin.js` (add agent-activity + ai-employee/config endpoints)
- `real-estate-crm-app/src/pages/crm/BillingSettings.tsx`
...
✅ **Storage (MVP):**
- Stored in `server/.env` as `JWT_SECRET`
