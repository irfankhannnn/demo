...
4. Update `server/bailey.js` to send the API key in all outgoing requests to the Bailey service (`getPairingQr`, `sendWhatsAppMessage`, `disconnectWhatsApp`, `getConnectionStatus`).
...
- `server/bailey.js` always sends the API key.
...
### P0.5 — Add Unit Tests for `server/utils/whatsapp.js`

**Files:**
- `server/utils/whatsapp.js`
- `server/utils/whatsapp.test.js` (new)

**Sub-tasks:**
1. Create `server/utils/whatsapp.test.js` using the project's test framework (Jest/Vitest).
...
**Files:**
- `server/routes/aiEmployeeConfig.js`
...
- `baileys-service/src/config.js`
- `server/routes/auth.js` (or new webhook route)
- `server/services/notificationService.js` (modify if exists)
...
3. Add `POST /webhooks/bailey/fresh-link-required` in `server/routes/auth.js` (or a dedicated webhook route) that:
...
**Files:**
- `server/services/userCategoryService.js` (new)
- `server/utils/whatsapp.js`
...
2. Integrate with `server/utils/whatsapp.js` for normalization.
...
**Files:**
- `server/services/featureToggleService.js` (new)
...
**Files:**
- `server/services/whatsappAccessControl.js` (new)
- `server/services/userCategoryService.js`
- `server/services/featureToggleService.js`
...
**Files:**
- `server/agents/whatsapp-agent-prompt.js` (new)
...
**Files:**
- `server/agents/whatsapp-tool-executor.js` (new)
- `server/services/apiClient.js` (new or modify existing)
...
**Files:**
- `server/services/__tests__/` (create/update tests)
- `server/agents/__tests__/` (create/update tests)
