...
import { User, Edit, Save, X, Download } from 'lucide-react';
...
export default function OwnerTab({ flat, onUpdate }: OwnerTabProps) {
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState({
    name: flat.owner?.name || '',
    email: flat.owner?.email || '',
    phone: flat.owner?.phone || '',
    address: flat.owner?.address || '',
  ...
  const handleSave = async () => {
    try {
      await api.saveOwner(flat.flatId, {
        name: form.name,
        email: form.email,
        phone: form.phone,
      ...
      alert(error instanceof Error ? error.message : 'Failed to save owner');
    }
  };
    ...
    <div className="p-6">
      ...
      {editing ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-medium">{flat.owner.name}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Email</p>
              <p className="font-medium">{flat.owner.email}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Phone</p>
              <p className="font-medium">{flat.owner.phone}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Address</p>
              <p className="font-medium">{flat.owner.address || 'N/A'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            {flat.owner.photoUrl && (
              <div>
                ...
                <img
                  src={flat.owner.photoUrl}
                  alt="Owner"
                  className="w-32 h-32 object-cover rounded-lg border cursor-pointer"
                  onClick={() => openPreview(flat.owner!.photoUrl!, 'Owner photo')}
                />
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  <button
                    type="button"
                    className="text-sm text-blue-600 hover:text-blue-700 underline"
                    onClick={() => openPreview(flat.owner!.panUrl!, 'Owner PAN')}
                  >
                    View PAN
                  </button>
                  <a
                    href={flat.owner.panUrl}
                    download
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  <button
                    type="button"
                    className="text-sm text-blue-600 hover:text-blue-700 underline"
                    onClick={() => openPreview(flat.owner!.aadharUrl!, 'Owner Aadhar')}
                  >
                    View Aadhar
                  </button>
                  <a
                    href={flat.owner.aadharUrl}
                    download
                    className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100"
                  >
                    <Download className="w-4 h-4" />
        ...
        <div className="text-center py-12 text-gray-500">
          ...
          <p>No owner information added yet</p>
        </div>
      )}
