// Admin invite + member management flow.
...
export async function runAdminUiFlow(page: Page, ctx: EvidenceCtx): Promise<void> {
  ...
  await test.step('Admin: login with canonical phone+OTP flow', async () => {
    await loginWithPhoneOtp(page, ctx, {
      phoneNumber: '8291537522',
      otp: '123456',
      snapPrefix: 'admin-login',
  ...
  await test.step('Admin: check current member count', async () => {
    ...
    if (memberCount !== null) {
      log('Admin', 'PASS', `Current member count: ${memberCount}`);
    } else {
      log('Admin', 'INFO', `Could not parse member count from text: ${countText || 'n/a'}`);
    }
  ...
  await test.step('Admin: return to members and note final count', async () => {
    ...
    if (finalMatch) {
      ...
      log('Admin', 'INFO', `Could not parse final member count from text: ${finalCountText || 'n/a'}`);
    }
  });
}
