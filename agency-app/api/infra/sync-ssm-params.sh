...
# deleted.
#
# Reads agency-app/api/infra/cfn-params.json (already generated fresh by deploy.sh
# from .env.<env> — must exist and be current before calling this) and
# agency-app/api/infra/ssm-param-map.txt (ENV_VAR_NAME=CfnParameterName, checked
...
# point of view (agency-app/api/config/ssmBootstrap.js only sets process.env.X for
