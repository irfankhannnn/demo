...
#   Loads agency-app/api/.env.dev or agency-app/api/.env.prod (never a plain agency-app/api/.env) and
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy agency-app/api/.env.sample to $ENV_FILE and fill in the values."
  exit 1
fi
...
# (infra/cicd/agency-app/api/deploy.sh) to read afterward — it needs these
