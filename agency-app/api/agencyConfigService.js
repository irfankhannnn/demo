...
// Load environment variables from agency-app/api/.env
...
if (!AGENCY_CONFIG_TABLE_NAME) {
  throw new Error('AGENCY_CONFIG_DYNAMODB_TABLE_NAME is not set. Please configure it in agency-app/api/.env');
}
 ...
 *
 * Queries the connectedWhatsAppPhone-index GSI (agency-app/api/infra/cfn-backend.yaml)
...
export async function getTenantIdByInstagramWebhookToken(token) {
  ...
  // Queries instagramWebhookToken-index (agency-app/api/infra/cfn-backend.yaml)
