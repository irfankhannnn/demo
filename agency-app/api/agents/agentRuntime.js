...
// agency-app/api/agents/modelGateway/index.js for why this is a thin delegation,
// not a rewrite.
...
// Moved here from agency-app/api/scripts/whatsapp-message-processor.js (Phase 2 Slice 2b)
 ...
 * (e.g. "open the second one"). Archive tools (agency-app/api/shared/toolDefinitions.js)
 ...
 * agency-app/api/utils/whatsapp.js buildWhatsAppPrincipal(); a future `web:<userId>`
...
export async function invokeAgent(tenantId, agentId, prompt, context = {}) {
  ...
  try {
    ...
    // was re-keyed to a principal (e.g. `wa:<phone>`) -- see agency-app/api/utils/whatsapp.js
...
//
// agency-app/api/scripts/whatsapp-message-processor.js used to do all of this inline
 ...
 * @param {string} params.principal     - e.g. `wa:<phone>`, see agency-app/api/utils/whatsapp.js
