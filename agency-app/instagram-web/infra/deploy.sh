...
# agency-app/web/infra/deploy.sh: a pure CFN parameter change
# (CrmDistributionId) never needs npm ci/vite build/s3 sync, and a pure
# VITE_*-only change never needs a CFN update. infra/config-deploy.sh and
# infra/content-deploy.sh are the gated entry points for each.
SKIP_BUILD=false
SKIP_CFN=false
POSITIONAL_ARGS=()
for arg in "$@"; do
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy agency-app/instagram-web/.env.sample to $ENV_FILE and fill in the values."
  exit 1
fi
...
echo "Insta API:     https://$VITE_INSTA_API_DOMAIN_NAME/$VITE_INSTA_API_BASE_PATH"
...
if [ "$SKIP_BUILD" = true ]; then
  echo "[1/5] Skipping npm install (--skip-build)"
  echo "[2/5] Skipping vite build (--skip-build)"
else
...
# -----------------------------------------------------------------------------
echo "[3/5] Generating infra/cfn-params.json..."
...
if [ "$SKIP_CFN" = true ]; then
  echo "      Skipping CloudFormation deploy (--skip-cfn)"
else
  ...
  if [ -z "${CRM_DISTRIBUTION_ID:-}" ]; then
    ...
    echo "      docs/agency-app/instagram/09-CLOUDFRONT-INTEGRATION.md"
    echo ""
  fi
fi
...
# -----------------------------------------------------------------------------
if [ "$SKIP_BUILD" = true ]; then
  echo "[4/5] Skipping dist/ sync (--skip-build) — no content changed."
else
...
# -----------------------------------------------------------------------------
# 4. Invalidate — only meaningful when content actually changed.
# -----------------------------------------------------------------------------
if [ "$SKIP_BUILD" = true ]; then
  echo "[5/5] Skipping CloudFront invalidation (--skip-build, no content changed)"
elif [ -n "${CRM_DISTRIBUTION_ID:-}" ]; then
