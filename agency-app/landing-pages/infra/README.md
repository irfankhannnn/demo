...
`agency-app/web/infra/README.md` for the sibling implementation this
one is modeled on.
...
`infra/cicd/agency-app/landing-pages/deploy.sh <dev|prod>` — it delegates to
this script and then records the build.

## Why this isn't just a copy of `agency-app/web/infra/cfn-frontend.yaml`
...
- Routing is a `AWS::CloudFront::Function` (`UrlRewriteFunction` in
  `cfn-landing-pages.yaml`) that reimplements `agency-app/landing-pages/netlify.toml`'s
