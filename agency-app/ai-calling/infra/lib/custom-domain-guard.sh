# =============================================================================
# Custom-domain guard — ai-calling-service
# =============================================================================
...
validate_custom_domain_vars() {
  ...
  for var in CRM_INTERNAL_API_DOMAIN_NAME AI_CALLING_API_DOMAIN_NAME; do
    value="${!var:-}"
    if [ -z "$value" ]; then
  ...
  for var in CRM_INTERNAL_API_BASE_PATH AI_CALLING_API_BASE_PATH; do
    if [ -z "${!var:-}" ]; then
      echo "ERROR: $var is not set"; failed=1
    fi
  done
  if [ "${ENABLE_CUSTOM_DOMAIN_MAPPING:-false}" != "true" ]; then
    echo "ERROR: ENABLE_CUSTOM_DOMAIN_MAPPING must be true — WEBHOOK_BASE_URL is derived from the custom domain."; failed=1
  fi
