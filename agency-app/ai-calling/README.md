...
consumers dedupe on `callSessionId`. Shape: `docs/agency-app/followup-agent/CONTRACTS.md`
...
pattern in `agency-app/api/config/ssmBootstrap.js`. `lambda-handler.js` defers importing
...
Infrastructure lives in `agency-app/ai-calling/infra/cfn-ai-calling.yaml` and is
deployed through the CI/CD wrapper, not by invoking `aws cloudformation deploy`
by hand:

```bash
infra/cicd/agency-app/ai-calling/deploy.sh dev
infra/cicd/agency-app/ai-calling/deploy.sh prod
```
...
- **File upload for the knowledge base is not implemented.**
  ...
  the CRM under Agency Policies and indexed by `agency-app/api/services/knowledge/`.
