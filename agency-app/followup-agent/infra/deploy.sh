...
#
# Loads agency-app/followup-agent/.env.dev or .env.prod (never a plain .env) and
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy agency-app/followup-agent/.env.example to $ENV_FILE and fill in the values."
  exit 1
fi
...
Where each comes from:
  CRM_INTERNAL_API_KEY       must equal agency-app/api/.env.${DEPLOY_ENV}'s FOLLOWUP_INTERNAL_API_KEY
                             (the CRM's FollowupInternalApiKey parameter)
  AI_CALLING_CALLER_API_KEY  must equal agency-app/ai-calling/.env.${DEPLOY_ENV}'s CRM_CALLER_API_KEY
  CRM_CALLER_API_KEY         generate one, e.g. \`openssl rand -hex 32\`, and set the
                             same value as FOLLOWUP_CALLER_API_KEY in agency-app/api/.env.${DEPLOY_ENV}
