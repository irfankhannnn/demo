...
#
# Loads agency-app/instagram-api/.env.dev or .env.prod (never a plain .env) and
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy agency-app/instagram-api/.env.sample to $ENV_FILE and fill in the values."
  exit 1
fi
