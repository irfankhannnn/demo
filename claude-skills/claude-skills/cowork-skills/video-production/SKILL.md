...
```bash
mkdir -p my-video/src/compositions/ads
cat > my-video/src/compositions/ads/AdBanner.tsx << 'EOF'
...
```bash
cat > my-video/src/compositions/ads/ProductDemo.tsx << 'EOF'
...
```bash
cat > my-video/src/compositions/ads/InstagramReel.tsx << 'EOF'
...
```bash
cat > my-video/src/compositions/ads/Tutorial.tsx << 'EOF'
import { Composition, Sequence, useVideoConfig } from "remotion";
