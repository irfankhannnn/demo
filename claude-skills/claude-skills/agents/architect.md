...
- **Auth:** JWT middleware in `server/middleware/auth.js`
- **Deployment:** AWS Lambda + API Gateway + CloudFormation

### Key Patterns
- Routes in `server/routes/<resource>.js`
- Services in `server/<resource>DynamodbService.js`
- Frontend types in `real-estate-crm-app/src/types/`
- Frontend API services in `real-estate-crm-app/src/services/`
- Pages in `real-estate-crm-app/src/pages/crm/`
