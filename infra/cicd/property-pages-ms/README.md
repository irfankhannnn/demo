# infra/cicd/property-pages-ms

CI/CD wrapper for the public property-pages microservice (`apps/property-pages-ms/`
...
`apps/property-pages-ms/infra/deploy.sh` does the real work: test, install,
...
- `apps/property-pages-ms/.env.dev` and/or `.env.prod`, copied from
  `apps/property-pages-ms/.env.sample` and filled in — in particular
...
```bash
cd infra/cicd/property-pages-ms
...
Reads `apps/property-pages-ms/.env.<env>`. There is no plain `.env` fallback —
...
- **"env file not found"** — copy `apps/property-pages-ms/.env.sample` to
  `.env.dev` or `.env.prod` in `apps/property-pages-ms/` and fill in the
