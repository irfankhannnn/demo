...
Deploys `apps/crm/server/infra/launch-tables-cfn.yaml` — six standalone DynamoDB
...
| `Subscriptions` | tenant plan, seats, trial/billing status — read/written by `apps/crm/server/subscriptionService.js` + `apps/crm/server/routes/subscriptions.js`, and read-only by `reality-flow-authentication`'s invite flow (seat-limit check) |
| `NPSResponses` | Net Promoter Score survey responses |
| `BetaInvites` | beta program invite list |

`AIEmployeeProvisioning` is deliberately **not** here — `apps/crm/server/infra/cfn-backend.yaml`
...
it. `apps/crm/server/.env.launch-tables.dev` / `.env.launch-tables.prod` must exist
first (copy from the `.sample` file in `apps/crm/server/`) — these are separate from
`apps/crm/server/.env.dev` / `.env.prod` (which are for `cfn-backend.yaml`) because
