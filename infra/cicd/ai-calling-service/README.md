# infra/cicd/ai-calling-service

CI/CD wrapper for the AI calling microservice (`services/ai-calling-service/`). Same
...
`services/ai-calling-service/infra/deploy.sh` does the real work: install, package,
...
Reads `services/ai-calling-service/.env.<env>` (gitignored). Both `.env.dev` and
...
one until they are supplied. See `services/ai-calling-service/.env.example` for where
...
`services/ai-calling-service/infra/cfn-params.json` itself.
