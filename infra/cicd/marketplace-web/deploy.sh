...
# =============================================================================
# RealEstateFlow Homes (marketplace-web) — CI/CD entry point, with build/release tracking
# =============================================================================
# Usage:
...
# apps/marketplace/marketplace-web/infra/deploy.sh. This wrapper's only job is release
...
# frontend S3 bucket itself (versioned, per cfn-marketplace-web.yaml) IS the
...
#   ${SERVICE_NAME}/builds/<build>/<env>/cfn-marketplace-web.yaml  scoped archive
#                                                            (never overwritten)
#
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../apps/marketplace/marketplace-web" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
VERSIONS_DIR="$SCRIPT_DIR/deploy-versions"
CONFIG_VERSIONS_DIR="$SCRIPT_DIR/config-versions"

# validate_web_env_vars — shared with the infra/ scripts so the rules
# (https API URLs, no trailing slash, no raw execute-api on prod) can't drift.
# shellcheck disable=SC1091
source "$SERVICE_DIR/infra/lib/env-guard.sh"
  ...
  node -e "
    ...
    const manifest = {
      ...
      artifact: {
        ...
        // the live web bucket (below) IS the deployed content, and its
...
cmd_deploy() {
  ...
  cp "$SERVICE_DIR/infra/cfn-marketplace-web.yaml" "$build_dir/cfn-marketplace-web.yaml" 2>/dev/null || true
  ...
  local frontend_bucket="${WEB_S3_BUCKET_NAME:-${ENV}-${SERVICE_NAME}}"
  local build_prefix="${SERVICE_NAME}/builds/${build}/${env}"
  local build_dist_key="${build_prefix}/dist.tar.gz"
  local build_template_key="${build_prefix}/cfn-marketplace-web.yaml"
  ...
  if [ "$status" = "deployed" ]; then
    ...
    "$AWS_BIN" s3 cp "$build_dir/cfn-marketplace-web.yaml" "s3://${artifact_bucket}/${build_template_key}" --region "$AWS_REGION" --no-cli-pager
...
cmd_rollback_config() {
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$SERVICE_DIR/infra/cfn-marketplace-web.yaml" \
    --stack-name "$stack_name" \
...
EOF
  ...
  cp "$build_dir/cfn-marketplace-web.yaml" "$new_dir/" 2>/dev/null || true
...
cmd_rollback_full() {
  ...
  verify_build_env "$build" "$env" "$m"
  if [ ! -f "$build_dir/cfn-marketplace-web.yaml" ] || [ ! -f "$build_dir/cfn-params.json" ]; then
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$build_dir/cfn-marketplace-web.yaml" \
