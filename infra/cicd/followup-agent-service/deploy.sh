...
# =============================================================================
# Follow-up Agent Service - CI/CD entry point, with build/release tracking
# =============================================================================
# Usage:
...
# services/followup-agent-service/infra/deploy.sh. This wrapper's only job is release
...
#   <prefix>/builds/<build>/<env>/cfn-followup.yaml   permanent, never
...
#
# NOTE: cfn-params.json carries real secret values (CRM / ai-calling shared keys)
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../services/followup-agent-service" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
VERSIONS_DIR="$SCRIPT_DIR/deploy-versions"
CONFIG_VERSIONS_DIR="$SCRIPT_DIR/config-versions"
SERVICE_NAME="realestateflow-followup"
TEMPLATE_NAME="cfn-followup.yaml"
...
cmd_deploy() {
  ...
  echo "============================================="
  echo " Follow-up Agent Service - build $build -> $env"
  echo "============================================="
...
cmd_config_deploy() {
  ...
  echo "============================================="
  echo " Follow-up Agent Service - config-only deploy -> $env"
  echo "============================================="
  echo "Branch:  $GIT_BRANCH ($GIT_COMMIT_SHORT, dirty=$GIT_DIRTY)"
  echo ""
...
cmd_rollback_config() {
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$SERVICE_DIR/infra/$TEMPLATE_NAME" \
    --stack-name "$STACK_NAME" \
    --capabilities CAPABILITY_NAMED_IAM \
    --no-fail-on-empty-changeset \
    --tags Environment="$env" Service="$SERVICE_NAME" \
...
verify_build_env() {
  ...
  if [ "$recorded_env" != "$env" ]; then
    ...
    echo "       point it at the wrong tables and the wrong CRM."
    echo "       Refusing."
    exit 1
  fi
}
...
EOF
  ...
  # Both functions share one zip (different Handler:), so both must move
  # together or the API and the worker would run different code.
  for fn in "${env}-realestateflow-followup-api-lambda" "${env}-realestateflow-followup-worker-lambda"; do
