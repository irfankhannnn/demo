...
security hardening done in `agency-app/api/infra`. Deleted rather than re-synced, so
there's only one copy to ever go stale again (same reasoning as
`infra/cicd/platform/auth`).

## Running it

```
cd infra/cicd/agency-app/api
...
`infra/cicd/platform/auth`'s, because the underlying
...
- ~100 of this template's ~120 CFN parameters are consumed by the app at
  ...
  (`agency-app/api/config/ssmBootstrap.js`), not via CFN `Environment.Variables` — a
  ...
  it only ever needed `agency-app/api/infra/sync-ssm-params.sh` (already existed,
...
Same as `infra/cicd/platform/auth`: one counter across
...
`infra/cicd/platform/auth` (a rollback is always a new,
...
1. **Code artifact key.** Auth uploads to a *fixed* S3 key
   ...
   needed. That timestamp is generated inside `agency-app/api/infra/deploy.sh` at a
...
2. **Two Lambdas share one code artifact** — the API Lambda and the
   ...
   `agency-app/api/infra/deploy.sh` (`DEPLOY_CFN=false` mode).
...
`agency-app/api/infra` now uses the same `<env>-realestateflow-*` (env-first)
...
| API | env vars (`agency-app/api/.env.<env>`) | dev base path | prod base path |
|-----|-------------------------------|---------------|----------------|
...
- The old single-URL vars `AUTH_SERVICE_URL`, `MCP_BASE_URL`, `AI_CALLING_SERVICE_URL`
  ...
  composes URLs in `agency-app/api/config/serviceUrls.js`, which also rejects any
...
- `deploy`, `config-deploy`, `agency-app/api/infra/deploy.sh` and `agency-app/api/infra/config-deploy.sh`
  all run `agency-app/api/infra/lib/validate-service-endpoints.sh` first: each `*_DOMAIN_NAME`
