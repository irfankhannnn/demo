...
#                                             agency-app/api/infra/config-deploy.sh and
...
# script: agency-app/api/infra/deploy.sh. This wrapper's only job is release
# bookkeeping. Design mirrors infra/cicd/platform/auth
...
#      agency-app/api/infra/deploy.sh itself — never overwritten, no S3 versioning
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../../agency-app/api" && pwd)"
...
# agency-app/api/.env.<env>. Every API endpoint server owns or calls must be an API
# Gateway custom domain + base path pair (<STEM>_DOMAIN_NAME +
# <STEM>_BASE_PATH), never a raw execute-api URL. Shared with
# agency-app/api/infra/deploy.sh and config-deploy.sh.
require_custom_domain_endpoints() {
  local env="$1"
  # shellcheck disable=SC1091
  source "$SERVICE_DIR/infra/lib/validate-service-endpoints.sh"
  if ! validate_service_endpoints "agency-app/api/.env.$env"; then
    echo "Refusing to deploy: fix the custom-domain settings in agency-app/api/.env.$env first."
    exit 1
  fi
}
...
cmd_deploy() {
  ...
  # + base path pair (same guard agency-app/api/infra/deploy.sh runs again itself).
  ...
  if [ "$status" = "deployed" ]; then
    ...
    if [ -f "$artifacts_file" ]; then
      ...
      echo "WARNING: deploy reported success but $artifacts_file was not written — agency-app/api/infra/deploy.sh may be an older version missing this. Artifact keys will be recorded as 'unknown'."
    fi
  fi
...
EOF
  ...
  # reasoning as the direct-update path in agency-app/api/infra/deploy.sh itself.
