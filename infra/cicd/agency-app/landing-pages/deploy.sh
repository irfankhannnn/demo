...
# =============================================================================
# RealEstateFlow Landing Pages — CI/CD entry point, with build/release tracking
# =============================================================================
# Usage:
...
# agency-app/landing-pages/infra/deploy.sh. This wrapper's only job is release
# bookkeeping — see README.md in this folder for the full design, and
# infra/cicd/agency-app/web/README.md for the reference design
# this one is copied from (same shape — a static site, not a Lambda service).
#
...
#   ${SERVICE_NAME}/builds/<build>/<env>/cfn-landing-pages.yaml scoped archive
#                                                               (never overwritten)
#
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../../agency-app/landing-pages" && pwd)"
...
Usage:
  ...
  ./deploy.sh content-deploy <env>            Content-only change (rebuild+sync, no CFN)
...
write_manifest() {
  # write_manifest <out-file> <buildNumber> <env> <status> <stackName> <rollbackOf|null> <rollbackKind|null> <artifactBucket> <buildDistKey> <buildTemplateKey> <siteBucket> <distributionId> <commit> <commitShort> <branch> <dirty> <deployer> <region> <serviceName> <deployDate>
  node -e "
    const fs = require('fs');
    const [ , out, buildNumber, env, status, stackName, rollbackOf, rollbackKind,
            artifactBucket, buildDistKey, buildTemplateKey,
            siteBucket, distributionId,
    ...
    const manifest = {
      ...
      artifact: {
        ...
        // the live site bucket (below) IS the deployed content, and its own
        // S3 versioning is the primary content-rollback mechanism.
...
cmd_deploy() {
  ...
  cp "$SERVICE_DIR/infra/cfn-landing-pages.yaml" "$build_dir/cfn-landing-pages.yaml" 2>/dev/null || true
  ...
  local site_bucket="${FRONTEND_S3_BUCKET_NAME:-${ENV}-${SERVICE_NAME}}"
  local build_prefix="${SERVICE_NAME}/builds/${build}/${env}"
  local build_dist_key="${build_prefix}/dist.tar.gz"
  local build_template_key="${build_prefix}/cfn-landing-pages.yaml"
  ...
  if [ "$status" = "deployed" ]; then
    ...
    "$AWS_BIN" s3 cp "$build_dir/cfn-landing-pages.yaml" "s3://${artifact_bucket}/${build_template_key}" --region "$AWS_REGION" --no-cli-pager
...
# =============================================================================
# content-deploy — a pure content-only change: rebuild + re-sync, skip the
# CloudFormation update. Records a normal numbered build, not a config
# revision (see cmd_config_deploy below for the CFN-parameter-only axis).
# =============================================================================
cmd_content_deploy() {
  local env="$1"
  require_env_arg "$env"
...
cmd_rollback_config() {
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$SERVICE_DIR/infra/cfn-landing-pages.yaml" \
    --stack-name "$stack_name" \
...
cmd_rollback_content() {
  ...
  local artifact_bucket dist_key site_bucket distribution_id
  artifact_bucket="$(json_read "$m" artifact.bucket)"
  dist_key="$(json_read "$m" artifact.buildDistKey)" || true
  site_bucket="$(json_read "$m" site.bucket)"
  distribution_id="$(json_read "$m" site.distributionId)" || true
  ...
  echo "Rolling back content: bucket=$site_bucket  from s3://$artifact_bucket/$dist_key"
...
EOF
  ...
  "$AWS_BIN" s3 sync "$tmp_dir/dist" "s3://${site_bucket}" \
    --region "$AWS_REGION" --delete \
    --cache-control "public,max-age=3600,must-revalidate" \
    --exclude "*.html" --exclude "sitemap.xml" --exclude "robots.txt" --exclude "llms.txt" \
    --no-cli-pager

  HTML_CACHE_CONTROL="public,max-age=0,must-revalidate"
  if [ -d "$tmp_dir/dist" ]; then
    find "$tmp_dir/dist" -name "*.html" | while IFS= read -r f; do
      rel="${f#"$tmp_dir"/dist/}"
      "$AWS_BIN" s3 cp "$f" "s3://${site_bucket}/${rel}" \
        --region "$AWS_REGION" --cache-control "$HTML_CACHE_CONTROL" --no-cli-pager
    done
    for f in sitemap.xml robots.txt llms.txt; do
      if [ -f "$tmp_dir/dist/$f" ]; then
        "$AWS_BIN" s3 cp "$tmp_dir/dist/$f" "s3://${site_bucket}/${f}" \
          --region "$AWS_REGION" --cache-control "$HTML_CACHE_CONTROL" --no-cli-pager
      fi
    done
  fi
  ...
  cp "$build_dir/cfn-landing-pages.yaml" "$new_dir/" 2>/dev/null || true
...
cmd_rollback_full() {
  ...
  verify_build_env "$build" "$env" "$m"
  if [ ! -f "$build_dir/cfn-landing-pages.yaml" ] || [ ! -f "$build_dir/cfn-params.json" ]; then
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$build_dir/cfn-landing-pages.yaml" \
