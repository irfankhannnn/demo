# infra/cicd/agency-app/followup-agent

CI/CD wrapper for the follow-up agent microservice (`agency-app/followup-agent/`).
...
`agency-app/followup-agent/infra/deploy.sh` does the real work: test, install,
...
- Node 20+, `zip`
- `agency-app/followup-agent/.env.<env>` filled in (see its `.env.example`)
...
Reads `agency-app/followup-agent/.env.<env>` (gitignored). Stack name is
