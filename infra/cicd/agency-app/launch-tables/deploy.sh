...
# agency-app/api/infra/deploy-launch-tables.sh. This wrapper's only job is release
...
# AWS::DynamoDB::Table resources (agency-app/api/infra/launch-tables-cfn.yaml). So
...
#     nested-template key — see agency-app/api/infra/deploy-launch-tables.sh)
...
cmd_deploy() {
  ...
  if [ "$status" = "deployed" ]; then
    ...
    if [ -f "$artifacts_file" ]; then
      ...
      echo "WARNING: deploy reported success but $artifacts_file was not written — agency-app/api/infra/deploy-launch-tables.sh may be an older version missing this. Artifact keys will be recorded as 'unknown'."
    fi
  fi
