# infra/cicd/agency-app/ai-calling

CI/CD wrapper for the AI calling microservice (`agency-app/ai-calling/`). Same
...
`agency-app/ai-calling/infra/deploy.sh` does the real work: install, package,
...
Reads `agency-app/ai-calling/.env.<env>` (gitignored). Both `.env.dev` and
...
one until they are supplied. See `agency-app/ai-calling/.env.example` for where
...
`agency-app/ai-calling/infra/cfn-params.json` itself.
