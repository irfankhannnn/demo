...
# =============================================================================
# property-pages-ms - CI/CD entry point, with build/release tracking
# =============================================================================
# Usage:
...
# public-app/property-pages/infra/deploy.sh. This wrapper's only job is release
...
#   <prefix>/builds/<build>/<env>/cfn-property-pages.yaml   permanent, never
...
# CloudFront distribution (EnableCloudFront param / PagesDistributionId
# output). HTML is cached at the edge, so a Lambda code change — from a
...
# helper as public-app/property-pages/infra/deploy.sh, reused here for rollback-full,
...
get_distribution_id() {
  # get_distribution_id <stack-name> — prints the PagesDistributionId output,
  ...
  "$AWS_BIN" cloudformation describe-stacks --stack-name "$1" \
    ...
    --query "Stacks[0].Outputs[?OutputKey=='PagesDistributionId'].OutputValue" \
...
invalidate_distribution() {
  ...
  # this service (EnableCloudFront param), so an absent PagesDistributionId
  ...
  "$AWS_BIN" cloudfront create-invalidation --distribution-id "$dist_id" --paths "/*" \
    ...
    echo "  WARNING: CloudFront invalidation failed (non-fatal) — stale HTML may serve until it expires from cache"
}
...
cmd_deploy() {
  ...
  echo "============================================="
  echo " property-pages-ms - build $build -> $env"
  echo "============================================="
  ...
  cp "$SERVICE_DIR/infra/cfn-property-pages.yaml" "$build_dir/" 2>/dev/null || true
...
EOF
  ...
  for f in cfn-property-pages.yaml cfn-params.json manifest.json; do
  ...
  # HTML is cached at the CDN edge — without this, a successful code deploy
...
cmd_config_deploy() {
  ...
  echo "============================================="
  echo " property-pages-ms - config-only deploy -> $env"
  echo "============================================="
  echo "Branch:  $GIT_BRANCH ($GIT_COMMIT_SHORT, dirty=$GIT_DIRTY)"
  echo ""
...
EOF
  ...
  # Some config-only-safe values (GoogleMapsEmbedApiKey, business-logic
  # constants) directly affect the HTML this service renders, same as a
...
cmd_rollback_config() {
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$(winpath "$SERVICE_DIR/infra/cfn-property-pages.yaml")" \
    --stack-name "$STACK_NAME" \
    --capabilities CAPABILITY_NAMED_IAM \
    --no-fail-on-empty-changeset \
    --tags Environment="$env" Service=realestateflow-pages \
...
EOF
  ...
  "$AWS_BIN" lambda update-function-code \
    --function-name "${env}-realestateflow-pages-lambda" \
    --s3-bucket "$bucket" \
    --s3-key "$key" \
    --region "$AWS_REGION" \
    --profile "$AWS_PROFILE" \
...
cmd_rollback_full() {
  ...
  local build_dir="$VERSIONS_DIR/$build"
  local tpl="$build_dir/cfn-property-pages.yaml"
  ...
  if [ ! -f "$tpl" ] || [ ! -f "$prm" ]; then
    ...
    mkdir -p "$build_dir"
    "$AWS_BIN" s3 cp "s3://${ARTIFACT_BUCKET}/${prefix}/cfn-property-pages.yaml" "$tpl" \
      --region "$AWS_REGION" --profile "$AWS_PROFILE" --no-cli-pager
