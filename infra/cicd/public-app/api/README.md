# infra/cicd/public-app/api

CI/CD wrapper for the consumer marketplace API
(`public-app/api/` — Express on Lambda behind API Gateway,
...
`public-app/api/infra/deploy.sh` does the real work:
...
- `public-app/api/.env.dev` and/or `.env.prod`, copied
...
Reads `public-app/api/.env.<env>`. There is no plain
