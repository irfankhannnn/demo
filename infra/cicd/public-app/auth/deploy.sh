...
# =============================================================================
# marketplace-authentication — CI/CD entry point, with build/release tracking
# =============================================================================
# Usage:
...
# script: public-app/auth/infra/deploy.sh (full) or
# public-app/auth/infra/config-deploy.sh (config-only). This
...
#   ${SERVICE_NAME}/builds/<build>/<env>/cfn-params.json
...
#     infra/cicd/agency-app/api/deploy.sh for a real two-zip-shared case)
#
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../../public-app/auth" && pwd)"
...
write_manifest() {
  # write_manifest <out-file> <buildNumber> <env> <status> <stackName> <rollbackOf|null> <rollbackKind|null> <bucket> <codeKey> <codeVersionId> <codeStorageClass> <buildCodeKey> <buildTemplateKey> <buildParamsKey> <commit> <commitShort> <branch> <dirty> <deployer> <region> <serviceName> <deployDate>
  node -e "
    const fs = require('fs');
    const [ , out, buildNumber, env, status, stackName, rollbackOf, rollbackKind,
            bucket, codeKey, codeVersionId, codeStorageClass,
            buildCodeKey, buildTemplateKey, buildParamsKey,
    ...
    const manifest = {
      ...
      artifact: {
        ...
        buildParamsKey: buildParamsKey || null,
      },
      rollbackOf: rollbackOf === 'null' ? null : rollbackOf,
...
cmd_deploy() {
  ...
  local build_params_key="${build_prefix}/cfn-params.json"
  ...
  if [ "$status" = "deployed" ]; then
    ...
    # Permanent build+env archive copies (server-side copy for the code;
    # cfn-backend.yaml / cfn-params.json are fresh uploads since
    # infra/deploy.sh never puts them in S3 itself — the template is
    # inline-deployed, under 51.2KB, and there is no nested routes template
    # for this service).
    ...
    "$AWS_BIN" s3 cp "$build_dir/cfn-params.json" "s3://${bucket}/${build_params_key}" --region "$AWS_REGION" --no-cli-pager

    tag_object "$bucket" "$code_key" "$status"
    tag_object "$bucket" "$build_code_key" "$status"
    tag_object "$bucket" "$build_template_key" "$status"
    tag_object "$bucket" "$build_params_key" "$status"
  else
    build_code_key=""
    build_template_key=""
    build_params_key=""
  ...
  write_manifest "$build_dir/manifest.json" \
    ...
    "$bucket" "$code_key" "$code_version" "$code_storage_class" \
    "$build_code_key" "$build_template_key" "$build_params_key" \
...
EOF
  ...
  write_manifest "$new_dir/manifest.json" \
    ...
    "$(json_read "$m" artifact.buildParamsKey 2>/dev/null || echo '')" \
    "$GIT_COMMIT" "$GIT_COMMIT_SHORT" "$GIT_BRANCH" "$GIT_DIRTY" "$DEPLOYER" "${AWS_REGION:-unknown}" "$SERVICE_NAME" "$DEPLOY_DATE"
...
cmd_rollback_full() {
  ...
  # The template is self-contained (single {proxy+} route, no nested
  # stack), so the local snapshot + its cfn-params.json is everything a
  # full rollback needs.
