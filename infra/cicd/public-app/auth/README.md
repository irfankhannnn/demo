...
```
public-app/auth/infra/deploy.sh          (full deploy)
public-app/auth/infra/config-deploy.sh   (config-only)
```
...
same design as `infra/cicd/platform/auth/` minus the nested
...
Requires `public-app/auth/.env.<env>` (copy
