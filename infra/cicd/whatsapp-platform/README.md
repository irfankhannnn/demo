...
```
services/whatsapp-platform/infra/deploy.sh
```
...
```
cd infra/cicd/whatsapp-platform
...
auth/server — it uses a single `services/whatsapp-platform/.env` for shared config
...
`services/whatsapp-platform/README.md` for the full picture.
