...
# script: services/whatsapp-platform/infra/deploy.sh. This wrapper's only job is
# release bookkeeping — see README.md in this folder for the full design.
# Design mirrors infra/cicd/reality-flow-authentication and
# infra/cicd/server (same manifest shape, same global build counter,
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../services/whatsapp-platform" && pwd)"
...
# Same helper as apps/crm/server/infra/deploy.sh and services/whatsapp-platform/infra/deploy.sh.
winpath() {
...
# also global across all envs, mirroring services/reality-flow-authentication/server's
