...
# =============================================================================
# marketplace-api - CI/CD entry point, with build/release tracking
# =============================================================================
# Usage:
...
# apps/marketplace/marketplace-api/infra/deploy.sh. This wrapper's only job is release
# bookkeeping, matching the design already used by
# infra/cicd/{server,reality-flow-authentication,backend_insta_sol_ms}.
#
...
#   <prefix>/builds/<build>/<env>/cfn-marketplace-api.yaml   permanent, never
...
# (This is bug (a) fixed in backend_insta_sol_ms's wrapper: an earlier
# version tagged only one of these two sets and left the other silently
# untagged. Both cmd_deploy's tagging loops below cover both sets.)
#
# CloudFront: this service optionally fronts its API Gateway with a
# CloudFront distribution (EnableCloudFront param / MarketplaceDistributionId
# output). Public JSON reads are cached at the edge, so a Lambda code change — from a
# fresh deploy, a rollback-code, or a rollback-full — is invisible to
# visitors until that cache is busted. Every command that can change what
# the Lambda serves therefore invalidates /* afterward via
# invalidate_distribution, which no-ops cleanly when the stack has no
# distribution (EnableCloudFront=false is a supported, non-error config).
#
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../apps/marketplace/marketplace-api" && pwd)"
...
# any --template-file argument has to be handed a D:/... path instead. Same
# helper as apps/marketplace/marketplace-api/infra/deploy.sh, reused here for rollback-full,
# which also calls `cloudformation deploy --template-file`.
winpath() {
...
get_distribution_id() {
  # get_distribution_id <stack-name> — prints the MarketplaceDistributionId output,
  # or an empty string when the stack has no distribution (EnableCloudFront
  # was false). Best-effort: a describe-stacks failure prints nothing rather
  # than aborting the caller.
  "$AWS_BIN" cloudformation describe-stacks --stack-name "$1" \
...
invalidate_distribution() {
  # invalidate_distribution <stack-name> <label> — busts the edge cache after
  # anything that changes what the Lambda serves. CloudFront is optional for
  # this service (EnableCloudFront param), so an absent MarketplaceDistributionId
  # output is a normal, silent no-op here, not an error.
  local stack="$1" label="$2"
  local dist_id
  dist_id="$(get_distribution_id "$stack")"
  if [ -z "$dist_id" ]; then
    echo "  (no CloudFront distribution on $stack — skipping invalidation)"
    return 0
  fi
  echo "  Invalidating CloudFront distribution $dist_id ($label)..."
  "$AWS_BIN" cloudfront create-invalidation --distribution-id "$dist_id" --paths "/*" \
    --profile "$AWS_PROFILE" --no-cli-pager >/dev/null 2>&1 || \
    echo "  WARNING: CloudFront invalidation failed (non-fatal) — stale responses may serve until it expires from cache"
}
...
cmd_deploy() {
  ...
  echo "============================================="
  echo " marketplace-api - build $build -> $env"
  echo "============================================="
  ...
  cp "$SERVICE_DIR/infra/cfn-marketplace-api.yaml" "$build_dir/" 2>/dev/null || true
  ...
  local distribution_id="none"
  if [ "$status" = "deployed" ]; then
    distribution_id="$(get_distribution_id "$STACK_NAME")"
    [ -z "$distribution_id" ] && distribution_id="none"
  fi
...
EOF
  ...
  # below. This is bug (a): tagging only one of the two sets leaves the
  # other silently untagged, and both are "every object this script touches".
  ...
  # Permanent, build+env-scoped archive in S3 — also tagged, per the same
  # "every object this script touches" rule as above.
  local prefix="${ARTIFACT_PREFIX}/builds/${build}/${env}"
  for f in cfn-marketplace-api.yaml cfn-params.json manifest.json; do
  ...
  # Public reads are cached at the CDN edge — without this, a successful code deploy
  # would silently not be visible to visitors until the cache naturally expires.
  if [ "$status" = "deployed" ]; then
    invalidate_distribution "$STACK_NAME" "deploy build $build"
  fi
...
cmd_config_deploy() {
  ...
  echo "============================================="
  echo " marketplace-api - config-only deploy -> $env"
  echo "============================================="
  echo "Branch:  $GIT_BRANCH ($GIT_COMMIT_SHORT, dirty=$GIT_DIRTY)"
  echo ""
...
EOF
  ...
  # Some config-only-safe values (MarketplaceWebOrigin, limits, model settings
  # ) directly affect what this service serves, same as a
  # code change would — bust the edge cache the same way cmd_deploy does.
  invalidate_distribution "$STACK_NAME" "config-only deploy to revision #$cfg"
...
# values on top of whatever code build is CURRENTLY live. Recorded as a new
# forward config revision.
# =============================================================================
cmd_rollback_config() {
  local env="$1" cfg="$2"
  require_env_arg "$env"
  ...
  "$AWS_BIN" cloudformation deploy \
    --template-file "$(winpath "$SERVICE_DIR/infra/cfn-marketplace-api.yaml")" \
    --stack-name "$STACK_NAME" \
    --capabilities CAPABILITY_NAMED_IAM \
    --no-fail-on-empty-changeset \
    --tags Environment="$env" Service=realestateflow-marketplace-api \
    --parameter-overrides "${PARAM_OVERRIDES[@]}" \
    --region "$AWS_REGION" --profile "$AWS_PROFILE" --no-cli-pager

  invalidate_distribution "$STACK_NAME" "rollback-config to revision #$cfg"
...
verify_build_env() {
  ...
  # infra/cicd/server and reality-flow-authentication.
  ...
  if [ "$recorded_env" != "$env" ]; then
    ...
    echo "       point it at the wrong tables/data. Refusing."
    exit 1
  fi
}
...
EOF
  ...
  "$AWS_BIN" lambda update-function-code \
    --function-name "${env}-realestateflow-marketplace-api-lambda" \
    --s3-bucket "$bucket" \
    --s3-key "$key" \
    --region "$AWS_REGION" \
    --profile "$AWS_PROFILE" \
  ...
  # Same reasoning as cmd_deploy: this changed what the Lambda serves, so the
  # edge cache is now stale until busted.
  invalidate_distribution "$STACK_NAME" "rollback-code to build $build"
}
...
cmd_rollback_full() {
  ...
  local build_dir="$VERSIONS_DIR/$build"
  local tpl="$build_dir/cfn-marketplace-api.yaml"
  ...
  if [ ! -f "$tpl" ] || [ ! -f "$prm" ]; then
    ...
    mkdir -p "$build_dir"
    "$AWS_BIN" s3 cp "s3://${ARTIFACT_BUCKET}/${prefix}/cfn-marketplace-api.yaml" "$tpl" \
      --region "$AWS_REGION" --profile "$AWS_PROFILE" --no-cli-pager
  ...
  # already cached locally. This is bug (b): an earlier version only pulled
  # (and therefore only checked) the manifest when the template/params were
  # ALSO missing locally — so a build whose template+params snapshot was
  # cached but whose manifest.json had been deleted (or never copied) would
  # fall straight through to `cloudformation deploy` with no dev/prod check
  # at all. The manifest fetch and the guard below run every time, whether
  # or not the template/params needed pulling above.
  if [ ! -f "$m" ]; then
  ...
  # Template+params (and therefore code, via LambdaCodeS3Key) just changed —
  # same edge-cache staleness as cmd_deploy and cmd_rollback_code.
  invalidate_distribution "$STACK_NAME" "rollback-full to build $build"
}
