...
```
services/reality-flow-authentication/infra/deploy.sh
```
...
```
cd infra/cicd/reality-flow-authentication
...
than one Lambda artifact (see `infra/cicd/server/`, which has an
...
1. Resolves `ENV_FILE = services/reality-flow-authentication/.env.<env>` — the
...
| `SUBSCRIPTIONS_TABLE` | `dev-realestateflow-subscriptions` | `prod-realestateflow-subscriptions` | Must exactly match what `apps/crm/server/infra/launch-tables-cfn.yaml` names it for the *same* `EnvironmentName` — this stack only reads it |
