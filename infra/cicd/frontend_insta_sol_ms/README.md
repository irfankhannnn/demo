...
(`apps/instagram/frontend_insta_sol_ms/`). Same design as the `server` and
`reality-flow-authentication` wrappers in this folder.

## What this adds over the raw deploy script

`apps/instagram/frontend_insta_sol_ms/infra/deploy.sh` does the real work: install, package,
...
Reads `apps/instagram/frontend_insta_sol_ms/.env.<env>`. `.env.prod` targets account
