...
```
apps/crm/real-estate-crm-app/infra/deploy.sh
```
...
`infra/cicd/reality-flow-authentication` and `infra/cicd/server`,
...
```
cd infra/cicd/real-estate-crm-app
...
`apps/crm/real-estate-crm-app/infra/lib/api-domain-guard.sh` against `.env.<env>`:
...
1. Resolves `ENV_FILE = apps/crm/real-estate-crm-app/.env.<env>` — the argument
