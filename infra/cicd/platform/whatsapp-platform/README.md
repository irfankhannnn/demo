...
```
platform/whatsapp-platform/infra/deploy.sh
```
...
```
cd infra/cicd/platform/whatsapp-platform
...
auth/server — it uses a single `platform/whatsapp-platform/.env` for shared config
...
`platform/whatsapp-platform/README.md` for the full picture.
