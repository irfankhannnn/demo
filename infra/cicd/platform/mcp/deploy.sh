...
#
# Usage: ./deploy.sh [OPTIONS] [dev|test|prod]   (run from infra/cicd/platform/mcp/)
#
...
#   --config-only         Config-only deploy — ../../../../platform/mcp/infra/
...
# service's own script, which deploys platform/mcp/infra/cfn-backend.yaml
# — the single source of truth — and writes platform/mcp/infra/
...
SERVICE_DIR="$(cd "$SCRIPT_DIR/../../../../platform/mcp" && pwd)"
