...
- Deleted `server/agents/intentResolver.js`
- Deleted `server/agents/intentResolver.test.js`
...
Created `server/agents/inputNormalizer.js` with:
...
### Deleted
- `server/agents/intentResolver.js`
- `server/agents/intentResolver.test.js`

### Created
- `server/agents/inputNormalizer.js` (241 lines) — money, date, phone normalization
- `server/agents/inputNormalizer.test.js` (164 lines) — unit tests for normalization

### Modified
- `server/agents/agentRuntime.js` — removed fallback, fixed retry logic, added `rawText` return from LLM loops, exported `shouldRetryForToolCall` and `validateJsonOutput`
- `server/agents/prompts.js` — completely rewrote WhatsApp prompt (147 new lines)
- `server/agents/agentRuntime.test.js` — fixed 1 test expectation, added retry logic and JSON validation tests
- `server/skillInvoker.js` — enhanced all tool descriptions, integrated input normalizer
