...
#
# Loads ai-calling-service/.env.dev or .env.prod (never a plain .env) and FORCES
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy ai-calling-service/.env.example to $ENV_FILE and fill in the values."
  exit 1
fi
...
Where each comes from:
  CRM_INTERNAL_API_KEY       must equal server/.env.${DEPLOY_ENV}'s AI_CALLING_INTERNAL_API_KEY
