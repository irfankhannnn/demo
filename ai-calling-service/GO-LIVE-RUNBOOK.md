...
`ai-calling-service/` places outbound AI voice calls to real estate leads: the
...
|---|---|
| `CRM_INTERNAL_API_KEY` | Must equal `AI_CALLING_INTERNAL_API_KEY` in `server/.env.dev` |
...
| Thing | Where |
|---|---|
| Agent system prompt (paste-ready) | `ai-calling-service/elevenlabs-agent-prompt.md` |
| Server tool definitions (paste-ready) | `ai-calling-service/elevenlabs-agent-tools.md` |
| CFN template | `ai-calling-service/infra/cfn-ai-calling.yaml` |
| Deploy | `cfn-templates-cicd/ai-calling-service/deploy.sh dev\|prod` |
| Env contract | `ai-calling-service/.env.example` |
| Architecture + known gaps | `ai-calling-service/README.md` |
