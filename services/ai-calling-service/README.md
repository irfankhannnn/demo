...
consumers dedupe on `callSessionId`. Shape: `docs/services/followup-agent-service/CONTRACTS.md`
...
pattern in `apps/crm/server/config/ssmBootstrap.js`. `lambda-handler.js` defers importing
...
Infrastructure lives in `services/ai-calling-service/infra/cfn-ai-calling.yaml` and is
deployed through the CI/CD wrapper, not by invoking `aws cloudformation deploy`
by hand:

```bash
infra/cicd/ai-calling-service/deploy.sh dev
infra/cicd/ai-calling-service/deploy.sh prod
```
...
- **File upload for the knowledge base is not implemented.**
  ...
  the CRM under Agency Policies and indexed by `apps/crm/server/services/knowledge/`.
