...
#
# Loads services/ai-calling-service/.env.dev or .env.prod (never a plain .env) and FORCES
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Copy services/ai-calling-service/.env.example to $ENV_FILE and fill in the values."
  exit 1
fi
...
Where each comes from:
  CRM_INTERNAL_API_KEY       must equal apps/crm/server/.env.${DEPLOY_ENV}'s AI_CALLING_INTERNAL_API_KEY
  ...
  CRM_CALLER_API_KEY         generate one, e.g. \`openssl rand -hex 32\`, and set the
                             same value in apps/crm/server/.env.${DEPLOY_ENV} so the CRM
