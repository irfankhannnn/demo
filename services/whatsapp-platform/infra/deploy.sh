...
# infra/cicd/whatsapp-platform/deploy.sh calls for every deploy. Run
...
# infra/cicd/whatsapp-platform/README.md for that design.
#
# Usage:
...
# Same helper as apps/crm/server/infra/deploy.sh — see its comment for the full reasoning.
winpath() {
...
case "${ENV_NAME}" in
  dev)
    ...
    # naming convention now used by services/reality-flow-authentication/server.
...
# runs (LOCAL_STORAGE=true, no ECS involved) — see services/whatsapp-platform/.env.sample.
ENV_FILE="${ROOT_DIR}/.env.${ENV_NAME}"
if [ -f "${ENV_FILE}" ]; then
  set -a; source "${ENV_FILE}"; set +a
elif [ "${COMMAND}" = "deploy" ]; then
  echo "ERROR: ${ENV_FILE} not found — create it before deploying ${ENV_NAME} (see services/whatsapp-platform/.env.sample)."
  exit 1
else
...
# (infra/cicd/whatsapp-platform/deploy.sh) to read afterward — it
