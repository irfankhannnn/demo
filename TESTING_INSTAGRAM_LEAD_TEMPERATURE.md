...
`<50L` → 4,900,000 (lower bound minus 1L, conservative) · `50L-80L` → 5,000,000 · `1Cr+` → 10,000,000. If these look wrong for your actual ManyChat bracket copy, adjust `parseBudgetBracket()` in `server/routes/webhooks.js` — it's a pure function, easy to unit test in isolation.
...
1. Deploy `ai-calling-service/cfn-template.yaml` with real Exotel + ElevenLabs credentials.
...
4. During the call, the agent should ask 2-3 short questions per the rubric (`server/utils/leadRubric.js`) and end within ~90 seconds.
...
node server/scripts/backfill-lead-temperature.js --tenant=<tenantId>

# Review the printed JSON summary: migrated / leftUnscored / skippedAlreadyScored counts

# Then actually write
node server/scripts/backfill-lead-temperature.js --apply --tenant=<tenantId>
```
...
- `ai-calling-service/cfn-template.yaml` doesn't wire a `WEBHOOK_BASE_URL` env var — pre-existing gap, needed before ElevenLabs intent webhooks work live, unrelated to this migration.
